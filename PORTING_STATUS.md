...
## 第 530 轮：第十八把尺子——比较一个已经被自己覆盖掉的值

接上一轮的"下一步"：上一轮那个"先赋值后比较"的错，还有没有别处。

### 查的结果：没有第二处

扫了全 crate **198 个文件里的 106 个 `update_from`**。
第一版扫描按"赋值之后还读 `self.X`"报了 11 处，
**逐一看过，全是对的**——按新的数目裁剪子节点列表、
用新值搭一个 staged 对象，这些本来就该读新值。

**真正的信号要窄得多**：赋值之后拿 `self.X` 去和 `fresh.*` **比较**。
按这个窄条件再扫，**一处都没有**。

### 但"查过了"会过期，所以做成尺子

`tools/dead_guards.py`，第十八把。理由写在文件头：
**这类错没有任何东西拦得住**——编译通过、测试全绿、那条分支只是永远不走。
它不是"算错了"（断言能看见），是"根本没算"。

宽的那条规则**故意不报**：那 11 处正确的读法要是每轮都报一次，
**一把没人信的尺子就是一把没人看的尺子**。窄的那条正是真出过错的那条，
而且不用读上下文意图就能判定。

### 尺子自己也要被验一次

**一把从没见过自己那个案例亮红的尺子，是没人验过的尺子。**
所以把第 529 轮的旧代码原样放回去跑了一遍：

    render.rs:21737  self.behavior assigned, then `self.behavior != fresh.behavior`

exit 1，点名到行、到表达式；换回修好的代码，exit 0。

尺子：**十八把**全部 exit 0。门：Rust 6978 通过、`cargo fmt --check` 干净；
C++ 52 个 gtest 全过；gallery 357 通过；`rustflutter_unittests` 6978 通过；
三个目录 default 与 `rustflutter_engine` 都 exit 0。

**下一步**：回到第 529 轮记下、当时判断"不是一轮的量"的那件事——
**焦点高亮**。现在 `operable` 有五个用户，谁都看不见键盘在哪儿。
拆开做的第一刀应该是**形状**：`ink_well.rs` 的高亮要各控件自己给形状
（胶囊/圆/方），所以第一轮该做的是
**给 `operable` 加一个形状参数并让五个调用点各自说出自己的形状**，
...
## 第 536 轮：三种状态，三种颜色——字段一直在，只是没人读

先查上一轮记的那件事：`ThemeData` 里有没有那三个颜色。
**三个都有**（theme.rs 第 216–218 行），而且默认值**和上游一模一样**：
亮色下 12% 黑、4% 黑、`0x66bcbcbc`，暗色下对应的白和 `0x40cccccc`。

再查谁读它们：**只有 lerp 读**（component_themes.rs 里插值那两处）。
也就是说这三个字段**存了、会插值、从不上屏**——
每一种状态都画着同一个 `theme.primary` 的 12%。
所以这一轮不是加字段，是**接上去**。

（顺带一提：这正是 `unread_theme_fields` 那把尺子管的species，
但它只盯"决定分支的字段"，颜色不在其中——**尺子看得见的和看不见的，
边界本身也值得记一笔**。）

### 三层，不是一层取最大

第 534 轮我写过一句理由："三种颜色一样，所以叠成一层取最大，
两层同色会读成第三种更深的状态"。**前提没了这句就不成立**——
颜色一旦不同，upstream 的做法就是**真的叠三层**，
鼠标停在一个已经聚焦的控件上，看到的应该是两层，不是一个混出来的答案。

改成按 hover → focus → pressed 的顺序压栈，各用各的颜色、各按各的淡入进度
缩放**自己那个颜色的 alpha**（而不是统一一个常数）。
第 534 轮那句话随之删掉——**前提变了，理由就得跟着变**。

### 测试要读颜色，不是读"有没有"

之前的测试只问"画了几个矩形、alpha 多少"。
这一轮那条读的是**完整的 argb 列表**，并且断言
"悬停时只画 hoverColor"、"再聚焦时是 hover 在下、focus 在上的两层"。
末尾专门加一句 `assert_ne!(hover_color, focus_color)`——
**两个颜色要是碰巧相等，上面那条断言什么也证明不了**。

五个变异全部杀死：全用焦点色、两色对调、叠放顺序倒过来、
没开始淡入的层也画、不按淡入进度缩放。

尺子：十八把全部 exit 0。门：Rust 6990 通过、`cargo fmt --check` 干净；
C++ 52 个 gtest 全过；gallery 357 通过；`rustflutter_unittests` 6990 通过；
三个目录 default 与 `rustflutter_engine` 都 exit 0。

**下一步**：这一套（节点、激活、三层高亮）现在只服务
`operable` 的五个控件，而 `ink_well.rs` 里**另有一整套**
同样三种状态的 ink feature 实现（`HighlightType` 那套）。
**两套实现，一个概念**——这正是本项目反复清理的那种东西。
**先查一件事**：`InkWell` 的三种高亮和 `StateHighlight` 的三层，
是不是真的能合并——差别在于前者是 ink feature（有水波、有形状回调、
...
## 换方向：剩下的不是"类"，是"成员"——于是有了 AST 与生成器

用户要求重新考虑移植方向：把剩下的类的 AST 全部 dump 出来，写转换工具直接给出。
先查了一件事，结果改变了这件事该怎么做。

### 先量一量：**没有"剩下的类"了**

`coverage.py`：**2107 个公开类、598 个文件，2106 个有着落，0 个 MISSING**。
所以"剩下的类"是个空集合，74% covered / 17% mapped 里缺的
**全是成员深度**——一个类在，但它的 80 个成员只答了 10 个。
方向因此不是"再找类来移植"，而是**按成员来推进**，
而 `depth.py --all` 只印前 20 行，看不见队伍有多长。

### 一、`tools/dart/dump_ast.dart`——用真正的分析器读上游

这个目录里其他工具都是用正则读 Dart 的。**数**成员够用（coverage/depth 就是这么做的），
**移植**一个成员不够：正则分不出具名参数和位置参数、带不下默认值、
找不到那句"为什么是这个默认值"的文档。而这三样恰恰是移植需要的大部分东西。

所以改用 `package:analyzer`，走 Flutter checkout 自己的 package config。
故意用 `parseFile`（未解析的 AST）而不是完整分析会话：
解析要把每个 import 都加载起来，在整个 framework 上要跑几分钟；
这里要的是每个声明的**形状**，那是 parse 级别的事实。
代价是类型是作者写下的那串文本（`Color?`、`ValueChanged<bool>`）而不是解析后的元素——
**对驱动移植来说这反而是更有用的那个，因为它就是上游读者看到的东西**。

**685 个文件、5364 条声明、0 个解析失败。**
解析失败会报到 stderr 并以 1 退出：清单上有个不出声的洞，比清单明说自己停在哪里更糟。

### 二、`tools/port.py`——把 dump 接到 Rust 这边

`--list` 按缺口排队（826 个类还有成员没答），`<Class>` 报单个类，`<Class> --emit` 生成骨架。

三件让它说实话的事：

- **`{@macro}` 要展开。** Flutter 把文档写一遍、让所有共享该行为的 widget 指过去，
  所以最有价值的注释都不在成员那里——`SelectableText.showCursor` 的文档正文
  就是一行 `{@macro flutter.widgets.editableText.showCursor}`。
  展开与否，是**把理由带下来**和**把一个指针带下来**的区别。
- **`this.x` 这种构造参数不带类型**，得回去跟字段的声明类型对上。
- **认不出的类型就写 `TODO`，不猜。** `TYPES` 表是故意短的：
  猜错的类型比 `TODO` 糟，因为**猜错的类型能编译**。

### 三、边界：它生成脚手架，**不生成行为**

每个函数体都是 `todo!()`，这是设计，不是没写完。
真正花掉那些轮次的，是判定"焦点高亮在 50ms 内淡入而不是 200ms"（第 534 轮）、
"`showCursor` 默认等于 `!readOnly`"（第 519 轮）、
"只解析 `{hovered}` 会丢掉 chip 的选中态"（第 539 轮）。
**一个会吐出貌似合理函数体的转译器，这三处会全答错**——而能编译的错答案比缺答案更糟。

### 四、拿 rustfmt 当解析器，826 个类全过一遍

生成器"差不多是 Rust"等于生成完还得手修，那正是它本该省下的功夫。
rustfmt 不肯格式化它解析不了的东西，所以拿它当**解析检查**，不需要 crate 上下文。

第一遍：**826 个里 22 个解析失败**。三类真 bug，都在名字上：

1. **Dart 的运算符声明**（`operator -`、`[]`、`&`……）成员名就是运算符本身，
   直接输出就成了 `pub fn -()`。现在映射成 `op_sub` 之类，把原运算符留在上面的注释里——
   该变成 `impl Sub` 还是别的，是移植者的判断，不是生成器的。
2. **Rust 关键字撞名**：`type` 在 Dart 里是普通 getter（`LocalizationsDelegate.type`）
   也是普通参数；`fn`、`ref`、`move` 会出现在参数名里。
   用后缀 `_` 而不是 `r#type`——因为这些名字要**被拼接**：
   字段的 builder 是 `with_` 加名字，而 `with_r#type` 根本不是标识符。
3. 顺带修掉一个**比解析错误更坏**的 bug：类型认不出来时字段被注释掉，
   但它的 `///` 文档没跟着注释掉。这不会报错——
   那段文档会**悄悄挂到下一个真字段头上**，
   于是这个 port 就有了一个带着别人解释的字段。`MediaQuery` 里就是这样，而且它当时是"通过"的。

修完再全量跑：**826 个全部解析通过。**

检查脚本本身也修了一处：`capture_output` 没设 `errors='replace'` 时，
控制台解不出的字符会打死读取线程，`stderr` 变空，
于是**恰恰在它没看成的那个文件上报告"通过"**——尺子在自己失灵的地方读数最干净。

### 产物不入库

`tools/upstream_ast.json` 33MB，而且**绑在某一个上游修订版上**。
仓库变大是小事，真正的问题是它会悄悄过期：读它的人拿到上游"当时"的样子，
而这个工具正是用来对齐上游"现在"的样子。所以 gitignore，
文件不在时 `port.py` 直接把重新生成的命令打出来。

十八把尺子全部 exit 0。本轮不动任何 Rust 源码，
因此第 539 轮验过的 ninja/测试门没有重跑——没有可跑出不同结果的输入。

**下一步**：拿 `--list` 的队头真正走一遍全流程，
建议从 `MediaQuery`（70 缺口、表面干净）或 `SemanticsConfiguration` 起，
把生成的骨架**合并**进已有类型（不是并排粘贴），
再按老规矩逐个成员填行为 + 变异验证。
另外 `SliverSemantics` 报 0/86——Rust 侧找到了类型却一个成员都没对上，
先查清是名字对不上还是真的空壳，**这个数字本身要先被信任**。
