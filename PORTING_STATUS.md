...
**Windows**:框架 407 个测试、FFI 17 个、flutter_gallery 21 个、相册 60 个
...
读回 `rustflut`。clippy 警告数与改动前持平。

**Android**(模拟器,Android 14,x86_64):九个示例的截图与第二十一条之后
**逐像素相同**(状态栏以下);`platform_channels` **PASS**;flutter_gallery
滑动 → 打开 demo → 返回,内容与列表位置都对 —— 甩动的落点在这台模拟器上不可
重现(同一个构建连跑两次也不一样),所以这一步看的是内容而不是字节;用一个
单独构建的无障碍服务读 showcase,树照读、`CLICKED|Primary|true`;相册 150 张
图的九宫格与看图页与之前**逐像素相同**。
...
- **语义收集仍然长在绘制上**。第二十一条让边界在有读者时不交旧层,这是对的,
  但代价是读屏软件打开时重绘边界全部失效。上游让语义树自己走一趟渲染树,这里
  做不到是因为渲染对象不知道自己的父亲 —— 而它现在知道了(第十八条的父链)。
  这条路是通的,只是还没走。
