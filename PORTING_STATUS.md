...
- **文字缩放是全局的，不能按子树给** —— 写下这一条的时候是。它当时在
  `painting::shape` 上应用，覆盖是全的，但本该在上一层：上游 `MediaQuery.textScaler`
  是每个 `Text` 从 widget 树上读的，于是一张密集表格可以退出缩放。它要等
  `InheritedWidget` 的依赖追踪，而那两件都在第十七节做了（第 2 条与第 13 条）。
...
- **视图内边距没报** —— 写下这一条的时候没报，状态栏、刘海、手势条的
  高度一律是零。第十七节第 1 条把 `onApplyWindowInsets` 照上游移了过来，
  现在填的是真值。（软键盘那一路一直是好的：窗口设了 `adjustResize`，
  键盘弹出会重排窗口，于是 `surfaceChanged` 会带着新尺寸进来。）
...
## 十七、补齐与上游 framework 的差距 —— 已完成 ✅

原来还有一节"下一步",按价值排了七条待办;**七条都做完了**,所以那一节折进
这里。留下的是当时的排序和理由 —— 每一条为什么值得做,那部分没有过期:

1. **持久化 render object。** 不是性能理由(整条 UI 线程当时实测 0.5–0.6 ms,
   其中重建 0.15 ms,见第十三节),是架构理由:repaint boundary 要有东西可以
   挂,惰性列表要 render object 活过一帧。从 UI 到上屏这条路上当时已经没有
   已知的流水线差距了,剩下的都是框架架构工作。→ 第 15 条,以及长在它上面的
   第 18–22 条。
2. **InheritedWidget 的依赖追踪。** 让 `Provider` 只重建真正读了它的 widget。
   → 第 6 条。
3. **多平台。** Windows 与 Android 都通了(第十六节)。→ 见本节末尾"没做的"。
4. **把平台状态接进 widget 树。** 通道那一侧当时已经齐了(第十五节),差的是
   **框架里面**:文字缩放只能全局给,亮度要应用自己去挑 `Theme::dark()`。
   上游的答案是 `MediaQuery` 加依赖追踪,所以这条要等第 2 条。→ 第 1 条与
   第 13 条。
5. **焦点树。** 键盘当时只到应用级。上游那一摊在框架侧一万六千行,其中
   `focus_traversal.dart` 单文件 2575 行 —— 光是"Tab 该往哪儿走"。→ 第 7 条。
6. **键盘 redispatch。** 让框架能真正吃掉一个键:把 `on_key` 的答案接回窗口
   线程,再把没人要的消息 post 回队列并在回来时认出来。→ 第 14 条。
7. **文本编辑与输入法**(第十五节)、**无障碍**(第 16–17 条)、**rustc 用
   CIPD 固定**(还没做,见本节末尾)。

这一节记录每一块,以及做的过程中抓出来的几个真实缺陷。每一块都对着上游的对应
文件写,测试和真机验证在每块末尾。

### 1. 视图内边距、MediaQuery 与 SafeArea

插头一直在，只是没人插：`ViewportMetrics` 有字段、`RuntimeController` 会
转换、Rust 的 `ViewMetrics` 有对应的成员，而 Android host 一个都没填，于是
全是零，上面也就没有东西可用。

三半，都照上游：

- **Java** —— `onApplyWindowInsets`，照 `FlutterView` 移植，包括它在 API 30
  处的分叉和 30 以下那个"底部 inset 超过屏幕 18% 就当成键盘"的启发式；外加
  `FlutterActivity` 设的那几个窗口标志(`FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS`、
  状态栏染色、`SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN`)。没有这几个标志，视图根本
  伸不到系统栏底下，每个 inset 都还是零。
- **C++** —— 多一个 JNI 入口，`SendViewportMetrics` 填 padding 和 view inset
  两组。padding 槽里放的是 **view** padding，这正是 `FlutterRenderer` 放进去
  的东西；相减是框架的活。
- **Rust** —— `MediaQueryData`，发布在每个 widget 应用的根之上，位置与
  `WidgetsApp` 发布 `MediaQuery.fromView` 相同；`SafeArea` 消费它。

`ViewMetrics::padding()` 是逐边 `max(0, view_padding - view_insets)` ——
dart:ui 的推导，放在 dart:ui 做它的那一层。这就是为什么键盘弹起时屏幕底部的
`SafeArea` 会停止加内边距：键盘已经把内容顶离手势条了。

`AppBar` 在自己的表面**内部**加内边距而不是被 `SafeArea` 包住，所以栏的颜色
仍然铺到屏幕顶端；`Scaffold` 把顶部内边距从 body 的 `MediaQuery` 里去掉，
免得加两次；`BottomNavigation` 按手势条长高并把目的地顶上去。自绘标题的示例
自己写 `SafeArea`；两个纯渲染对象的示例读 `context.metrics.padding()` ——
那里没有元素树可以继承任何东西。

### 2. 阴影与 elevation

`BoxShadow` 是上游的，包括三个数字的分工:offset 移动阴影,`spread_radius`
在模糊**之前**放大形状(是更大的物体,不是更软的边),`blur_radius` 软化边缘
而不移动它。`kElevationToShadow` 整表逐条抄进来,每级三层阴影一个不落 ——
少掉本影或环境光那一层,出来的东西说不上哪里不对但就是不对。

半径换 sigma 用 `Shadow.convertRadiusToSigma`。半径是模糊能看见的范围,
sigma 是背后那个高斯的标准差,拿一个当另一个用,阴影会明显过紧或过散。

上游的表是稀疏的(只定义某个控件真正用到的级别),所以 `elevation_shadows`
向下取最近的一级而不是返回空。Card 1、Dialog 6、BottomSheet 1、SnackBar 6 ——
各自的 Material 3 默认值。

### 3. 长按、双击、双指

三个路由里没有的识别器,常量照上游:长按 500ms,两次点击之间最长 300ms、
最短 40ms,两点相距不超过 100 逻辑像素。

其中两个是**时间**而不是移动决定的 —— 长按在手指不动时触发,而一个同时想要
双击的区域上的单击,要等第二次点击的窗口关闭才能上报 —— 所以路由多了一个
`tick()`。上游这两个是 `Timer`;这里帧是按需的,于是带截止时间的按压会一直
要下一帧,没有待决事项的按压什么都不要。这也是为什么单击会被**押后**:上游
`DoubleTapGestureRecognizer` 是 hold 住竞技场而不是拒绝,所以 Flutter 里同时
注册两个回调的 `GestureDetector` 报单击本来就是迟的。同样的规则,同样的迟。

捏合是同一区域上的两根手指。scale 相对起始间距、rotation 相对起始角度并归一化
(手腕越过半圈线不该报出六弧度),焦点上报出去,好让缩放把手指下面的东西留在
手指下面。捏合会把它的手指从原本的拖动里带走:上游是 scale 识别器赢下竞技场
从而拒绝掉 drag,而被拒绝的 drag 是一次 cancel,所以结束时速度为零。

**目标的身份用区域 id,不用处理器的指针相等。** 双击的第二次点击要和第一次比,
第二根手指要和第一根比,这两个问题都跨帧,而处理器每帧重建 —— `Rc::ptr_eq`
每次都答"不是同一个区域"。上游从不问这个问题:那边 recognizer 是活过帧的对象。
这个是在真机上发现的,现在有一条"两次点击之间重建整棵树"的回归测试。

### 4. 曲线、物理与隐式动画

六条手工拟合的多项式换成上游的真表。那边几乎每条命名曲线都只是一条三次贝塞尔
(`Curves.easeIn` 就是 `Cubic(0.42, 0.0, 1.0, 1.0)`),所以 `Cubic` 是变体、
名字是常量,三十条,系数照抄不重新拟合 —— 它们是设计决定,一条"接近
easeInOut"的曲线摆在真的旁边就是错的。bounce 和 elastic 各是自己的形状。
两个端点精确返回,这是上游 `Curve.transform` 在问形状之前就做的事:elastic
在 t=0 处算出来是千分之一,而"开场先跳千分之一"就是开场先跳。翻转一条三次
曲线是精确的 —— 控制点 (a,b),(c,d) 的反向是 (1-c,1-d),(1-a,1-b) —— 不再是
拿 ease-out 近似 ease-in 的反面。

物理:弹簧(三种阻尼,三个闭式解)、摩擦、重力,加上 `Tolerance` —— 一个永远
逼近目标的东西什么时候算"到了",得有人决定。弹簧是"没人挑过时长的到达"
长什么样,也是被打断之后能接着走而不是重来的原因。

隐式动画:上游的 `TweenAnimationBuilder`,整个 `Animated*` 家族都建在它上面。
给它一个目标,目标一动它就走过去 —— 而目标在半路又动时,它从**当前位置**
继续,这条规则是整个想法的根基。

**写这块抓出一个真实缺陷。** `advance` 返回的"要不要下一帧"同时也是"要不要
重建",而这里每个动画在**到达终点那一次**都答否 —— 于是最后一帧、也就是拿着
终点值的那一帧,从来没有被 build 过。淡出停在 98% 会在屏幕上留下残影。
Controller、Navigator 和新的隐式动画现在都把"落地这一帧"算作有事做,下一帧
才安静。

### 5. Wrap、AspectRatio 与两个 intrinsic

真正缺的是 `Wrap`:一行数量未知的小东西(chip、标签、筛选器)在 `Row` 里会溢出。
一行装不下就换行,而一个宽到任何行都装不下的孩子仍然占一行 —— 否则它会永远
另起一行。

`AspectRatio` 照上游的顺序:先取宽度,由它算高度,再逐个约束把结果推回界内。
反过来做会得到一个满足比例、违反约束的盒子,而该让步的不是约束。

`IntrinsicWidth`/`IntrinsicHeight` 会问孩子想要多大,也就是要试排一遍。上游
就是用这个措辞警告它的代价的;它是"把这排按钮都做成最宽那个的宽度"的答案,
别的用处不多。

### 6. InheritedWidget 的依赖追踪(七条里的第 2 条)

读一个继承值现在会把读者登记到发布者名下 —— 上游
`dependOnInheritedWidgetOfExactType`,重点在那个 *depend* —— 而
`ElementTree::publish` 换掉发布的值,并且只标记读者。

因此 `provide` 要求 `PartialEq`。这是 `updateShouldNotify` 在这里的形态:
元素树拿到值的时候类型已经被抹掉了,而一个无法比较的值说不出"变没变",
于是每次重建都像变了,追踪也就白做。

路和上游不同,目的地相同。那边是上层 widget 带着新值重建,而它的孩子是**同一个
widget 对象**,协调到此为止,剩下的交给 `notifyClients` 标记依赖者。这里的
widget 是闭包、无法比较,所以 `publish` 是把值换在元素上,而不是顺着树送下去。

立刻兑现的地方是视图指标:resize 仍然整棵重建(应用自己的 build 拿着尺寸,
必须重新决定),但**键盘弹出、状态栏改高、读者调大字号**现在只重建问过它的
widget。键盘动画期间 insets 每帧都在变,答案不该是把整页重建三十次。

### 7. 焦点树(七条里的第 5 条)

指针自带答案 —— 它有位置,可以问树"这下面是什么";按键什么都不带,所以得有
人决定它去哪儿。

节点、其中一个持有焦点、按键先给它再给它的祖先,Tab 在它们之间走。形状照上游:
`KeyEventResult`、`FocusManager.handleKeyMessage` 的祖先遍历(快捷键属于屏幕的
某个区域,而那个区域正是被聚焦者的祖先),以及 Tab 是**默认**而不是规则 ——
应用级处理器先看到键,和 `WidgetsBinding` 的顺序一样,所以想把 Tab 挪作他用的
应用拿走它,焦点树根本看不到。遍历顺序是节点被构建的顺序,即上游的
`WidgetOrderTraversalPolicy`;那边默认按几何排序,对一个从上往下写的表单,
两者一致。

节点通过读取外层 `Focus` 发布的值知道自己的祖先,这正是上游 `Focus` 用它的
inherited widget 做的事 —— 也是第 6 条依赖追踪的第一个用户。构建栈答不了这个
问题:子 widget 的 build 发生在父 build 返回很久以后。

**节点在渲染对象组装时注册,而不是在 build 时。** 这个区别就是它的全部正确性:
只有**脏**元素会重建,所以在 build 里注册的节点会在第一个它没理由重建的帧上
消失,Tab 就此失灵直到别的东西碰巧改变。渲染对象是每帧为每个元素组装的。
有一条"什么都不重建,Tab 仍然能走"的测试。

`TextField` 现在是焦点节点,它的会话跟着焦点走而不是跟着点击走:得到焦点开
连接,失去焦点关连接,这是 `EditableText` 在它的焦点监听里做的事。Tab 在两个
字段之间移动并把键盘带过去。它自己那层指针区域去掉了 —— `Focus` 已经用同一个
id 包了一层,而一个什么都没挂的区域会截住点击并遮住真正想要它的那个。这件事
让 `platform_channels` 的打字那一半失败过,直到被找出来。

### 8. 富文本

一句话里有一个粗体词。引擎边界原本只能建单一样式的段落,所以补上了
`txt::ParagraphBuilder` 和 dart:ui 的 `ParagraphBuilder` 都有的那套:push 样式、
加文本、pop、重复、build。

必须是**一个**段落而不是三个并排的文本,这也是这次动 FFI 的全部理由:断行、
bidi 重排和基线对齐都要看见整个段落。三个并排的文本会按三段来折行,于是靠近
右边距的粗体词会像新起一句那样另起一行。

`TextSpan` 是上游的,拍平了。那边一个 span 可以有孩子、孩子继承父亲没设的部分;
这里由调用方解析,因为到了排版器那里答案本来就是一个具体样式。

### 9. 惰性列表

上游的 `SliverFixedExtentList`,也就是惰性滚动里**不需要测量**的那一半:每项
等高时,哪些在屏幕上是算术,不在的就不建。索引的取整也是上游的,
`precisionErrorTolerance` 在内 —— fling 会把偏移留在离项边界一个舍入误差的
地方,没有它某一行会在相邻两帧之间忽隐忽现。

没建的项仍然**占位**:两端各留一个和它们代表的行等高的间隔,所以 extent、
偏移和任何量内容的东西都和"每行都在"时一样。项按索引加 key,窗口移动时行保住
自己的状态而不是继承邻居的。

缓存范围是上游的 250 逻辑像素,理由也是上游的:只建可见部分意味着 fling 的
第一行是在它出现的那一帧建的,而那一帧最没有余裕。

变高度的那一半是刻意没有的。上游的 `SliverList` 不把前面九千九百九十九项排完
就不知道第一万项在哪,所以它维护一个估计值并在滚动中修正 —— 那是另一件事,
而且需要活过一帧的渲染对象。

### 10. 悬停

路由原来把每个 Hover 事件都丢掉 —— Windows host 里那句注释写得明白:"一次没有
按键的移动是 hover,还没有识别器要它" —— 于是桌面上没有东西知道指针在自己上面。

上游的 `MouseTracker` 和 `MouseRegion`:进入和离开各报一次而不是每像素一次,
并且报给指针下面的**每一个**区域而不只是最内层,因为卡片里的按钮同时在两者里、
两者都想知道。手指从不悬停:它要么在触碰,要么不在那儿。

Windows host 现在会发这些移动。那是每个鼠标像素一个包,和每个 Flutter embedder
发的一样,也是知道指针到达某处的唯一办法 —— 代价里还包括向 Windows 要
`WM_MOUSELEAVE`,否则鼠标去了别的窗口之后,最后那个区域会一直亮着。悬停只在
真的通知了什么的时候才要帧。

### 11. 水波纹

上游的 `InkWell` 加 `InkRipple`,时间照抄:按压未确认时半径在一秒内长到目标,
颜色 75ms 淡入,松手后 375ms 淡出。目标半径是触点到盒子最远角的距离 ——
所以贴边按下的水波仍然铺满整个按钮。它是 Material 里唯一不属于装饰的东西:
变色说的是"被按住了",水波说的是"你按在**哪儿**、而且收到了"。

要让它能装进按钮,路由得更像上游一点:**原始指针事件现在发给命中路径上的每一个
监听者**,而不只是那个最终拥有手势的区域 —— 上游的 `PointerDownEvent` 就是发给
路径上每个 recognizer 的,因为竞技场还没决定任何事。"被按住"仍然只属于一个区域,
因为那是一种可供性而不是事件。这条规则一并取代了之前为"落指接住 fling"加的特例:
那本来就是这条规则的一个特例。cancel 也走同一条路(新的 `on_pointer_cancel`),
因为被平台夺走的按压要撤销而不是完成。

这块牵出两件事。**一个 stateful 组件是用同一个 widget 实例重建的**,所以把子
widget 存成 widget 的组件,第一次 build 交出去、第二次就没有了 —— 按下的按钮
正是这样整个消失的,现在 `Ink` 收的是构建闭包,并有一条"按下后测尺寸"的测试。
另外两个图片测试在和真正的解码线程赛跑(它们断言 worker 还没做完),负载高的
机器会证伪;现在它们只断言不会赛跑的部分。

### 12. 滚动条与 IgnorePointer

滑块的算术是 `ScrollbarPainter` 的:长度是内容可见的比例,下限 48 逻辑像素 ——
一万行的列表也要留下一个能瞄准的东西;它走的是轨道**减去自身长度**的距离,
所以到底时滑块的远端正好压在轨道远端,"我到底了吗"才能一眼看出来。列表停下
600ms 后开始淡出,历时 300ms,都是上游的数。

它没有子节点:是放进同一个 `Stack` 的覆盖层,所以加一条不会挪动任何东西 ——
也因此需要 `IgnorePointer`,上游对同一个问题的答案:画在上面的东西不该抢走
下面那层要的点击。相册的三个测试立刻发现了这件事,因为滚动条把滚轮和"打开
...
### 15. 持久化 render object(七条里的第 1 条)

渲染树原本每帧整棵重建。元素复用已经省下了 build,但每一个 render object
都被扔掉重造,连同它量过的一切 —— 于是 repaint boundary 没有东西可以挂,
布局没有东西可以跳过,变高度的惰性列表也没有东西可以记住。

`BoxedRender` 现在是共享句柄而不是 Box。把它当子节点持有的父亲,和造出它的
那个元素,指向同一个对象。上游本来就是这样并且不必多说 —— 那边垃圾回收让
"两个引用指向一个 `RenderObject`"毫不特别;这里则要一个引用计数和一个
cell。cell 不是伪装的锁:一棵树到每个节点只有一条路径,而只有布局会改动
render object,所以这个借用在构造上就不会争抢。

每个元素留着自己造的那个,`build_render_tree` 直接把它交回去 —— 除非这个
元素拿到了新 widget,或者它某个后代的对象被重造了。后半句不是可选的:父亲
按句柄持有孩子,所以一个被重造的孩子必须把**从改动处到根的那条脊**一起
重造,而且只有那条脊。于是在"一个计数器在跳"的界面上,其他每一个 render
object 都活着,量过的东西也都还在。上游是从另一头到达同一个地方的:描述
变了就原地改 render object。真正要紧的那件事 —— 对象还是同一个对象 ——
两边一样。

三个组合子现在收任意 render object 并自己包起来,所以既有的调用点不用改
写法;直接交出一个渲染对象的 build 用 `boxed()` 代替 `Box::new`。

这块**正确地**弄坏了焦点注册表。它原来每帧从渲染遍历里重建 —— 那是"遍历
是唯一会经过每个元素的东西"那会儿的修法 —— 而一个会复用 render object 的
遍历不再跑那些闭包。所以注册表改成持久的,这本来也就是上游的样子:
`FocusNode` 在 `initState` 里造、在 `dispose` 里销毁,跟帧没有关系。节点
现在在 build 时注册,在元素消失时被清掉,认的是 `ElementRef` 而不是 id ——
因为 id 是竞技场的槽位,会被下一个占用者拿走。

持久化只能以**同一性**被观察,所以验证也只能是同一性:没被碰过的元素交回
...
`SliverChildBuilderDelegate` 的默认(`addRepaintBoundaries`)。写下这一条的时候
它只在**列表之外**的变化上兑现 —— 一个跳动的表头不会碰到任何一行的层 —— 而
列表自己一滚,每一行都重做。第二十一条把那半边补上了。

### 21. 被改的渲染对象,不是被换掉的

前三条省下的都是**没被碰到的子树**。这一条是被碰到的那棵:一次 `set_state`
会把它下面每一个 widget 都重新造一遍,而在此之前,每一个渲染对象也跟着重新
造了一遍 —— 于是列表滚一行,一百行全部重新测量、重新排版、重新绘制。上游不是
这样:`RenderObjectElement.update` 把新的 widget 交给**已经在那儿的**渲染对象
(`updateRenderObject`),而每个 setter 又先比一比 —— `set padding` 在值没变
的时候什么也不做。前三条依据的都是同一性,而这一条正是让同一性活下来的东西。

**为什么两半必须合成一个方法。** 上游的 `Padding.updateRenderObject` 写
`renderObject.padding = padding`,`RenderPadding.set padding` 再决定要不要
`markNeedsLayout`。这里没有一个装着这些字段的 widget 类可以逐个赋值:配置藏在
组合子捕获的那个闭包里,拿不出来。所以新配置只能整个到达 —— 让闭包造出它本来
就要造的那个对象,再交给老的去取。比较因此写在渲染对象自己身上,这也正是上游
放它的地方。

**答案有三种,不是两种。** `Relayout` 是 `markNeedsLayout`,`Repaint` 是
`markNeedsPaint`,`Nothing` 是上游那些什么也不标的 setter —— 换掉一个
`onTap` 既不改变尺寸也不改变画面。为此加了 `mark_needs_paint`,和
`mark_needs_layout` 走同一条父链(同样一路走到根,同样因为没有 pipeline
owner),尽头是把留下的那层扔掉。没有它,只改了颜色的一格会停在边界留下的
旧画面里。

**不肯回答也是回答。** 默认返回 `None`,意思是"换一个新的",也就是这一条
之前所有类型的做法 —— 永远正确,只是不省。`RenderClipPath` 就这么答:路径
本身就是配置,而比较两条路径要把两条都走一遍,那正是比较想省下的事。
`Container` 和 `ListView` 写这一条时也这么答,第二十二条把它们补上了。

顺手修掉两件在路上撞见的:

**句柄套句柄。** 组合子把孩子的句柄交给一个"包住你给我的东西"的构造函数,于是
每个父亲每帧都拿到一个**新的外层句柄** —— 里面那个真正活着的对象,它的同一性
在上一层就看不见了。上游根本到不了这个形状:一个 `RenderObject` 不会是另一个
`RenderObject` 的全部内容。现在 `RenderRef::new` 认出这种情况并把原来那个还
回去。

**读屏软件会丢掉边界下面的一切。** 这个框架是在**绘制的那趟**里收集语义的,而
一个交回旧层的边界不往下走 —— 于是它下面的节点这一帧什么也没说。上游遇不到:
那边语义树是自己走一趟渲染树建出来的,留存的层对它不可见。这里的答案是:**有
读者在听的时候,边界不交旧层**。层是优化,阅读顺序不是。两个惰性列表刚给每行
套上边界,所以这件事从潜在变成了必然 —— TalkBack 会在第一帧之后丢掉每一行。

**这个答案后来被换掉了。** 它治的是症状:代价是读屏软件一开,整屏留存层全废。
第 23 条把语义收集从绘制里搬了出去,病根没了,边界现在无论有没有读者都交层。

### 22. 自己组装子树的那两个

第二十一条把新配置交给已经在那儿的渲染对象,但有两个类型当时答的是 `None`:
`Container` 和 `ListView`。它们不是漏掉的 —— 它们是这里唯二**自己组装一棵
子树**的渲染对象,而"把新配置塞进一棵已经搭好的子树"正是元素树的工作,
它们身边偏偏没有元素树。

**上游遇不到,因为上游的 `Container` 不是渲染对象。** 它是 `StatelessWidget`,
`build` 里嵌几个别的 widget,元素树一个槽一个槽地更新它们,从不问 `Container`
一句话。这里嵌的是渲染对象,所以那份对账只能由它自己做。规则照抄元素树的
两条:**槽里的 widget 类型没变就留着元素** —— 又要了同一个包装,就交给它而
不是换掉它;**类型变了就换元素** —— 要的包装换了一套,整个容器答 `None`,
由调用方造一个新的。

为此得记住上一次搭了哪几层。`shape()` 把配置读成一串包装(对齐、内边距、
装饰、定尺寸、外边距),搭的时候把每一层的句柄记下来;下一次先比这一串,
一样才逐层往里交 —— 而且是**从里往外**。每一层新造出来的包装,它的孩子是
里面那层**交完之后活下来的那个句柄**,于是 `RenderPadding::update_from` 里
那句 `self.child.is(&fresh.child)` 看到的是同一个对象,不会白要一次布局。
顺序反过来就什么也省不到:先交外层,外层会把里面那层换成新造的。

**孩子是唯一会拒绝的那一层**,因为它是从外面来的,可以是任何类型。所以先问
孩子,再动自己的字段 —— 被拒绝了就原样退回,不留下一个改了一半的容器。

`ListView` 是同一件事的小一号。视口和它滚的那一列都是它自己的:新的一列交给
旧的一列(`RenderFlex::update_from` 逐个比孩子),轴和滚动位置交给旧的视口。
视口没有自己的句柄 —— 这个列表就是它的句柄 —— 所以视口的回答就是列表的回答。
顺带修掉一件:两端的留白(让一张卡片能坐在正中间的那个)原来只在第一次布局
时算一次,而它是从约束里来的;现在窗口一变,那一列会在保住同一性的前提下重搭。

**两个都回答 `Nothing`。** 不是"什么都没变",而是"该说的话已经有人替它说了":
里面哪一层变了,哪一层就已经标了自己,而标记一路走到根(第十八条),路上正好
经过这个容器。再标一次不会更准。

### 23. 语义走自己的一趟

语义收集一直长在绘制那一趟上,理由当时是充分的:节点要一个根坐标系里的矩形,
而**渲染对象只有被画的时候才知道自己在哪儿** —— 绘制是唯一带着偏移往下走的
那趟。上游不这样做,它让语义树自己走一趟渲染树。这一条把那趟走出来。

**两处真实的坏,都不是省一趟能换来的。**

第一处是**边界**。交回留存层就是不走子树,而子树正是语义的来源,所以有读者
在听的时候重绘边界只能不交层 —— 读屏软件一打开,整块屏幕的留存层全部作废。
第二处更糟,而且此前没人发现:**`record_retained` 把偏移放进图层,把孩子画在
原点**。于是边界里的每个节点报的是"在这个边界里的位置",却被当成"在玻璃上的
位置"交给平台。推图层的不止边界 —— `push_opacity` 和 `push_transform` 也一样。
一个淡入到一半的子树,它的每个节点都报在错的地方。

写下这一条时先写了一个测试:一个 10 像素内边距里、重绘边界下的按钮,应该报
`(10, 10)`。它报的是 `(0, 0)`。

**上游的两个方法,这里合成一个。** `visitChildren` 说有哪些孩子,
`applyPaintTransform` 说每个孩子在哪儿 —— 上游能分开,是因为孩子身上挂着
`BoxParentData.offset`,是父亲在布局时写上去的,所以从孩子身上读得回来。这里
没有 `parentData`:偏移在父亲自己的字段里(`RenderFlex` 是一串,`RenderPadding`
是从内边距算出来的),父亲是唯一答得上来的人,所以它一次答两件:

```rust
fn visit_children(&self, visit: &mut dyn FnMut(&dyn RenderBox, Offset))
```

**只有平移,没有矩阵。** 上游那个是 `Matrix4`,`RenderTransform` 能把孩子转过
去。这里是一个偏移,旋转无从表达 —— 而报**没转过去的**那个矩形不是更差的猜测,
是**对得上的**那一个:矩形存在的意义是让手指在玻璃上摸得到这个节点,而手指摸
到什么由 `hit_test` 回答,`hit_test` 恰恰也是拿没转过去的几何去比的。两个"这
东西在哪儿"的答案不一致,比一个近似的答案更坏。

`visit_children_for_semantics` 是上游的 `visitChildrenForSemantics`,默认就是
上面那个,理由也照抄:**不会画的孩子也不该被描述** —— 屏幕上没有的东西,对着
玻璃摸索的读者来说也没有。目前只有 `RenderOpacity` 在透明度为零时覆写它,和
上游一样,也和这里 `hit_test` 早就拒绝的一样。`describe_semantics` 是上游的
`describeSemanticsConfiguration`:全框架只有两个类型答它 —— 特意放上去的那个
标注,和段落。段落自己说话,因为屏幕上的字就是读者来读的东西(上游的 `Text`
在自己的 `build` 里裹一层 `Semantics`,同一件事)。

**代价:每一帧重建整棵语义树。** 上游留着一棵持久的语义树,只重访标脏的渲染
对象。这里没有 pipeline owner 存那张脏表,所以和整棵树的布局下降是同一个理由
(第十八条)—— 而且这仍然比它替换掉的便宜,那个的代价是整屏重绘。何况一帧
也不走,除非真有读者在听。

> **这个代价后来被去掉了。** 第 24 条按上游的三个地方装了三道闸门:没人在听、
> 没人标脏、走完发现没变,任意一道都让这一帧一个字都不发。"每一帧"这三个字从
> 这里起不再成立;从根下降那一半仍然成立,理由也仍然是这一段写的那个。

顺带把一句话变成真的:`RenderSemantics::update_from` 什么也不标,因为改了的
标签下一帧会从活着的对象上读出来。以前这句成立靠的是"边界被迫不交层所以绘制
一定走到这里";现在它成立是因为**那趟走是真的一趟走**。

> **这一句也被换掉了。** 一趟走既然可以被跳过,那它就不能再是标签更新的唯一
> 依靠。第 24 条让 `RenderSemantics::update_from` 在标注真的变了时标脏 ——
> 这正是上游 `set properties` 做的事,而且是这个框架里唯一一种布局和绘制都
> 不会替它注意到的变化。

两个 bug 都是潜伏的:全工程只有 `LazyList` 会造重绘边界,而示例和相册都还没
用上它。但 `LazyList` 正是真应用滚动时要用的东西,它们只是在等着。

### 验证

**Windows**:框架 410 个测试、FFI 17 个、flutter_gallery 21 个、相册 60 个
(cargo 独立构建)全过;`platform_channels --probe` **PASS**(两处既有 SKIP);
counter / flutter_gallery / gallery / hello_world / showcase 五个示例的无头
`--png` 在这一条前后**逐字节相同**;用真 UI Automation 客户端读真窗口重跑:
树、`Invoke`(0 → 1 taps)、`Toggle`(Off → On)、`FromPoint` 都照旧,text_demo
的 `SetFocus` 之后用真键盘打进 `rustflutter` 并从 Value 模式读回,退格三次
读回 `rustflut`。第二十三条另外把改动前的 flutter_gallery 重新编出来,用同一个
UIA 客户端读了一遍两棵树:**每个节点相对窗口的矩形一字不差**,即没有回归 ——
坏掉的那两处此前没有示例走到。clippy 警告数与改动前持平(53)。

**Android**(模拟器,Android 14,x86_64):九个示例的截图与第二十二条之后
**逐像素相同**(状态栏以下);`platform_channels` **PASS**;flutter_gallery
滑动 → 打开 demo → 返回,返回后与滑动后**逐字节相同**;用一个单独构建的无障碍
服务读 showcase,树照读、`CLICKED|Primary|true`、点击数真的往前走;相册 150 张
图的九宫格与看图页**逐像素相同**,滚动后那张不同 —— 同一个构建连跑三次,三次
互相都不同(17 万、23 万字节),甩动的落点在这台模拟器上不可重现,所以这一步
看的是内容而不是字节。

**更早各节验过的**(留档,不再重复):引擎侧逐字节相同、Impeller、帧时间实测、
真键盘与焦点、文字缩放、水波、留存层的 FFI 双帧验证、Windows 与 Android 两边
的无障碍桥、相册在两个平台上的构建与手势。
...
第 23 条把语义收集搬到自己那一趟上,然后每一帧都走那一趟 —— 只要有读者在听。
上游不这样。上游在**三个不同的地方**拒绝做这件事,而这一条把三道闸门都装上。

**第一道:没人在听就什么都不标。** 上游的 `markNeedsSemanticsUpdate` 开头是
`if (!attached || owner!._semanticsOwner == null) return;`,`flushSemantics`
开头是 `if (_semanticsOwner == null) return;`。这道原本就有。

**第二道:这一帧什么都没变就不走。** 上游在 `PipelineOwner` 上存着
`_nodesNeedingSemanticsUpdate`,`flushSemantics` 只访问里面的对象,按 `depth`
从浅到深排序(它的注释写了理由:自顶向下更快,否则一个父亲变了会作废整棵子
树),访问完清空。这里存的不是一张表而是**一个布尔**,理由和
`mark_needs_layout` 一路标到根是同一个:一张脏表只有在能**从表里的东西开始
下降**时才值得存,而这里没有 pipeline owner 来开始那次下降。所以这个布尔答
的是"走"还是"不走",省下来的是它说不走的那些帧 —— 对一块正在被读、而不是正
在动的屏幕来说,那是几乎所有的帧。

**谁来标,是照着上游一条条抄的**,而且这份清单是**闭合的**,不是但愿:

| 这里 | 上游 |
| --- | --- |
| `RenderRef::layout`,真的走了布局那条路 | `RenderObject.layout` 里 `performLayout()` 的**下一行**就是 `markNeedsSemanticsUpdate()` |
| `RenderSemantics::update_from`,标注本身变了 | `RenderSemanticsAnnotations.set properties` |
| `RenderOpacity::update_from`,透明度跨过零 | `set opacity` 里的 `if (wasVisible != (_alpha != 0))`,一模一样的条件 |
| `set_enabled`,读者刚到 | `scheduleInitialSemantics` |

第一行覆盖了几乎所有事:滚动、改了尺寸的重建、多出来的一行 —— 一个刚被量过
的盒子可能挪了、可能变大了、也可能不在了,而矩形是这三件事做的。第三行是这个
框架里**唯一**一处"只重绘却改变了读者会听到什么":全透明的子树是
`visit_children_for_semantics` 不肯进去的那一棵。段落改字不用单列,因为改字要
重新排版,重新排版就走第一行。

**第三道:走完发现没变就不发。** 上游的 `sendSemanticsUpdate` 开头是
`if (_dirtyNodes.isEmpty) return;`,而且只把脏节点放上线。这里留着**平台手上
那棵树**(`Collector::sent`),走完比一遍;树小到比较比记变更日志便宜,而这正
是 Windows 那边的桥**已经在做的同一笔交易**(`FlutterWindowsView::UpdateSemantics`
上游交给 `AXTree` 让事件生成器算差量,这里的桥自己比两张快照)。会走到这道闸
门的是:一圈长大的水波、一个补间的颜色、一次已经停下来的滚动 —— 布局动了,而
读者要听的没动。

**没装上的是上游的第四道**,得说清楚:上游只重走**脏的那个语义边界底下的子
树**,因为它的脏表里存的是渲染对象,而且它的节点矩形是**相对父节点**的 ——
一个滚动的视口只挪一个 transform,不是挪底下每一个矩形。这里的矩形是绝对的
(两边的桥要的都是"在玻璃上的哪儿"),所以挪过的子树没法复用,那一趟只能从根
下降。这和布局是同一笔交易、同一个缺口。

**一处必须跟着改的地方。** 跳过一趟走,就等于跳过一次"把动作处理器记下来"。
一个只换了闭包的重建,什么都没量、什么都没画,所以什么都不会标 —— 那正是这
一条要跳过的帧 —— 而上一趟走记下的处理器会**正好差一个 build**。所以
`perform_action` 不再查记下来的表,而是**当场去活着的渲染树上问**:
`RenderSemantics::update_from` 早把这一次 build 的闭包接了过去。上游从来不用
选:它的 `SemanticsNode` 拿着渲染对象,摸到一个就摸到另一个。顺带这也让处理器
不必参与"标注变了没有"的比较 —— 两个闭包本来就分不出来(每次 build 都是新的
`Rc`),比就帧帧标脏,不比就得有人拿着旧的,而现在没人拿。

**顺手抓到一个真的坏。** `SemanticsProperties` 的三个滚动字段用 `NaN` 表示
"这东西不滚动"(上游的 `double? scrollPosition` 一过嵌入层就是这个),而
`NaN != NaN` —— 派生出来的相等**永远为假**。第二道和第三道闸门都要问这个问
题,一个永远答"不一样"的比较会让两道闸门同时失效,却看着像在工作。所以
`PartialEq` 是手写的:两个都说"不滚动"的盒子说的是同一句话。这也让
`RenderSemantics::update_from` 不再帧帧标脏。

### 验证

**Windows**:框架 **416** 个测试(第 24 条新增 6 个)、FFI 17 个、
flutter_gallery 21 个、相册 60 个(cargo 独立构建)全过;
`platform_channels --probe` **PASS**;counter / flutter_gallery / gallery /
hello_world / showcase 五个示例的无头 `--png` 与第 23 条之后**逐字节相同**;
用真 UI Automation 客户端读真窗口:树、`Invoke`(0 → 1 taps)、`Toggle`
(Off → On)、`FromPoint` 都照旧,text_demo 打进 `rustflutter`、退格三次读回
`rustflut`。**又把改动前的 showcase 与 flutter_gallery 重新编出来读了两遍
树**,用同一个客户端:**每个节点相对窗口的矩形一字不差**(40 行与 99 行,
含 INVOKE / TOGGLE / FROM POINT 三段)。相册在 Windows 上跑起来,450 张图的
网格照画。clippy 警告数与改动前持平(53)。

**Android**(模拟器,Android 14,x86_64):九个示例的截图与第 23 条之后
**逐像素相同**(状态栏以下);flutter_gallery 滑动 → 打开 demo → 返回,返回
后与滑动后**逐字节相同**;相册九宫格与看图页(同一张 43/150)**逐像素相同**。
无障碍这一侧专门验了两件事,因为这一条动的就是它:一是用无障碍服务读
showcase,树照读、`CLICKED|Primary|true`、点击数真的往前走 —— 走的是新的
"当场去活着的树上问处理器"那条路;二是**读者在听的时候滑一下 flutter_gallery,
每一个矩形都整整齐齐挪了 414 像素**,这就是"`RenderRef::layout` 标脏"那一行
在真机上的样子。
...
- **还没回答 `update_from` 的只剩 `RenderClipPath`**,而那是故意的(见第二十
  一条):路径本身就是配置,比较两条路径要把两条都走一遍,正是比较想省下的事。
- ~~**语义收集仍然长在绘制上**~~ —— 写这一条时还没走,**第 23 条走了**,
  连带修掉两个此前没人发现的坐标 bug。
- ~~**语义树每帧重建**~~ —— **第 24 条不再每帧重建了**:没人听、没人标、
  或者走完发现没变,三道闸门任意一道都会让这一帧一个字都不发。
- **语义还是从根走,不是从脏边界走**。上游只重走脏边界底下的子树,因为它的
  节点矩形相对父节点 —— 滚动一个视口只挪一个 transform。这里的矩形是绝对的
  (两边的桥要的都是"在玻璃上的哪儿"),所以挪过的子树没法复用。同第十八条的
  缺口:没有 pipeline owner,一趟下降只能从根开始。
- **裁剪不裁语义矩形**。上游在 `_SemanticsGeometry` 里把节点的矩形和祖先的
  裁剪求交,剩不下的标成隐藏;这里视口外面的行照报不误。绘制那一趟也是这样,
  所以这一条没变坏,只是没变好。
...
- **rustc 还没有用 CIPD 固定**。`rustc_path` 至今默认走 PATH(第三节末尾那条
  注记),所以一次构建用的是本机装的哪一版 rustc。进 CI 之前要像 clang 一样固定
  到 `//flutter/buildtools/$host_os-$host_cpu/rust`;而且现在只写了 Windows 的
  rust toolchain,mac / linux / android 要照做。
- **Windows 与 Android 之外的平台**。每多一个缺的仍然只是一个窗口和一个消息
  循环 —— `rf_host_run` 之上的一切都是可移植的,Android 那次移植没有改动
  `flutter/rust/rustflutter` 里的任何一行。
- **不会有的东西:hot reload**。它是 Dart VM 的能力,Rust 没有对等物。桌面上
  或许能做 dylib 热加载,但状态保持做不到,iOS 上禁止动态代码。这是这条路线的
  固定成本,不是待办事项。
