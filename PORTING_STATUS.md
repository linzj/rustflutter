...
上一轮 `ffi_tables.py` 报出的那十三个语义动作，补齐了。它们不是十三行常量。

**九个是关于文本的**：按字符前后移动光标、按词前后移动、设置选区、复制、剪切、
粘贴、设置文本。而这个端口早就有了**字段**——`SemanticsFlags` 上的
`is_text_field`、`is_obscured`、`is_read_only` 都在——**却一个动词都没有**。读屏器
可以被告知它找到了一个文本框，然后没有任何办法去操作它。

另外四个各有各的说法：

**`customAction` 是唯一一个不说明自己做什么的位。** 它说的是"应用自己的某个动作"，
而**是哪一个由另一个整数带过来**——所以一个把它当成别的位来处理的桥接，扔掉的正是
唯一携带意义的那部分。

**`scrollToOffset` 不是第五个滚动方向。** 那四个方向是**推一下**（大约挪一屏），
这一个带着**目的地**。读屏器拖动滚动条时发这个，按翻页键时发那四个之一。它也不
在引擎唯一把这些位打包起来的那个名字 `kVerticalScrollSemanticsActions` 里——那个
是上下两个方向的或，因为只提供一个方向的列表是一个能往下走、回不来的列表。

**按词移动的那一对和按字符移动的那一对不相邻**，隔着十个位（9/10 对 19/20）——
按词那一对是后来加的，上游没法把它们放到搭档旁边。一个凭猜测编号的端口会把它们
配成一对。这条写成了断言。

七条承重规则逐条强制改错。第一条有两种版本，都有意思：把按词那一对搬到 11/12 位
**编译器直接拒绝了**（`discriminant value 2048 assigned more than once`）——这是
比测试更强的一道防线，而它之所以存在，只是因为这些位现在是稠密的；搬到没人用的
26 位则由测试抓住。

尺子：coverage 2102/0，constants 158/0/0，wire_strings 122/0，wire_enums 4/0,
**ffi_tables 5 张表 0 问题（上一轮开的那个问题关掉了）**，unwired 48/0，unvaried 0,
unread_strings 44+16+7/0，unpainted 0，hollow 67/0，vacuous 8，stale_engines
全部不落后，unread_theme_fields 2。
门：5757 + 333 通过；五个输出目录的 rustflutter_engine 与 rust_lib 全部重建。
...
## 第 328 轮：那三条“文字”里最后一条是短的，而四个角只画了一次

接上一轮的“下一步”。`liveTextInput` 是上游 `_getContentWidget` 里**唯一单独一支**的按钮：
别的都落到同一个 `Text`，它拿到的是一个 13×13 的 `CustomPaint`。这跟第 316 轮查到的
“它在 `getButtonLabel` 里答空串”正好接上——**它没有标签，是因为它根本不显示标签**，
不是漏掉了一个。

`_LiveTextIconPainter` 里两件按名字猜不到的事：

1. **一个角只写了一遍，画四次，中间转画布。** 上游先把画布平移到中心，构一条
   “一条臂 → 一个圆角直角 → 另一条臂”的路径，然后 `drawPath` 四次，每次把**画布**
   转四分之一圈，而不是重新算路径。反过来做——把四个角都写出来——正是错误藏身的地方：
   四个角不仅位置要对，**圆弧的方向也要对**，而其中三个是第一个在某个轴上读反的样子。
   （变异扫描里“两条臂对调”“圆弧反向”都能红，就是在钉这一点。）
2. **里面那三条“文字”，最后一条是短的。** 前两条走满 `-3..3`，第三条停在 1。
   这是**一段文字参差不齐的末行**，也正是这个图标读起来像“文字”而不是像三道横线或一个等号的
   原因。它还是凭记忆重画时最先丢掉的那个细节。而且三条的**左端都在 -3**——短的是右端收进来，
   不是整条居中缩短。

几何按本 crate 既有的做法端成**纯点**（`corner()` 答四个 `Offset`、`lines()` 答三对），
理由跟 `animated_icons` 那边写的一样：`RenderPath` 是一次引擎分配，会牵着测试要一个引擎。

**变异扫描 11 个，第一遍九个红，两个漏是同一个窟窿，而且是个典型的：**
“臂长和圆角半径一样长”和“圆角退化成斜切”都活了下来，因为我那条角的测试**两边都是用常量写的**
（`arm_end.dy - corner.dy == ARM`），改了常量，等式两边一起变。补了两条绝对断言
（3.5 和 1.0 各自钉死）加一条关系断言（半径必须小于臂长，否则那个转弯放不进它要转的那条臂），
两个变异随即转红。

尺子：十六把全部 exit 0。门：6232 通过；`cargo fmt --check` 干净；三个输出目录的
rustflutter_engine 与 rust_lib、以及 rustflutter_unittests、flutter_gallery_unittests 全部重建。

至此 `CupertinoTextSelectionToolbarButton` 走完了。

**下一步**：回队头。`depth.py` 上 `CupertinoSearchTextField`（0.29，10/35）是下一个，
而它跟这两轮的东西不在一条线上，值得先按行为扫一眼再决定。上游
`cupertino/search_field.dart` 里最可能是真缺口的一处：**前缀放大镜与后缀清除按钮的显隐规则**
...
## 第 335 轮：想先写一条红测试，结果它**绿**了——而且它自己也有盲区

按上一轮的“下一步”，先写红测试再修。写出来的却是绿的。

逐层往上加，每一层都**没有**复现：

| 组合 | 结果 |
|---|---|
| `Row[Padding(Measured), Padding(Measured)]`，`rebuild` 两次 | 绿（16×16、24×24） |
| 加 `rebuild_dirty()` | 绿 |
| 改成经由 `many()` 组合 | 绿 |
| 外面套 `provide(..)` | 绿 |

所以第 334 轮那个 0×0 **不是**“任何一次 rebuild”都会有——它需要搜索框还在做、而这里没做的事，
最明显的候选是 **`stateful`**。

**然后对这条新测试做变异，发现它自己也是瞎的一块。** 把
`RenderPadding::update_from` 改成**直接返回 `None`**（等于“这次重建完全没走到 padding 的更新
路径”），测试**照绿**。也就是说：

> 这条测试里的 rebuild **根本没有走 `update_from`**——它是重新造了一批渲染对象，
> 而不是去更新那批被布局过的。

这多半正是它复现不出来的原因：**真 bug 在更新路径上，而这条测试压根不走那条路。**
这是三轮之内**第二次**发现“为抓这个 bug 写的测试抓不到它”（上一次是第 333 轮的 `Sized`）。
两次都写进了测试自己的文档里，而不是留给下一个人重新踩。

测试留下了（它确实钉住了“行 + padding + many + provide 这一整套重建之后尺寸要对”），
但文档里明写了它的盲区。

尺子：十六把全部 exit 0。门：6241 通过；`cargo fmt --check` 干净；三个输出目录的
rustflutter_engine 与 rust_lib、以及 rustflutter_unittests、flutter_gallery_unittests 全部重建。

**下一步**：**改方向——从已知会坏的那一头往下砍，别再从下往上搭。**
从上往下搭已经连着两轮搭出了抓不到 bug 的东西。第 334 轮那个失败用例是完整的搜索框，
一步步把它简化：先去掉 IME 打字（直接给 `CupertinoSearchTextFieldState.text` 赋初值行不行）、
再去掉 `provide`、再把自绘 mark 换成 `Measured`……**每砍一刀就跑一次，直到它由红转绿的那一刀，
就是触发条件。** 这条路一定收敛，从下往上搭不一定。
...
## 第 338 轮：21 处测试跨着重建用下降法布局——但“它们错了”这句我没有证据

按上一轮的“下一步”把教训兑现。先数。

写了 `tools/descent.py`：按大括号切出每个 `fn`，找**同时**满足
“用了 `build_render_tree` + `.layout(`”、“跨过重建”（`rebuild` ≥2 次、或有
`rebuild_dirty`、或有 `set_state`）、“没有 `flush_layout`”的函数。

**21 个**（改掉一个之后 20 个），分布在 11 个文件里：`framework.rs` 8 个、
`semantics.rs` 3 个、`components.rs`／`theatre.rs` 各 2 个，其余各 1 个。

**但“数出来”不等于“错了”。** 下降法只在“被标记的节点藏在一个干净祖先底下”时才骗人；
一个测试如果断言的东西不依赖布局走到深处，下降法完全无害。挑了最可疑的一个手工验：
`a_subtree_that_did_change_is_laid_out_again`——它最后一句是
`assert_eq!(layouts(), before)`，如果下降在顶上就早退、什么都没布局，这句会**空洞地成立**。
改成 `schedule_root_layout` + `flush_layout` 之后**照样通过**，所以它没有说谎。改动留下了。

**还试了一次自动判别，结果没敢要。** 想法是：在 `layout_child` 里，
只要一次下降开始时 `DIRTY_BOUNDARIES` 里还排着队，就打印一条。跑全量——**零**。
但**这个零不算证据**：我从来没有让它在第 337 轮那个已知会坏的用例上响过一次，
所以它可能只是什么都没在量。（第一次跑出的零更假，是 `eprintln` 被 cargo 捕获了；
加 `--nocapture` 之后仍然是零。）探针没有留下。

这个项目在这件事上已经栽过三次（第 288、291、301 轮），当时挣来的规矩是：
**一把没人见过它抓到真错的尺子，报出来的是一个数字，不是一个事实。** 这一轮照这条规矩办。

所以 `tools/descent.py` **不接进那十六把尺子、不参与门禁**，永远 exit 0，
文件头写明它为什么只是一张问题清单：一行是一个待查的问题，不是一条判决；
要把它接上 exit code，就等于替二十个还没人核过的结论签字。

尺子：十六把全部 exit 0（descent 是第十七个脚本，不计入）。门：6242 通过；
`cargo fmt --check` 干净；三个输出目录的 rustflutter_engine 与 rust_lib、以及
rustflutter_unittests、flutter_gallery_unittests 全部重建。

**下一步**：把那把判别尺子**先标定再用**——这才是上面那个零欠的东西。
做法很直接：把第 337 轮那个已知会坏的用例（`Container` 包住 `Row`，重建后用 `root.layout(..)`）
临时放回去当**标定样本**，让探针在它上面响；响了，再拿去跑全量，那时的零才有意义。
...
## 第 348 轮：对话框底下的页面不再被念出来——以及 `Box<R>` 会把新钩子悄悄吃掉

按上一轮的“下一步”接第二个：`BlockSemantics`。先查活路径有没有“先画过的兄弟”这个概念——**有**：
`flush` 的收集器把节点**按绘制顺序**堆进 `nodes`（它自己的注释就是这么写的），
`open` 是一摞已打开节点的下标。所以“先画过的”就是“已经在 `nodes` 里的那些”。

接法比预想的干净：走查是深度优先、节点按绘制顺序连续，所以一个父节点在下标 `p` 打开之后，
**它的子孙正好占 `p+1..`**。于是“挡掉在它之前画的东西”就是一次
`truncate(p + 1)` 加一次 `children.clear()`——**是截断，不是搜索**。

`RenderBlockSemanticsBox` 的性质写进了文档：它和上一轮的排除是上游说的“有意思的一对”，
**方向相反**——排除丢掉**自己和自己的后代**、别人不动；阻断对自己的子树**只字不提**，
拿掉的是**在它之前画的兄弟**。这正是模态要的：对话框画在页面之后，
而对话框**不是**页面的祖先，所以树序表达不了这件事。

**中途撞上一个更值得记的东西。** 钩子加好了、`RenderRef` 也转发了，测试却纹丝不动。
打点查到 `wrapped=false`：`RenderRef` 里装的是 `Box<dyn RenderBox>`，而
`impl<R: RenderBox + ?Sized> RenderBox for Box<R>` 是**逐个方法手写转发**的——
**漏掉的方法会掉进 trait 的默认实现**，于是盒子里的对象**根本没被问到**，
而默认答案 `false` 看起来跟一个真答案一模一样。
在那个 impl 顶上写了一句话记下来：**每一个带默认实现的 trait 方法，这里都得有一行。**

**变异扫描 9 个，第一遍 6 个红，2 个是真窟窿、1 个不可达：**

* **“阻断把父节点也带走”**（`truncate(parent)`）和**“阻断不清子节点列表”**都没红——
  我的断言只看**标签**，而这两个变异一个让幸存者悬空、一个留下指向已删节点的 id，
  **在标签列表里都看不出来**。补了一条：根节点还在，且它的 `children` 恰好等于幸存节点的 id 列表。
* **“根上的阻断改成 `unwrap_or(0)`”**没红，而且**是不可达的**：`flush` 在走查之前就打开了根，
  所以阻断永远有东西压在上面。按老规矩写进文档而不是硬凑一个测试。

尺子：十六把全部 exit 0。门：6264 通过；`cargo fmt --check` 干净；三个输出目录的
rustflutter_engine 与 rust_lib、以及 rustflutter_unittests、flutter_gallery_unittests 全部重建。

第 346 轮那八个模型，现在接活了两个。

**下一步**：接第三个——`RenderMergeSemantics`。它跟这两个又不一样：
排除是**不问**，阻断是**拿走已问到的**，而合并是**把后代说的话折进自己这一个节点**。
上游它把 `isSemanticBoundary` 和 `isMergingSemanticsOfDescendants` **同时**设为 true，
理由写在 `render_semantics.rs` 里那段文档：能被折进去的东西**正是**一个边界，
不当边界就等于让后代折进虚空。而第 345 轮已经查明**这两个标志在本项目里都是死的**——
所以这一轮大概率会撞上“活路径缺的是节点合并这一层”。
先按行为查 `flush`／`open` 有没有“把子节点的标签并进父节点”的地方，
**有就接，没有就把缺口说清楚**，跟第 347 轮一样，接不上也是一个明确的答案。

---

## 第 349 轮：图标和标签合成一站——上游那一对标志，在这里是一件事

按上一轮的“下一步”接第三个：`RenderMergeSemantics`。这一家三个的性质现在凑齐了：
**排除是不问，阻断是把已经问到的拿回来，合并是把后代说的话折进自己这一个节点。**

先按行为查“活路径有没有把子标签并进父节点的地方”——**没有**，
但**规则本身已经在**：第 345 轮查明是死代码的 `SemanticsConfiguration::absorb`
里，两个标签正是用 `concat_attributed_string` 以 **`"\n"`** 接起来的。
所以缺的不是规则，是**遍历里那一层折叠**。

接法：收集器多一个 `merging: Vec<usize>`（合并节点的下标，最内层在末尾）。
`describe_subtree` 遇到一个会合并的盒子，就**用盒子自己的 rect 开一个空标签的节点**，
把下标压进 `merging`，走完孩子再弹出、关闭、**直接返回**；
`open` 在 `merging` 非空时**不再开新节点**，而是把标签折进 `nodes[*merging.last()]`。

上游把 `isSemanticBoundary` 和 `isMergingSemanticsOfDescendants` **一起**设为真，
不得不如此——能被折进去的东西**正是**一个边界。这里那个配对不是两个标志而是**一件事**：
答 true 的盒子就是开节点的那个，这句话写进了 `merges_descendant_semantics` 的文档。

**分隔符提成了一个常量 `MERGED_LABEL_SEPARATOR`。** 上游没给它名字，
它就是 `_concatAttributedStrings` 里裸的那个 `'\n'`。这里必须给名字，
因为活路径的折叠和被建模的 `concat_attributed_string` 是**两处代码做同一件拼接**，
写两遍的分隔符就是会走散的分隔符——而读屏软件听得出来：
`"Save Ctrl S"` 是一句话，`"Save\nCtrl S"` 是两句。

**嵌套是白捡的。** 合并盒子套合并盒子时，内层的 `open` 因为已经在合并中而返回 `None`，
于是它落回普通路径，孩子继续折进**外层**——一站不是两站。
这是规则推出来的，不是补的特例，为它写了一条测试钉住。

**变异扫描 12 个，第一遍 9 个红，1 个坏变异、2 个幸存：**

* **“合并栈永远不弹出”全绿——真窟窿。** 我那几条测试的合并盒子后面**都没有兄弟**，
  于是“折叠有没有终点”根本没被问过。而不弹出意味着
  “把这两样合起来”变成“把屏幕剩下的都合进来”——那是另一个、坏得多的控件。
  补了一条：合并盒子后面画的兄弟**仍然是自己一站**。补完转红。
* **“合并栈不在两帧之间清空”全绿，而且是不可达的**：push/pop 在
  `describe_subtree` 里裹着一次递归调用，中间没有提前返回的路。
  按老规矩写进注释而不是硬凑测试——留着它，是因为它上面那三行是同一个承诺
  （一次走查从零开始）。
* 一个坏变异（“后代照样自己开一站”）第一次写漏了 `return None` 那一半，
  等于什么都没改；改对之后**五条全红**。

尺子：十六把全部 exit 0（`stale_engines` 先报了三个陈旧，重建后转 0——它在干活）。
门：6269 通过；`cargo fmt --check` 干净；三个输出目录的 rustflutter_engine 与 rust_lib、
以及 rustflutter_unittests、flutter_gallery_unittests 全部重建。

第 346 轮那八个模型，现在接活了三个。

**下一步**：这一家三个接完了，**回头把第 345 轮那句话验一遍**——
`SemanticsNode::is_merged_into_parent` 仍然**只被写成 `false`**。
合并现在是活的，但它是**在收集时把标签折掉**，折进去的后代**根本没有节点**；
而上游的模型是**后代有节点、只是标记为已并入父节点**，
`SemanticsUpdateBuilder` 会把这个标志发给引擎。
两条路给读屏软件的结果一样，**给引擎的字节不一样**。
先查 FFI 那一侧到底发不发这个标志：**发就是真缺口，不发就说明这个字段本身是个模型**，
跟第 346 轮那批一样归档说清楚。

---

## 第 350 轮：一个指不到的站不再发给平台——以及一条过期理由撑着的偏离

按上一轮的“下一步”查 FFI 到底发不发 `isMergedIntoParent`。**答案是不发**：
`RfSemanticsNode` 里没有这个字段，26 个标志位里也没有它的位置，
整个 C++ 侧 grep 不到。所以那个字段只活在 Rust 这一边——
它唯一的用处是 `SemanticsNode::is_invisible` 里的那个守卫。

**顺着守卫往下查，撞上真缺口。** 上游 `object.dart:5764`：
`static bool shouldDrop(SemanticsNode node) => node.isInvisible;`，
6267 行 `children.removeWhere(shouldDrop)`。而本项目的 `is_invisible`
**只被它自己的测试调用过**——没有人丢过任何节点。

写了个探针确认，不是猜的：

```
PROBE id=0   label=""          w=200 h=200
PROBE id=901 label="nowhere"   w=0   h=0      <-- 发过去了
PROBE id=536870912 label="somewhere" w=63 h=16.8
```

一个 0×0 的节点带着标签渡过 FFI。读屏软件拿到一个**指不上、聚不了焦、
框不出高亮**的站。上游给的理由正是这条：一个不可见的节点“可以安全丢弃而
不损失任何描述当前屏幕内容的语义信息”。

**真正难看的是它的不对称。** `Clips::applied_to` 开头有个捷径：
两个裁剪都没有时直接 `return Some(bounds)`，空不空都发。于是**同一个空矩形**，
落在裁剪里就丢、落在裁剪外就发。

而捷径上写着理由：“这里空矩形通常意味着测试引擎没有排版文本，
而一段说了话的段落仍然值得读”。**这条理由已经过期了。**
`engine_test_stubs` 里 `StubParagraph` 的文档自己写着：
“**这些之前一直返回硬零**，于是本 crate 里每个字符串都是零乘零”——
后来给它补了宽度模型，字符串越长量出来越宽（上面探针里 "somewhere" 量到 63×16.8 就是证据）。
**一条为了保护不再发生的情况而留着的偏离，保护的代价是一整类节点。**
删掉捷径，全套 6270 条测试**一条都没红**——没有任何测试钉着那条理由。

顺手补上第二处：上一轮我自己加的合并分支**根本没问裁剪**，
直接拿 `offset`／`size` 开节点。同一条规则，两个开节点的地方。

**变异扫描 6 个，第一遍 4 个红，2 个是真窟窿：**

* **“丢的时候继续走子树”**全绿。我文档里写了“底下的东西跟着节点一起走”，
  却没测过。补测试时第一次用 `RenderStack` 想造零尺寸父节点——
  **假设错了**，无非定位子节点时 stack 取 `constraints.biggest`。
  换成 `RenderSizedOverflowBox::new(Size::ZERO, ..)`：自己零尺寸、
  孩子按进来的约束排。补完转红。
* **“合并盒子重新无视裁剪”**全绿，**而且我的测试是靠错的理由通过的**：
  被折进去的后代在进门时**被它自己那次裁剪检查**丢掉了，
  所以不管合并盒子问不问，标签都不见。漏出去的是**合并盒子自己那个节点**——
  空标签、坐在屏幕外的坐标上。断言改成**看节点在哪儿**而不是看它说什么，
  转红。

尺子：十六把全部 exit 0。门：6273 通过；`cargo fmt --check` 干净；
三个输出目录的 rustflutter_engine 与 rust_lib、以及 rustflutter_unittests、
flutter_gallery_unittests 全部重建。

**下一步**：这一轮是顺着“一个被建模却没人调用的谓词”摸到真缺口的，
**这个模式值得再走一遍，但不要造尺子**——第 341/346 轮两次尝试都没通过校准。
改成手查一批：`semantics.rs` 里还有哪些 `pub fn` 谓词只被自己的测试调用过。
已知的下一个候选是 `SemanticsNode::index_in_parent`（上游 `indexInParent`，
读屏软件靠它说“第 5 项，共 5 项”）——它在 `open` 里被写成什么？
`RfSemanticsNode` 里同样**没有这个字段**，所以先问的还是那句：
**发就是真缺口，不发就说明它和 `is_merged_into_parent` 一样是个只在 Rust 侧的模型**，
那就查清楚它在 Rust 侧到底有没有读者。

---

## 第 351 轮：五项里的最后一项仍然是第五项——上一轮开的洞，这一轮补的口

按“下一步”查 `index_in_parent`。查下来它跟 `is_merged_into_parent` 一样，
`RfSemanticsNode` 里没有这个字段。但顺着往下摸，
`SemanticsNode::indices_in_parent` 这个**已经写好、有文档、有单元测试的规则函数，
只被它自己的测试调用过**——正是上一轮那个形状。

**而这个缺口是上一轮亲手做出来的。** 第 350 轮之前，除了裁剪没有任何节点被丢掉，
“幸存者的位置”和“完整列表里的位置”是同一个数，
**这正是一个马虎的测试会挑的情形**。上一轮让空矩形真的被丢了，
于是“五项列表被裁掉前两项”第一次成为可能——
上游文档里的原话：三个节点，最后一个的下标仍然是 **4**。
数幸存者会说“第 3 项，共 3 项”，悄悄告诉读屏软件这个列表比实际短。

接的是第 346 轮那八个模型里的第四个：`RenderIndexedSemantics`。
上游 `describeSemanticsConfiguration` 就一行 `config.indexInParent = index`。

**方向是反的，这点写进了文档。** 上游靠 config 往上 `absorb`
（`indexInParent ?? child.indexInParent`）汇进那个节点；
这里的走查是**进门时建节点**而不是出门时并 config，
所以下标**往下传**：`RenderIndexedSemanticsBox` 在下降时把它放进 `pending_index`，
**子树里第一个开出来的节点把它取走**（`take`，不是 copy）。
两条路到同一个节点。

**收尾用了一个 `OnDrop` 守卫，理由写在它自己的文档里**：
`describe_subtree` 有**三个出口**——正常走完、裁剪丢弃、合并分支提前返回——
放回去的东西必须三个门都放。手写三遍就是三次机会去加第四个门然后漏掉一个，
而症状是**一个毫不相干的兄弟节点**领走了一个本该属于被丢掉子树的下标。

**变异扫描 9 个，第一遍 6 个红，2 个是真窟窿、1 个不可达：**

* **“offer 被复制而不是被取走”**全绿。我的列表项每项只开一个节点，
  于是“取一次”和“取无数次”看不出区别。补了一条：一个带下标的盒子里放两行文本，
  **第一行拿到 3，第二行是 None**。不然读屏软件会说“第 3 行的每一个部件也都是第 3 行”。
* **“offer 被清空而不是被还原”**全绿——嵌套没测过。补了“行里套单元格”：
  外层 7、内层 2，内层花掉自己的，**外层的还在**（`cell` → 2，`after the cell` → 7）。
* **“offer 不在两帧之间清空”**全绿，**且不可达**：守卫在每个出口都还原了。
  跟上一轮的 `merging.clear()` 是同一类，所以没有另起一段，
  而是把那条注释扩成同时说这两行——并把“真要是脏了会怎样”写清楚：
  一个是把下一帧整棵树折进一个已经不存在的节点，一个是把上一帧的位置发给下一帧的第一个节点。

顺手更新了第 345 轮那条 `the_walk_does_not_invent_an_index_in_parent`
的注释：它现在不是“记录一个缺口”，而是同一条规则的另一半——
**“搬运”和“发明”是两件最容易混的事**，这棵树里没人声明，所以没人拿到。

尺子：十六把全部 exit 0。门：6277 通过；`cargo fmt --check` 干净；
三个输出目录的 rustflutter_engine 与 rust_lib、以及 rustflutter_unittests、
flutter_gallery_unittests 全部重建。

第 346 轮那八个模型，现在接活了四个（排除、阻断、合并、下标）。

**下一步**：下标接上了，但**它现在哪儿也去不了**——
`RfSemanticsNode` 里没有 `index_in_parent` 字段，
所以一个刚刚被正确算出来的位置，走到 FFI 就掉了。
这是一条**能在一轮里做穿**的路：`RfSemanticsNode` 加字段、
`rust_app_api.h` 同步、`app.rs` 里 `send_semantics` 填上、
`size_of` 那条静态断言跟着改。先按行为确认一件事：
...
## 第 353 轮：“第 3 行，共 40 行”终于能说出口——先给没人看着的结构体装上眼睛

四段一起做穿：属性、走查、C 头文件、C++ 消费端。但**第一件事是上一轮答应的那条断言**。

### 先装眼睛，再动手

`RfAppHost` 和 `RfAppInterface` 两侧都有 `static_assert`，`RfSemanticsNode`
**两侧都没有**。而它恰恰是最危险的一个：另外两个全是指针，字段错位就整体平移一个指针；
这个混了 `int32_t`／`float`／`double`／指针／`size_t`，
**一侧多一个字段，错位的只是一部分**，读的时候就顺着读进下一个字段的字节里——
读屏软件会收到一个由半个指针拼成的滚动范围。

两侧都加上（Rust 侧 `size_of == 120`，C++ 侧 `sizeof(void*) != 8 || sizeof(...) == 120`，
32 位跳过而不是断错）。**并且校准过**：只往 Rust 侧加两个字段，编译立刻红：

```
error[E0080]: evaluation panicked: RfSemanticsNode has drifted from rust_app_api.h
```

文档里也写清了它**抓不到**什么：两个同宽字段对调，尺寸不变，它看不出来。
把检查说得比它实际更强，比没有检查更糟。

### 然后是那条链

`SemanticsProperties` 加 `scroll_child_count` 与 `scroll_index`。
**它们来自两个不同的地方，所以是两个字段而不是一对**：
数量是**声明**的（上游 `config.scrollChildCount = semanticChildCount`，
只有列表自己知道有多长），下标是**发现**的（上游 `assembleSemanticsNode` 里
`firstVisibleIndex ??= child.indexInParent`，只有走查知道谁活下来了）。

于是下标填在 `close` 里——一个列表要等自己的行走完才知道哪一行在显示。
上游跳过 `isHidden` 的孩子；**这里那一步早就做完了**——不在玻璃上的节点第 350 轮起
根本不会打开，所以 `nodes` 里剩下的**就是**上游那份 not-hidden 列表。
搜索从 `index + 1..` 开始，靠的是第 348 轮就在用的那条连续性。

FFI 两侧各加两个 `int32_t`，断言同步改到 128。
**null 用 -1 而不是 0**，理由写在两侧：引擎的 `scrollIndex`／`scrollChildren`
是裸 `int32_t`，默认 0，而**第 0 行是一个真答案**——用 0 当空值，
每一个普通方块都会声称自己是个正显示着第一行的列表。C++ 侧对负数**不赋值**，
让引擎保留它自己的默认。

顺手改掉一句错的注释：`scroll_position` 的文档原本说它就是读屏软件念的“第 3 行，共 40 行”。
不是。它是**像素**。“340，共 1200”不是一句给人听的话——这正是新加那两个字段的理由。

### 变异扫描 9 + 6，两轮下来 4 个真窟窿、1 个坏变异、1 个如实记账

* **“连自己上面的节点一起找”**（`nodes[..]`）全绿：我的测试里列表之前没有别的带下标的东西。
  补了一条——列表**上面**放一个 index 99 的 chip，列表里放 index 4 的行，
  断言列表报 4。不然它会把一个**不属于自己的行号**念出来。
* **“两个显示不同行的列表算相等”**全绿。两道门（`update_from` 和 `flush` 第三关）
  都靠属性比较，漏掉的字段就是**变化被悄悄吞掉**的字段。
  补了一条：同样的像素、同样的字，行号 2 对 102，断言 `assert_ne!`。
* **“声明的数量被丢掉”**全绿——**活路径里没有任何东西声明数量**
  （`RenderViewport` 只有一个孩子，诚实地传 `None`），所以那个参数从没被带值调用过。
  直接对函数补了一条。
* **“-1 变成 0 过 FFI”**全绿。把那条空值编码抽成 `pack_scroll_count`
  （跟隔壁 `pack_text_direction`／`pack_semantics_flags` 同一个写法）并补测试，转红。
* 一个坏变异（数量那条第一版写出了语法错）。

### 一个测出来的、不打算糊过去的事实

**`send_semantics` 里那二十行打包代码，一条测试都没跑过。**
不是推断——校准过：把每个节点的 `left` 改成 `0.0`，**全套 6287 条全绿**。
纯函数各自有测试，产出节点的走查有一堆测试，
**把前者搬进后者的那二十行一条都没有**——而那正是“字段读错节点”或
“读对了节点填错了槽”唯一会发生的地方。按老规矩写进 `send_semantics` 的文档，
而不是硬凑一个测试。

尺子：十六把全部 exit 0（`stale_engines` 因为改了头文件先报陈旧，重建后转 0——它在干活）。
门：6287 通过；`cargo fmt --check` 干净；三个输出目录的 rustflutter_engine 与 rust_lib、
以及 rustflutter_unittests、flutter_gallery_unittests 全部重建。

**下一步**：就做上面那件事——给 `send_semantics` 造夹具。
一个 `RfAppHost`，它的 `update_semantics` 把收到的东西记下来，
于是可以推一帧、读回字节。这一轮已经证明了它值得做（`left: 0.0` 全绿），
而且它一次性覆盖的是**二十个字段**，不只是这一轮新加的两个。
先按行为确认一件事：`RfApp` 能不能在测试里造出来而不去碰真引擎——
**能就直接接，不能就说清楚卡在哪一层**，那也是一个明确的答案。

---

## 第 354 轮：那二十行终于被跑过了——不是造夹具，是把它挪到能被跑的地方

上一轮量出来的事实：`send_semantics` 里把 `SemanticsNode` 抄进 `RfSemanticsNode`
的二十行，**一条测试都没跑过**（把每个节点的 `left` 改成 `0.0`，全套全绿）。
这一轮就做这一件事。

**先回答上一轮问的那个问题**：`RfApp` 能不能在测试里造出来？
`RfAppHost` 只是一堆函数指针，好造；但 `AppInstance` 有十几个字段
（`Box<dyn Application>`、两个 `HashMap`、`GestureRouter`、`Keyboard`……），
为了测二十行打包代码去搭一整个 app 实例，**是拿夹具的复杂度换测试的可信度**。

所以换了个方向：**这二十行本来就不需要 `self`，它只需要节点。**
抽成 `PackedSemantics::of(nodes)`，`send_semantics` 剩下三行——
空检查、view id、长度。缺口不是“缺一个夹具”，是**这段代码长在了够不着的地方**。

**为什么是一个结构体而不是一个函数**：`raw` 里存的是指进 `strings` 和 `children`
的裸指针，三者必须一起活、一起死。文档里写了它能安全移动的理由，
**而且没有只写理由**——补了一条测试真的把它 `Box` 起来再移动两次，
然后从移动后的指针读回字符串和子节点。“它可以移动”正是那种应该跑一遍而不是想一遍的断言。

**测试用三个节点，每个字段都互不相同**，这样“从隔壁节点读来的值”看起来就不像个合理的数
而像个属于别处的值。第三个节点是后补的，理由见下。

**变异扫描 12 个，第一遍 11 个红，1 个真窟窿：**

* **“子节点指针取自第一个节点”**（`children[0]`）全绿——
  因为我原来只有两个节点，而第二个是叶子，`child_count` 是 0，
  **指错了也读不出来**。补了第三个节点、让第二个也带一个孩子，转红。
* 那个引发这一轮的变异 **`left: 0.0` 现在红了**。
* 一并红的还有：id 全取第一个、**hint 与 value 互换槽位**（最难用眼睛看出来的一种：
  提示会被当成值念出来，念了，只是用错了语气）、字符串块整体偏一个节点、
  子节点长度多一、NUL 截断标签、两个 extent 互换、两个 scroll 计数互换、
  方向不打包、top/bottom 互换、动作位集丢掉。

`send_semantics` 上那段“这里没有任何测试”的注释**改掉了，没有删掉**：
现在说清楚被盖住的是打包、**剩下三行仍然没被盖住**，以及为什么这次不追（要 host，三行不是二十行）。

尺子：十六把全部 exit 0。门：6291 通过；`cargo fmt --check` 干净；
三个输出目录的 rustflutter_engine 与 rust_lib、以及 rustflutter_unittests、
flutter_gallery_unittests 全部重建。

**下一步**：这一轮的教训值得马上用一次——**“够不着”和“没测”是两回事**，
而前者可以靠挪代码解决。C++ 那一侧现在是对称的窟窿：
`RuntimeController::OnUpdateSemantics` 把 `RfSemanticsNode` 抄进
`flutter::SemanticsNode`，**同样是几十行、同样在一个需要整个 controller 才能调到的方法里**，
...
## 第 358 轮：第三把尺子，也没留下——但这次知道为什么

上一轮说：三条假声称现在是一个**已知的坏样本集**，
拿它当校准集去试一把尺子，**先验它会响，再看它报多少**。这一轮就做这一件事。

坏样本已经被修好了，所以先从 git 里把它们捞回来
（`ec24aff~1` 的 `app.rs`、`af7d26a~1` 的 `render.rs` 与 `async.rs`）。

### 先校准，再测量——顺序这次是对的

第一版 **3 中 1**。两条漏掉的原因都是机械的，不是判据错：

* 声称**跨两行注释**。改成把连续注释行拼成一个块——
  这是修解析，不是往样本上凑。→ 3 中 2。
* 还漏 `app.rs`。查出来是个正则 bug：`\bunittests` 在
  `rust_ffi_unittests` 里**永远匹配不上**，因为 `_` 和 `u` 之间没有词边界。
  → **3 中 3**。

### 然后是决定它去留的那一步

全仓库报 **13 条**。数量正好是可以一条条看的——于是一条条看了：

* **6 条根本不是声称**。`covered by` 和 `pinned by` 是普通英语：
  “被一个透明矩形盖住”“positioned children are pinned by their edges”
  “what is covering it（键盘挡住了多少）”。它分不出来。
* **4 条是上一轮已经验证为真的**（tooltip 五字段、零延时、FAB 旋转，加我自己写的那段修复叙述）。
* **3 条是新的、可查的**——`theme.rs`、`cupertino.rs`、`engine_test_stubs.rs`。
  逐条查了：`tapping_the_clear_button_tells_the_application_the_field_is_empty`
  **确实存在**（`cupertino.rs:7752`）；`ffi_unittests.cc` **确实**光栅化并读回像素；
  `theme.rs` 那段是在叙述一个**已经补上的**缺口。**三条全真。**

**净收获：零个新故障。** 一把在自己被调出来的样本上得满分、
在别处一个新问题都找不到的仪器——这正是“报的是数字不是事实”。**丢掉。**

### 比工具更值钱的是丢掉它的理由

**那个坏样本集已经被清空了。** 第 356、357 轮是**用手**找到并修好那三条的，
所以这把尺子唯一被证明的本事，是认出三句**已经不存在**的话。
**在已经修好的故障上校准，等于没有校准。**

这句话写进了 `tools/stale_notes.py` 的文档——它是最该长出这个功能的地方，
所以也是下一个人最该先读到“试过了、为什么没留”的地方。
连同一句可操作的：要让它值得造，需要一个**用别的办法找到的**坏样本集；
在那之前，手过一遍 `covered by|exercised by|pinned by` 就是这门技术的全部，一轮就能做完。

三次造尺子（341、346、358）都没留下。前两次输在**先造后校准**；
这一次校准做对了，输在**校准集是自己刚扫干净的地面**。这是新的一课，不是同一课。

尺子：十六把全部 exit 0（`stale_notes.py` 只改了文档，仍然 exit 0）。
门：Rust 6296 通过；`cargo fmt --check` 干净。

**下一步**：这条线到此为止，回到真正的移植缺口。
先跑 `python tools/depth.py` 看队头——上一次看时头部是
`Icons`／`CupertinoIcons`（那是字模表，不是行为）、
...
## 第 360 轮：一张卡片重新变成一件东西——一条“暂时接不了”的理由到期了

按上一轮的“下一步”查 `Card`。第 345 轮已经把两个缺口记得很清楚，
所以这一轮**不是重新发现**，是去核对那两条记录**今天还成不成立**。

`margin` 那条仍然成立（还是本项目用 `padding`、上游用 `margin`，
一个在材质里一个在材质外，改了整个 gallery 的卡片都会动）。

**`semanticContainer` 那条到期了。** 原话是：

> Not portable here yet: 它要设的两个标志在
> `SemanticsConfiguration` 和 `RenderSemanticsAnnotations` 上，
> **而这两个都不是树真正建的东西**。

写这句话的时候是对的。但第 347–349 轮把那一家三个接上了活路径，
其中第 349 轮的 `RenderMergeSemanticsBox` **正好就是这对标志合起来的意思**——
“把底下所有东西折进一个节点”。

上游是一个标志设在两处、第二处取反：

```dart
Semantics(container: semanticContainer,
  child: Padding(... child: Material(...
    child: Semantics(explicitChildNodes: !semanticContainer, child: child))))
```

两半说的是同一件事：卡片要么是**一个节点、孩子折进去**，
要么**根本不是节点、孩子各自站着**。于是这里**一个包装说两半**——
而且比上游更难写错：上游那两个可以被设成互相矛盾，这一个不能。

测试走真遍历：一张放着 "Sunset" 和 "Taken in June" 的卡片，
默认读作 **一站**（`"Sunset\nTaken in June"`），
`with_semantic_container(false)` 读作**两站，各自可达**。

### 变异扫描 4 个，第一遍 3 红，1 个是真窟窿——而且是上一轮那一课的复读

**“把构造函数里的默认值从 true 改成 false”全绿。**
因为我那两条断言**每次都显式调了 setter**，默认值一次都没被走到。
这正是上一轮刚写下的那句话：**默认值最容易抄错，因为它不出现在任何调用点上**——
它也因此不出现在任何**只调用了它的测试**里。
补了第三条断言：一张**谁都没配置过**的卡片仍然是一件东西。补完转红。

尺子：十六把全部 exit 0。门：Rust 6302 通过、`cargo fmt --check` 干净；
C++ 34 个 gtest 全过；三个输出目录与三个测试二进制全部重建。

第 346 轮那八个模型接活的第四个，现在有了第一个**运行时调用者**：
在这之前 `RenderMergeSemanticsBox` 只被它自己的测试建过。

**下一步**：这一轮验证了一个做法值得重复——**回头核对旧笔记里“暂时做不了”的理由**，
因为后来的轮次可能已经把前提改掉了，而笔记不会自己更新。
`Card` 这条是被第 347–349 轮解锁的。同一批解锁的还应该有别的：
先按行为查 `semantics_markers.rs` 与 `render_semantics.rs` 里
...
## 第 362 轮：滑块第一次说出自己是滑块、现在几成、再划一下会到哪儿

按上一轮的“下一步”读 `RenderSemanticsAnnotations`。它本身很薄（五个字段），
但按说好的**逐条问“这条属性活路径送到平台了吗、送对了吗”**，第一条就撞上了。

`increased_value` / `decreased_value` / `is_slider`——
全仓库 grep，**没有任何东西设过它们**。和第 352 轮的列表一模一样：
**管子早就通到 FFI 了**（`IS_SLIDER` 有位、三个字符串有字段、
`runtime_controller.cc` 有赋值），**只是没人往里放水**。

也就是说：**这个 port 里的滑块，到读屏软件那里是一个普通方块**——
不说自己是滑块、不报当前值、也没有任何办法去改它。

接的是上游 `_RenderSlider.describeSemanticsConfiguration`。三件事值得记：

**一、三个值是字符串，因为只有控件知道那个数是什么意思。**
上游允许调用者用 `semanticFormatterCallback` 替掉全部三个，
而它兜底的默认**不是原始值而是百分比**——所以一个 100..200 的滑块停在 150
报的是 `"50%"`，不是 `"150%"`。这是唯一一个**不知道量程也能听懂**的数。

**二、先夹再念，不是念完再夹。** 上游是 `clampDouble(value + unit, 0.0, 1.0)`：
97% 的滑块加 5% 步长，报的下一站是 **100%**，不是 102%。
念未夹的数等于**先许诺一个它去不了的地方，然后不去**。

**三、步长是 `divisions != null ? 1/divisions : _adjustmentUnit`**，
而 `_adjustmentUnit` 在 iOS／macOS 是 0.1、其余是 0.05，
各自对着那个平台自家滑块的行为。分档滑块必须用分档做步长——
它只能停在分档上，更小的步会落在两档之间。
上游读的是 `Theme.of(context).platform`，所以这里读 `ThemeData::platform`
（那个“把桌面当手机用”的覆盖开关也就跟着生效了）。

动作只在 `isInteractive` 时给出，`isEnabled` 同源——
给一个没有 `onChanged` 的滑块挂上“上划增大”，等于告诉读屏用户这控件是能用的。

顺带补了 `Slider::with_on_change`：在这之前设 `on_change` 的**唯一**公开途径是
`wired(handle, set)`，要一个 `StateHandle`。`with_on_change` 就是它内部那一层，
也就是上游的 `onChanged` 本身。

**变异扫描 9 个，全红**：不说话了、不说自己是滑块、不夹就念、增减互换、
报原始值而不是比例、分档不影响步长、所有平台都用苹果的步长、
不可交互也给动作、禁用了还说自己可用。

尺子：十六把全部 exit 0。门：Rust 6311 通过、`cargo fmt --check` 干净；
C++ 34 个 gtest 全过；三个输出目录与三个测试二进制全部重建。

**下一步**：连着两轮（352 列表、362 滑块）都是同一个形状——
**FFI 字段早就铺好，活路径没有生产者**。这不是巧合，值得**一次性数清楚**：
`SemanticsProperties` 和 `SemanticsFlags` 里的每一个字段，
按行为查“**除了测试和默认值，还有谁写过它**”。
`RfSemanticsNode` 的 26 个标志位是现成的清单。
**先手查，不要造尺子**——第 358 轮刚证明这类判据的校准集一旦被自己扫干净就失效了；
这一次是**先数一遍还没被扫的地面**。数出来是空的就说清楚，
数出来有几条，就挑最影响读屏体验的那条，下一轮接。

---

## 第 363 轮：先把地面数了一遍，再接一条——照片开口说自己是照片

按上一轮的“下一步”**先手数**：`SemanticsFlags` 十九个标志，
逐个查“除了 `semantics.rs` 自己的构造函数和默认值，活路径上还有谁写过它”。

第一版脚本太糙——`selected` 匹配到 `chip.rs` 的 78 处（那是 chip 自己的字段，
跟语义标志无关）。**这正是第 358 轮那把尺子死掉的原因，只是这次发现得早。**
换成两级问法才对：**构造函数在不在（规则有没有）+ 构造函数有没有调用者（规则有没有生产者）**。

数出来的结果：

| 构造函数 | 调用者 |
| --- | --- |
| `button` / `disabled_button` | 2 / 2 |
| `text_field` / `header` / `route_header` | 1 / 1 / 1 |
| `slider`（第 362 轮刚接） | 1 |
| `scrollable`（第 352 轮刚接） | 1 |
| `label` | 4 |

**而 `is_image` 和 `is_link` 连构造函数都没有。** 它们只是字段：
会 `merge`、会参与 `conflicts_with`、会过 FFI（`IS_IMAGE` / `IS_LINK` 有位），
**从来没有人设过**。

也就是说：**这个 port 里的照片，到读屏软件那里是一个没有名字的方块。**

这一轮接 `is_image`（`is_link` 留给下一轮，理由见“下一步”）。
上游 `_ImageState.build` 三行两个决定：
...
上游**无论有没有标签都设 `image: true`**，用 `container` 决定它是否成为一站：
有标签就自成一站，没标签就把这个标志**折进外面那个节点**——
所以按钮里的装饰图给按钮添一句“图片”，而不是变成第二个可落脚的地方。

这个走查**没有 config 合并那一层**（见 `SemanticsConfiguration` 上的注释），
一个 annotation 就是一个节点。给无标签的图片也加注解，
等于**给每一张装饰图添一个没有字的站**——**比上游吵，不是比上游忠实**。
所以标志只在“上游会成站”的地方升起，无标签的情况原样放着，
并写明：想要上游那种折叠，今天可以用 `RenderMergeSemanticsBox` 包一层，
那就是同一条规则的另一种写法。

`exclude_from_semantics` 压过标签——那是给“意思已经由旁边的文字说完了”的图片用的，
连“图片”这个词都嫌多。

**变异扫描 5 个，全红**：不说话了、不说自己是图片、无视 exclude、
无标签也注解（那个吵的版本）、只送标志不送标签。

尺子：十六把全部 exit 0。门：Rust 6314 通过、`cargo fmt --check` 干净；
C++ 34 个 gtest 全过；三个输出目录与三个测试二进制全部重建。

**下一步**：`is_link` 是同一张清单上剩下的那条，但它**不是同一种活**——
`is_image` 的宿主 `RenderImage` 本来就在，只差一句话；
而这个 port **有没有一个“链接”控件**得先查。
...
## 第 365 轮：单选按钮开口说自己是一组里的一个——规则写好很久了，没人叫它

按上一轮的“下一步”先核对：`is_header` / `names_route` 那个唯一的调用者是不是活的。
**是活的**——`components.rs:948`（标题样式的 Label）和 `components.rs:1912`（路由标题），
两处都在真 build 里。所以那两条不缺。

转去数 `hint` 与 `is_read_only`，中途撞上一个**更值钱也更难看的**东西。

### 我第 363 轮那张表是不完整的

那一轮我按**一份写死的构造函数名单**去数调用者，
而 `SemanticsProperties::radio` **不在名单上**。
它存在，文档写得极好（三段，把上游那个“同一件事说两遍”的平台分歧讲透了），
**而它的调用者只有它自己的测试**。

不止一处：`radio_group.rs` 里还有一份 `RawRadio::semantics` → `RadioSemantics`，
同样有测试，同样**在自己文件外零调用**。
**同一条规则写了两遍，两遍都没有生产者**——
于是 `controls.rs` 里那个真的 `Radio` 控件，
到读屏软件那里是**一个方块加旁边一个词**。

（这一轮我一开始还差点写第三遍：新加了个 `SemanticsProperties::radio`，
编译器报 duplicate definition 才发现已经有了。删掉重写的那份，接现成的。
**这就是“规则没有生产者”会导致的下一件事：有人再写一遍。**）

### 接上的是哪三件事

* `inMutuallyExclusiveGroup` + `checked`：每个平台都听，**这两个合起来才是“单选”**。
* `selected` **只给苹果**：VoiceOver 读它自己那个属性。
  到处都设不是中性的——TalkBack 会把一个单选同时念成 selected 和 checked。
  而且 `None` 必须是**第三个答案**，不能塌成 `false`：
  “没被告知”和“告知了未选中”不是一回事。
* **提示只给没选中的那个**，且只在苹果上：选中的那个 `selected` 已经说过一遍了；
  而没选中的那个如果沉默，**跟一个什么都不做的控件分不出来**。

平台从 `ThemeData::platform` 读，理由和第 362 轮滑块一样。
禁用时清掉动作并把 `is_enabled` 落下去。

**变异扫描 8 个，全红**：不说话了、不说自己是一组里的、
`selected` 到处发、提示给了选中的那个、提示给了所有平台、
禁用了还给点击、禁用了还说可用、平台被写死。

尺子：十六把全部 exit 0。门：Rust 6318 通过、`cargo fmt --check` 干净；
C++ 34 个 gtest 全过；三个输出目录与三个测试二进制全部重建。

**下一步**：这一轮暴露的**不是又一个缺口，是我数缺口的方法有洞**——
第 363 轮那张表按写死的名单数，漏掉了 `radio`，也就可能漏掉别的。
下一轮**重数一遍，这次不写名单**：
把 `semantics.rs` 里**每一个**返回 `SemanticsProperties` 的 `pub fn` 列出来
（按签名找，不按名字猜），逐个数文件外的调用者。
**先把这一步做完再接任何东西**——
连着四轮（352/362/363/365）都是“规则有、生产者无”，
在没数清总共有几条之前，一条一条接是在**猜哪条最值钱**。

---

## 第 366 轮：把账重新数清楚，然后接掉最后一条——复选框会说自己勾没勾了

按上一轮的“下一步”：**不写名单，按签名重数。**

第一次 `grep "pub fn .*-> SemanticsProperties"` 只捞到 **8** 个。
**这正是第 363 轮那张表出洞的原因**：签名跨行的函数捞不到，
而 `radio`、`route_header`、`scrollable`、`slider` 的参数表都是多行的。
换成跨行正则（`re.S`）后是 **12** 个。
...
| --- | --- |
| `label` | 7 |
| `button` / `disabled_button` / `toggle` | 2 各 |
| `header` / `route_header` / `text_field` / `radio` / `scrollable` / `slider` | 1 各 |
| **`check`** | **0** |

**账清了：一共 12 条规则，只剩一条没有生产者。** 于是这一轮把它接掉。

`Checkbox` 在 `controls.rs` 里一直是画得好好的一个方块加一个词——
**到读屏软件那里就真的只是一个方块加一个词**，跟一行标签分不出来。

接的时候有一处必须想清楚：`check(label, value)` 的 `value` 是 `Option<bool>`，
而 **`None` 是上游的 `mixed`**——`Checkbox` 只在 `tristate` 打开时才传它。
这个复选框没有第三态，所以传 `Some(checked)`；
传 `None` 会把**每一个普通复选框都念成“部分勾选”**，
而“部分勾选”是读屏用户会照着做事的一个真答案。

**变异扫描 5 个，全红**：不说话了、普通复选框被念成部分勾选、
勾与未勾互换、禁用了还给点击、`checked` 标志根本不设。
（其中一个选择串第一遍没匹配、第二遍匹配到 2 处——
**Checkbox 和 Radio 现在长得一样**，这本身是个信号；
加了唯一锚点才算数。**跳过和撞多都不是通过。**）

尺子：十六把全部 exit 0。门：Rust 6321 通过、`cargo fmt --check` 干净；
C++ 34 个 gtest 全过；三个输出目录与三个测试二进制全部重建。

**这条线到此为止**：`semantics.rs` 的十二条属性规则，现在**每一条都有活的生产者**。

**下一步**：变异扫描时撞见的那件事值得做——
**`Checkbox` 和 `Radio` 的语义包装现在逐字一样**（约二十行：建属性、
落 `is_enabled`、禁用时清动作、`semantics_with_action` 里转发 tap）。
第 353 轮的 `MERGED_LABEL_SEPARATOR` 记过同一条道理：
**写两遍的规则就是会走散的规则**。
下一轮先按行为确认这两处**确实逐字等价**（不是看着像），
是就抽成一个共用的包装，并**用变异验证抽完之后两边都还红**；
如果有哪怕一处不同，那个不同点本身就是缺口，先查它。

---

## 第 367 轮：三份一模一样的包装并成一份——先证明它们真的一样

按上一轮的“下一步”，**先按行为确认，不靠眼看**：用正则把 `controls.rs` 里
两处 `let described = { ... };` 整块抠出来做 diff。

结果很干净——**两者只在“怎么造属性”那几行不同**：

```
-            let mut properties = SemanticsProperties::check(
+            let properties = SemanticsProperties::radio(
-                Some(checked),
+                selected,
+                ThemeData::of(context).platform,
+                DefaultWidgetsLocalizations.radio_button_unselected_label(),
```

**其余逐行相同。** 再去看 `components.rs` 的按钮，尾巴也是同一段。

所以抽出来的**不是“去重”，是一条规则**：
**语义上的那次点击，必须落到手指落到的那个 handler 上。**
这句话本来就写在按钮那段注释里（“两条路不能对‘按下去会怎样’有分歧”），
现在它有一个自己的函数可待——`semantics::tappable`——而不是散在三处的注释。

`on_tap` 为 `None` 时**仍然建节点**，理由写进文档：
**说一个东西是什么，不取决于它能不能被操作**；
一个禁用控件如果干脆从树里消失，比它说自己被禁用要糟。

三处差异保留在各自那边，**因为它们是真的不同**：
按钮靠挑 `button` / `disabled_button` 两个构造函数来表达禁用，
复选框和单选按钮靠改标志。合并那一层会把一个真差别抹平。

### 变异扫描：抽完之后两头都要还红

这才是抽取的验收条件，不是“测试还绿”。6 个变异全红：

* **共用规则本身** 3 个：不认 Tap、不叫 handler、干脆不建节点（7 红）。
* **三个调用点各 1 个**：复选框不用它（2 红）、单选不用它（3 红）、按钮不用它（1 红）。

三个调用点**各自独立地**还能变红——说明抽取没有把它们的覆盖并成一份。

尺子：十六把全部 exit 0。门：Rust 6321 通过（数目不变，这一轮不加行为）、
`cargo fmt --check` 干净；C++ 34 个 gtest 全过；
三个输出目录与三个测试二进制全部重建。

**下一步**：这一轮的 diff 手法值得再用一次，因为它**便宜且能证伪**。
`controls.rs` 里还有 `Switch`（`toggle` 那条规则的宿主，2 个调用者之一）
和 `Chip`——先按同样的办法抠出它们的构建块做 diff：
**如果 `Switch` 的语义包装和这三个也一样，它就该用 `tappable`**；
**如果它没有语义包装**，那它就是第 366 轮那张表数不出来的那种缺口——
表数的是“规则有没有生产者”，数不出“**同一条规则该有几个生产者**”。
Switch 用的是 `toggle`，而 `toggle` 有 2 个调用者；先查那 2 个是谁。

---

## 第 368 轮：选中的筛选片和没选中的，终于听起来不一样

按上一轮的“下一步”查 `toggle` 那两个调用者，以及 `Switch` / `Chip`。

### 一、两个 Switch，一个该合并，一个不该

`components.rs:1186` 的 Material 开关，尾巴和上一轮抽出去的**逐字相同**——
它是第四份拷贝，换成 `tappable`。

`cupertino.rs:1212` 的 Cupertino 开关**看着像，其实不是**：
它的 handler 是 `changed(!value)`（一个 bool 回调），不是 `TapEvent` 回调。
**这是真差别，不是走散**，所以留着。
上一轮那句“先证明它们真的一样”在这里第二次挣回了工钱。

### 二、`Chip` 根本没有语义——而这正是第 366 轮那张表看不见的缺口

那张表数的是“**一条规则有没有生产者**”。
`Chip` 缺的不是规则的生产者，是**它自己从来没被算进任何一条规则**——
它到读屏软件那里是一个方块加一个词，
于是**开着的筛选片和关着的筛选片听起来一模一样**，
而那恰恰是筛选片存在的唯一理由。

接上游 `RawChip.build`：`button: tapEnabled`、`container: true`、
`selected`、`enabled`。两处分寸写进了文档：

* **用 `selected` 而不是 `checked`。** 上游按 `kIsWeb` 分叉，
  自己的注释给了理由：`aria-selected` 只对少数 ARIA role 有效，
  网页上引擎会丢掉它，所以那边改用 `aria-checked`。
  **那是关于一个本项目不构建的平台的事实**，所以这里只有非网页那一支，
  并写明“真有 web target 的那天，另一支加在这里”。
* **没人监听的片子不是按钮。** `button: tapEnabled`，
  而且那时 `enabled` 也是空——**“没有启用状态”是第三个答案**，不是“被禁用”。

### 一个我自己撞上去的错，值得记

我一开始按 `Card`（第 360 轮）的样子，把注解**包进** `RenderMergeSemanticsBox`。
结果三条测试全红：折叠节点**只收标签，不收标志**，
于是片子的 `selected` / `is_button` 全被折没了。

正确的答案是**根本不需要合并**：片子**自己带标签**，
而“被标签包住的文本不再单独念”这条规则（`yields_to_a_label`）已经保证了一站。
`Card` 需要合并，是因为**它自己不带标签**。
这两件事看着像同一个需求，其实是两个。

顺带给 `Chip` 补了 `on_tap`——在这之前设 handler 的唯一公开途径是
`wired(handle, action)`，和第 362 轮滑块一模一样的形状。

**变异扫描 8 个，全红**：片子不说话、每个片子都成按钮、
标签片子谎称有启用状态、选中与否听不出来、选中被反转、
标签片子被给了点击、`tappable` 被写死、开关不再够到它的 handler。

尺子：十六把全部 exit 0。门：Rust 6324 通过、`cargo fmt --check` 干净；
C++ 34 个 gtest 全过；三个输出目录与三个测试二进制全部重建。

**下一步**：这一轮证明了**“哪些控件根本没有语义”是一个和第 366 轮那张表正交的问题**，
而且它比“规则有没有生产者”更贴近读屏用户的实际损失。
所以下一轮**把这个也数清楚**，同样先手数不造尺子：
列出 `controls.rs` 与 `components.rs` 里所有 `impl Component for X`，
逐个查它的 `build` 里有没有 `semantics` 字样。
数出来会有一批（`ActionChip`、`Switch` 的各种变体、`Tooltip`……），
**然后按“听不出差别的损失有多大”排序，而不是按顺序接**——
筛选片这一轮之所以值钱，是因为它的两个状态本来完全同音。

---

## 第 369 轮：标签栏说出“第 3 个，共 5 个”，并说清楚你在哪一个——顺带修好一条折叠会吞字的规则

按上一轮的“下一步”先数：`controls.rs` + `components.rs` 里所有
`impl Component for X`，看各自的 `build` 里有没有 `semantics`。

* `controls.rs`：**会说话 4 个**（Checkbox、Radio、Chip、TooltipBubble），**哑的 16 个**。
* `components.rs`：**会说话 5 个**（Button、Label、Switch、Slider、AppBar），**哑的 10 个**。

**26 个哑控件。** 按“听不出差别的损失”排，队头不是最大的那个（AlertDialog 470 行），
而是 **`TabBar`**——理由和第 368 轮筛选片一样、而且更重：
**没有 `selected`，一栏标签里每一个的播报完全相同**，读屏用户不知道自己在哪一页。

上游每个标签包成
`MergeSemantics(Semantics(selected: index == current, label: tabLabel(i+1, n)))`。
而 `DefaultMaterialLocalizations::tab_label` 在本项目里**写好了、文档写了
“一基 vs 零基”那个坑、零个调用者**——又一条有规则没生产者的。

### 撞出一条真 bug：带标签的折叠节点会把自己要收的字吞掉

接上之后测试红：合并节点只剩 `"Tab 1 of 3"`，**标签自己的字没了**。

原因是第 349 轮那条抑制规则：`yields_to_a_label && inside_labelled()`——
折叠节点自带标签，于是 `labelled_depth` 抬起来了，
里面那个 `Text` **在还没被折叠之前就被判定“上面已经有人说了”而丢掉**。

**折叠存在的全部意义就是去收下面的字**，所以这条抑制不该在合并里生效。
加了 `inside_merge()` 例外，并写清楚：
**第 349 轮自己的测试全用的是空标签的合并节点，所以一直没人发现。**

### 一处与上游不同，写下来而不是留给人猜

上游把 `Semantics` 放在 `Stack` 里当**兄弟**，所以顺序是
“Home，第 3 个共 5 个”；这里折叠节点自己的标签在前，
出来是“第 3 个共 5 个，Home”。两者说的是同样两件事，
要拿到上游那个顺序得让位置文本当**后一个兄弟**，
而**没有尺寸的兄弟在能被折叠之前就被丢掉了**（第 350 轮那条规则）。

**变异扫描 6 个，全红**：标签栏不说话、每个标签听着都像当前的、
位置按零基播报、位置被丢掉、**折叠吞掉自己要收的字**、
**折叠完全不抑制导致标签重复**。后两个是一对，把新例外的**两边**都钉住了。

尺子：十六把全部 exit 0。门：Rust 6326 通过、`cargo fmt --check` 干净；
C++ 34 个 gtest 全过；三个输出目录与三个测试二进制全部重建。

**下一步**：那 26 个哑控件的清单还在，**队列的下一个是 `AlertDialog` / `Dialog`**——
损失和标签栏不同但更重：**读屏用户根本不知道有个模态弹出来了**，
而模态还会抢走焦点。上游是
`Semantics(scopesRoute: true, namesRoute: true, explicitChildNodes: true, label:)`。
先按行为查两件事：**`names_route` 这个标志本项目有生产者吗**
（第 365 轮数到 `route_header` 有 1 个调用者，是页面标题，不是对话框），
以及 **`scopesRoute` 有没有对应物**——
没有的话它就跟 `is_link` 一样是“缺的不是标志是机件”，那就说清楚再决定。

---

## 第 370 轮：对话框开口说自己是对话框——以及 `scopesRoute` 缺的不是标志

按上一轮的“下一步”先查两件事：

* **`names_route` 有生产者**，但只有一个：`route_header`（页面标题），不管对话框。
* **`scopes_route` 在本项目里完全不存在**——`SemanticsFlags` 没有这个字段，
  `RfSemanticsNode` 没有这个位，引擎这一侧也没有。
  和 `is_link` 一样：**缺的不是一个标志，是标志加它整条过境路线**，是另一轮的活。

而 `AlertDialog::resolved_semantic_label`（平台相关的“该叫它什么”规则，
苹果上返回 `semanticLabel` 本身、其余平台兜底 "Alert"）
**写好了、测过了、零个活调用者**——第四轮遇到同一个形状。

于是对话框弹出来时，读屏用户拿到的是**一页悄悄变了、焦点跑到了别处、
而没有任何人说过“有个模态打开了”**。

接上游那四个标志里能接的两个，并把**为什么另外两个不接**写进代码：

* `namesRoute` + label：**这两个就是那句announcement**，接上。
* `explicitChildNodes`：在这个走查里**是个空操作**——
  它要求后代保留自己的节点而不是折进来，而这里本来就是这样，
  折叠只在有人明确要求时发生（`RenderMergeSemanticsBox`）。
* `scopesRoute`：如上，没有对应物，写清楚它是什么（告诉平台焦点从此被限制在这棵子树里）。

**苹果那一支是有理由的，不是遗漏。** 上游的 `label` 在苹果上就是 `semanticLabel`
（可以为空），其余平台才兜底。所以一个没写标签的对话框在 iOS 上**谁都不命名这条路由**——
VoiceOver 的焦点本来就落在标题上，再念一句 "Alert" 是多出来的一个词。
测试把这条也钉住了：iOS 上没有标签就没有 `names_route` 节点，
但**调用者自己写的标签仍然照用**。

顺带给对话框留了一个稳定 id（`DIALOG_SEMANTICS_ID`），
并写明理由：**平台拿这个 id 认它的无障碍节点，逐帧变化的 id 等于每帧弹出一个新对话框。**

**变异扫描 5 个，全红**：对话框不说话、不命名路由、只发标志不发标签、
**苹果上也念（于是标题被念两遍）**、平台被写死。

尺子：十六把全部 exit 0。门：Rust 6329 通过、`cargo fmt --check` 干净；
C++ 34 个 gtest 全过；三个输出目录与三个测试二进制全部重建。

第 369 轮那 26 个哑控件，接掉两个（TabBar、AlertDialog），还剩 24 个。

**下一步**：清单上和 `AlertDialog` 同族的 `SimpleDialog` 与 `Dialog` 就在旁边，
而 `SimpleDialog::resolved_semantic_label` 是**同一条规则的第二个副本**
（第 367/368 轮的教训：写两遍的规则会走散）。
所以下一轮**先按行为 diff 这两个 `resolved_semantic_label`**——
...
## 第 372 轮：第三个模态面开口——而它叫 `Dialog`，却不是上游那个 `Dialog`

按上一轮的“下一步”先查 `Dialog` 到底是什么，**不默认它和另外两个同类**。

查下来它**两头都不是我预期的**：

* 它**不是上游的 `Dialog`**。上游那个是**裸容器**——一个形状加一层阴影，
  自己不命名路由，因为包在里面的东西会命名（`AlertDialog` / `SimpleDialog` 各自加）。
* 它**是一个复合体**：自带 title、body、actions；
  grep 全仓库，**没有任何地方把它套在另一个对话框里**。

所以它是一个独立的模态面，该像模态面一样自报家门。
**名字是那个陷阱**：按名字比对会以为拿到一个容器，实际拿到一个对话框——
这个 port 反复撞见的“同名不同物”，写进了 `resolved_semantic_label` 的文档，
免得下次再被重新发现一遍。

接法就是上一轮抽出来的 `announced`，**第三个调用点**——
而 `modal_surface_label` 的文档早就写着“写在这里一次，而不是三遍”，
**它一开始就是按三个调用者写的**，今天补齐了。

兜底词是 **"Dialog" 而不是 "Alert"**：它询问，不打断，
和 `SimpleDialog` 同侧、和 `AlertDialog` 相反。

**变异扫描 4 个，全红**：不宣告、自称 alert、忽略调用者自己的标签、
**苹果也拿兜底词**（7 红——那条平台规则是三个对话框共用的，动它会同时打翻三处）。

尺子：十六把全部 exit 0。门：Rust 6332 通过、`cargo fmt --check` 干净；
C++ 34 个 gtest 全过；三个输出目录与三个测试二进制全部重建。

第 369 轮那 26 个哑控件，接掉四个，还剩 22 个。三个模态面全部就位。

**下一步**：模态这一族做完了，回到那张表按“听不出差别的损失”挑下一个。
队列里 **`BottomSheet`** 是同一族的最后一个（模态面，会抢焦点，
而 `ResolvedBottomSheet` 里已经有 `is_modal` 的概念）——
**先按行为查它有没有 `resolved_semantic_label` 这类规则**：
有就是第四个调用点；**没有的话先查上游 `BottomSheet` 到底命不命名路由**，
因为一个非模态的 sheet 命名路由是错的，这个判断不能照抄前三个。

---

## 第 373 轮：底部弹层的拖拽条第一次能被找到——而“命名路由”这件事它做不了

按上一轮的“下一步”，**先查上游再决定**，结果两条都不是照抄前三个能得到的。

### 一、命名路由不归 `BottomSheet` 管，本项目里也做不到

上游那段 `Semantics(scopesRoute: true, namesRoute: true, label: routeLabel, ...)`
在 **`_ModalBottomSheetState`** 里——**是模态路由干的，不是 sheet 干的**。
非模态的持久 sheet 不命名路由，所以“照搬前三个”会是错的。

而本项目里更彻底：`is_modal` **根本不是 sheet 的字段**，
是 `resolved(context, is_modal)` 的参数，文档写得很清楚——
“同一个 sheet 用两种方式展示会解析成两样，**怎么展示是调用者说了算**”。
`Component::build(&self, context)` 拿不到这个信息，
所以**这个宣告在 `BottomSheet::build` 里根本无处可放**。
跟 `is_link`、`scopesRoute` 同类：**缺的不是一行代码，是那个信息不在这里。**

（顺带核对：`_getRouteLabel` 的平台规则和 `modal_surface_label` 一模一样——
苹果空、其余 `dialogLabel`。所以真接的时候不必再写一遍。）

### 二、拖拽条是 sheet 自己的，而它是哑的

上游给它 `Semantics(label: modalBarrierDismissLabel, container: true, button: true, onTap:)`。
本项目画的是一个 32×4 的圆角小条，**没有任何语义**——
于是读屏用户遇到的是一个什么都不说的小方块，
**而它恰恰是“把这个面板收起来”的唯一入口**。
一句话写进了代码：**一根说“可以拖我”的横条，只对看得见它的人说了这句话。**

标签用的是**屏障那一句**而不是新造一个词：收起来正是它做的事，
上游复用那个字符串，而不是给同一个动作起第二个名字。

### 变异扫描 4 个，全红——其中一个是先证伪、再换掉的

第一版变异是“把保留常量 `DRAG_HANDLE_SEMANTICS_ID` 换成另一个常量”，**绿的**。
**而它绿得对**：真正重要的性质是**逐帧稳定**，两个常量都稳定，
这个变异根本没改到那条性质（跟第 349 轮 `merging.clear()` 同一类）。

所以补了一条测试去钉**那条性质本身**：连推两帧，断言拖拽条的 id 相同。
再把变异换成“改用 `take_text_id()`（每帧新号）”——**转红**。
文档里也写明了这一点：值是任意的，**稳定才是主张**。

尺子：十六把全部 exit 0。门：Rust 6334 通过、`cargo fmt --check` 干净；
C++ 34 个 gtest 全过；三个输出目录与三个测试二进制全部重建。

第 369 轮那 26 个哑控件，接掉五个，还剩 21 个。

**下一步**：模态这一族收尾了，但留下一个**明确的、可以独立成轮的缺口**：
**模态路由的宣告没有归宿**。`is_modal` 只在 `resolved(context, is_modal)` 这条路上，
所以先按行为查**谁在调用它、谁在把 sheet 放进 overlay**
（第 364 轮的 `messenger.rs::snack_bar_slot` 就是这种“每一条都要过的门”）——
**找到那道门就有归宿，找不到就说明这个 port 目前没有模态路由这一层**，
那是另一种大小的结论，也该写清楚。

---

## 第 374 轮：进度条说出“60”——而底部弹层那个宣告，无家可归是因为那条路由不存在

按上一轮的“下一步”找那道门。

**模态这一层是有的**：`theatre::show_modal(overlay, barrier, builder)`，
`dialogs.rs`、`popup.rs` 都从这儿走。
**但没有任何东西把 `BottomSheet` 模态地展示出来**——
全仓库 grep 不到 `show_bottom_sheet` / `show_modal_bottom_sheet` 之类，
`BottomSheet::new` 除了测试没有别的调用者。

所以上一轮那个宣告**无家可归，不是因为找不到地方挂，而是因为那条路由根本不存在**。
这是我上一轮预判的第二种结论，如实记下：
**这个 port 目前没有“模态底部弹层路由”这一层。**
（顺带确认了：`show_modal` 也不该是它的归宿——
三个对话框已经在**控件层**自己宣告了，再在门上宣告一遍就是念两遍，
这正是第 371 轮担心过的那件事。）

### 于是回到清单，按损失挑下一个：`ProgressBar`

它一个字都不说。读屏用户**既不知道有事情在进行，也不知道进行到哪儿**。

上游 `_buildSemanticsWrapper` 给五样：`label`、`value`、`role`、`minValue`、`maxValue`。
这里能接的是前两样，另外三样**没有对应物**——
`SemanticsProperties` 没有 min/max，`RfSemanticsNode` 没有字段，role 同理。
和 `scopesRoute`、`is_link` 一样，是“标志加整条过境路线”的活，写清楚而不是糊过去。

**一个看着像、其实不是的细节，钉成了一条测试**：
进度条的默认值是 `'${(value * 100).round()}'`——**没有百分号**，
而滑块（第 362 轮）是 `'${...}%'`，**有**。
不是遗漏：进度条同时发 `minValue: '0'` / `maxValue: '100'`，
**范围由平台说，单位由平台念**；滑块只交出一个数，那个数得自带单位。
把一个抄进另一个，读屏软件就会念出 "60%%" 或者光秃秃一个 "60"。

**变异扫描 6 个，全红**：不说话、值带上百分号、值发原始小数、
截断而不是四舍五入、标签丢掉、值丢掉。

尺子：十六把全部 exit 0。门：Rust 6336 通过、`cargo fmt --check` 干净；
C++ 34 个 gtest 全过；三个输出目录与三个测试二进制全部重建。

第 369 轮那 26 个哑控件，接掉六个，还剩 20 个。

**下一步**：`Spinner`（`controls.rs:1999`）是 `ProgressBar` 的**不确定态兄弟**，
上游同一个 `_buildSemanticsWrapper` 走另一支：
`role: loadingSpinner`、**没有 value、没有 min/max**，只有 label。
所以它**不是把这一轮抄一遍**——
先按行为确认本项目的 `Spinner` 是不是真的没有进度值
（有的话它就是另一个 ProgressBar，那条 value 分支要走），
**没有的话它该说的就只有一个 label，而“只有 label”恰恰是最容易被写成“什么都不说”的那种**。

---

## 第 375 轮：转圈说出它在等什么——而它的 `value` 绝对不能被念出来

按上一轮的“下一步”先查：本项目的 `Spinner` 有没有进度值。

**有一个 `value` 字段，但它不是进度。** 构造函数自己的文档写着：
“把一个**循环**的 `Controller` 喂给它，就是一个不确定态的转圈。”
所以那个 `value` 是**旋转的相位**。

这一条把整轮的形状定死了。上游 `_buildSemanticsWrapper` 按
`value: double?` 的**空与非空**分两支——空是不确定、转圈，非空是确定、画弧。
**本项目表达不了这个区分**：`value` 永远有值。

于是**确定态那一支在这里不是“还没接”，是不可达**——
这个控件没有任何一种状态表示“完成了 60%”。
要给它一个，就得把 `value` 变成 `Option`，那会动到每一个调用者，是另一轮的活。

**而如果照抄上一轮的进度条**，就会把相位当进度念出来：
读屏软件会依次报 “0”“37”“88”“4”——
**一份关于“无”的进度报告，比沉默更糟**，因为读屏用户会照着它做判断。

所以这一轮接的是：**label**（在等什么）
加上**调用者自己给的 value**（上游在不确定支上也保留 `semanticsValue`——
“第 2 步，共 5 步”，弧照转，这句话照说），
**而永远不从 `value` 推导**。

**变异扫描 5 个，全红**，其中要害那条是
**“把相位当进度念出来”——2 红**。这一条就是这轮存在的理由。
另外“每帧换一个 id”也红（第 373 轮补的那条稳定性测试在这儿又抓了一次）。

尺子：十六把全部 exit 0。门：Rust 6339 通过、`cargo fmt --check` 干净；
C++ 34 个 gtest 全过；三个输出目录与三个测试二进制全部重建。

第 369 轮那 26 个哑控件，接掉七个，还剩 19 个。

**下一步**：这一轮记下了一个**独立成轮的真缺口**：
`Spinner::value` 应该是 `Option<f32>`——空为不确定、非空为确定，
那样确定态那一支（value + min/max + role）才谈得上。
但它**动到每一个调用者**，而且和第 374 轮记的 min/max/role 缺失叠在一起，
先做哪个要按行为定：**先数一遍 `Spinner::new` 到底有几个调用者**。
少就直接改签名做穿；多，就先在清单上挑下一个哑控件
（`ListTile` 287 行、`Badge` 300 行，两个都是内容多、损失大的），
把签名那件事留成一条写清楚的独立任务。

---

## 第 376 轮：列表行说出自己是一行、被选中、可以按——而上一轮那个“下一步”尺寸估错了

先按“下一步”数 `Spinner::new` 的调用者：**6 个**，其中 4 个是上一轮我自己写的测试，
真正的 2 个都在 gallery 的 `progress_indicator_demo.rs`：
...
**demo 自己就在两种用法上各用一次**——一个喂相位、一个喂真进度。
上游那对 determinate/indeterminate，在这里被塞进了同一个 `f32`。

**但我上一轮写的“下一步”把这件事的尺寸估错了。**
我以为改 `value: Option<f32>` 是个签名问题；不是。
`None` 需要**画出点什么**，而上游的不确定态是靠它自己的动画控制器转的，
本项目这个 `Spinner` **里面没有 ticker**——
`ArcSpinner` 的 `sweep = 360 * value` 是**确定态的画法**，
demo 是靠喂一个循环值把它假装成转圈。
所以那不是签名改动，是“不确定态该长什么样”的设计题 + 一处绘制改动。
**这是我第二次把下一步写成了另一种大小的活，如实记下。**

于是按清单挑损失最大的下一个：**`ListTile`**。

上游 `Semantics(button: onTap != null || onLongPress != null, selected:, enabled:)`，
而它一句话都不说：**能按的行被念成一行文字，
而“被选中”的那一行——主从列表里告诉你正在看哪一条的那一行——和其他行完全同音。**

三处分寸：

* **一站，折叠的。** 行里的字来自里面的 `Text`，标题和副标题分开念就是屏幕上一行、
  耳朵里两件事。这正是合并盒子的用处，而第 364 轮那个 `with_properties`
  让标志落在**有字的那个节点**上。
* **`button` 看的是有没有人监听，不是 enabled。** 一个禁用但接了 handler 的行仍然是按钮，
  只是同时说自己被禁用；而一行纯文字被念成按钮，就是在邀请一次什么都不做的点击。
* `has_enabled_state` 跟着 `pressable` 走——不可按的行**没有启用状态**，不是“被禁用”。

**变异扫描 6 个，全红。** 其中两个第一遍是 BUILD ERROR——
**编译不过不是通过**，重写成能编译的等价变异才算数：
“整个包装拿掉”（3 红）、以及**“标志挂在字的旁边而不是字上面”**
（把合并盒子换成外层注解，2 红）——后者钉住的正是“为什么必须用 `with_properties`”。

尺子：十六把全部 exit 0。门：Rust 6342 通过、`cargo fmt --check` 干净；
C++ 34 个 gtest 全过；三个输出目录与三个测试二进制全部重建。

第 369 轮那 26 个哑控件，接掉八个，还剩 18 个。

**下一步**：`Badge`（`components.rs`，300 行）是清单上下一个大的，
而它有一个**别的控件没有的形状**：它是**贴在另一个控件上的一个数字**
（“收件箱，3 条未读”），上游 `Badge` 用的是
`Semantics(container: true, child: ...)` 加上把 label 合进被贴的那个控件。
所以先按行为确认一件事：**本项目的 `Badge` 是包着子控件的，还是并排画的**——
包着就该把数字折进宿主那一站（一件东西说两句话），
并排就说明它在树里根本不和宿主同一个节点，那是另一个问题，先说清楚再动。

---

## 第 377 轮：那张“哑控件”清单是错的——`Badge` 一直在说话

按“下一步”查 `Badge` 是包着宿主还是并排画。**是包着的**（有 `child` 字段），
上游同形。但接着查上游 `badge.dart` 的语义——**一个都没有**，`grep Semantics` 零命中。

于是先跑了个探针，而不是照着清单动手：

```
PROBE ""       PROBE "Inbox"       PROBE "3"
```

**`Badge` 根本不哑。** 它的数字是个 `Text`，走查自己就会给它开节点；
两站而不是一站也正是上游的安排（`Stack` 套住 child，没有 `Semantics` 包装）。
再试 `is_label_visible(false)`：**"3" 消失了，"Inbox" 还在**——也是对的。

### 第 369 轮那张表数错了对象

它数的是“`build` 里提没提 `semantics`”，而那**不是**“读屏用户听不听得见”。
又抽查了三个：`MaterialBanner` → `["Offline"]`、`Scaffold` → `["Body"]`、
`CircleAvatar` → `["AB"]`——**全都在说话**。

清单真正指出的是**“这个控件自己不添加任何东西”**，
而对容器来说那是对的（Scaffold 不该说话，它的 body 说）。
**只有当控件自己有角色或状态、而孩子扛不动的时候，沉默才是缺口。**
所以“还剩 18 个”这个数没有意义，排序也不能照它来。

`Badge` 那两条行为原本没被钉住（隐藏后还念不念，是个细微且要紧的点），补了两条测试。

### 按新的判据挑：`BottomNavigation`

它有孩子扛不动的状态：**哪一个是你正在看的那一页**。
和第 369 轮标签栏一模一样的损失，而它是手机上的主导航。

接完立刻撞上第 367/368 轮那个信号：**变异模式匹配到 2 处**——
标签栏和底部栏的包装**逐字相同**。于是抽成 `one_of_many(region, index, count, active)`，
文档里写明：**两个“N 选一”的条不该在自报家门这件事上走散。**

**变异扫描**：两个调用点各自独立变红（2 红 / 1 红）；
共用规则那三条在 `semantics.rs` 里（第 369 轮已钉），复核后**同时打翻两个条**——
“每个都像选中的”2 红、“位置零基”3 红，失败用例里标签栏和底部栏都在。
（另有三条变异因为选择串对不上格式被跳过；**跳过不是通过**，改从规则那一侧验证才算数。）

尺子：十六把全部 exit 0。门：Rust 6348 通过、`cargo fmt --check` 干净；
C++ 34 个 gtest 全过；三个输出目录与三个测试二进制全部重建。

**下一步**：清单要重做，但**不要重跑一遍 grep**——
按这一轮的办法，**逐个挂起来跑一次走查**，看它实际发出什么。
先做一件小事：把这一轮那个一次性探针**变成一个非门控的清单脚本**
（像 `tools/descent.py` 那样 always exit 0），
输出“每个控件实际说了什么”，人来判断哪些缺角色/状态。
**先确认它对 `Badge`、`Scaffold` 这三个已知案例给出正确答案，再拿它去看别的**——
第 358 轮的教训：先有已知样本，才谈得上校准。

---

## 第 378 轮：一张“它到底说了什么”的清单，第一次跑就抓到了导航栏

按上一轮的“下一步”重做清单：**不再 grep，而是把控件挂起来跑一次真走查，打印它实际发出什么。**

产物两件：`controls.rs` 里一个 `spoken_census` 测试（**不断言、不会失败**，
和 `tools/descent.py` 同一个形状），加 `tools/spoken.py` 把它跑出来。
两处文档都写清了它为什么数这个：**“提到 semantics”和“读屏用户听得见”是两个问题**，
第 369 轮问的是第一个，第 377 轮在 `Badge` 上发现那答不了第二个。

**先校准，再用。** 三个已知答案全对：
`Badge` 两条、`Scaffold` 说 "Body"、`CircleAvatar` 说 "AB"。

### 第一次跑就抓到一条

```
TabBar           -> "Tab 1 of 2\nHome" +flags +actions, "Tab 2 of 2\nYou" +flags +actions
BottomNavigation -> "Tab 1 of 2\nH\nHome" +flags +actions, ...
NavigationRail   -> "H", "Home", "S", "Saved"
```

**四个光秃秃的站。** 第三个“N 选一”的条，也是唯一没接上 `one_of_many` 的那个——
正是第 367/368 轮警告过的走散，被新仪器第一次运行就照出来了。
接上之后三个条**逐字一致**，并补了一条把三个一起钉住的测试。

### 报告自己也有个盲区，当场补掉

第一版的过滤只看 label／flags／actions，于是 `ProgressBar`（只有 value）显示成空白——
**一个把“会说话”漏报成“沉默”的报告，正是它要取代的那种表**。
加上 value 之后它显示 `"" ="60"`。

（顺带：第一版生成器把同一段逻辑复制了 19 份——
**改一处要改十九处**，正是这些轮反复在抓的味道。重写成一个 `spoken_by` 辅助函数。）

### 变异扫描 3 个，第一遍 2 个红，1 个真窟窿

**“导航栏从另一头数”**（`count - index - 1`）全绿——
我那条测试只断言了**哪一个是选中的**，没断言**位置**。
于是一个把选项倒着编号的条可以完全不被发现。
断言改成 `(位置, 选中)` 成对之后转红。

尺子：十六把全部 exit 0；`tools/spoken.py` exit 0（它不是尺子，永不失败）。
门：Rust 6350 通过、`cargo fmt --check` 干净；C++ 34 个 gtest 全过；
三个输出目录与三个测试二进制全部重建。

**下一步**：清单现在有 18 行，**按“有没有孩子扛不动的角色或状态”逐行读**，
候选已经很清楚：`Snackbar` 只说 "Saved"（**它是自己冒出来的**，
第 364 轮已经给 messenger 那道门接了 live region，
但**这个控件自己**被直接放进树里时不会——先查它有没有第二条路径）；
...
## 第 380 轮：两个横幅开口——而清单当场抓到一句话被念了两遍

按“下一步”先读上游再决定。`banner.dart:443`：

```dart
materialBanner = Semantics(container: true, liveRegion: true, onDismiss: ...);
```

**给。** 我上一轮的怀疑（“不会自己消失的东西该不该打断”）是值得问的，
但答案是给——而理由跨过了那个差别：
**一样东西自己冒出来，就值得告诉别人，不管它会不会自己走。**
反过来猜的话，这里会留下一个哑横幅。这条判断写进了共用函数的文档。

（顺带记下两件事：上游在读屏开启时**跳过滑入动画**
（`accessibleNavigation ? banner : SlideTransition(...)`）——
本项目 `media_query` 里**没有 `accessible_navigation` 这个来源**，所以接不了；
以及和 `Dialog` 一样的同名陷阱：`controls::Banner` 是本项目自己的简单形状，
`components::MaterialBanner` 才是上游那个完整的，**两个都得说话**。）

因为这条规则马上要写第三、第四遍，**在造出重复之前**先抽成
`semantics::announces_itself`，提示条也改用它。

### 清单第一次跑就抓到一个真 bug

加上 `MaterialBanner` 那一行之后：

```
MaterialBanner -> "You are offline\nYou are offline"
```

**同一句话念两遍。** 追下去是第 369 轮那个修法留下的：
`Label` = 一个带字的注解 + 底下一个 `Text`，平时 `Text` 会让位给上面的注解；
可**在合并里注解会被折进去、`open` 提前返回**，于是没人抬 `labelled_depth`，
底下那个 `Text` 就又说了一遍。

第 369 轮我是用“合并里干脆不抑制”解决的——**太宽了**。
正确的规则是本来就该有的那一条：**有字的东西替它底下的东西说话，
不管它自己成不成节点。** 折叠时在这里抬深度、用 `OnDrop` 在子树走完后落回。

顺带修掉一个潜伏的不对称：`close` 原本靠**当时**的标签判断要不要落深度，
而合并节点的标签**是会长的**。改成开的时候记一笔（`labelled: Vec<bool>`），关的时候照记的还。

### 变异扫描 7 个，第一遍 4 红、3 个绿

* **两个横幅“不说话”都绿**——我接了却**没写测试**，清单只是显示、不是断言。补了两条，转红。
* “close 还了它没借的”仍然绿，**而且是等价的**：那条路径上深度已经是 0，
  `saturating_sub` 吃掉了差别。和第 349 轮 `merging.clear()` 同类，如实记账。

尺子：十六把全部 exit 0。门：Rust 6354 通过、`cargo fmt --check` 干净；
C++ 34 个 gtest 全过；三个输出目录与三个测试二进制全部重建。

**下一步**：这一轮的教训要立刻用一次——**接完就去看清单那一行**，
因为 `MaterialBanner` 那个双读正是这样冒出来的。
清单上还没查的：`DataTable -> "Name", "Size"`（表头是**列的名字**，
上游 `DataTable` 有 `sortColumnIndex` / `onSort`，
...
## 第 385 轮：标签条两层都开口——而另外两个条一个字都不该说

上一轮的“下一步”提醒了一件事：**本项目把三个条写进了一个函数，
这对上游那三个类没有任何证明力。** 逐个文件查，结果正好分成两半：

| 上游文件 | role |
|---|---|
| `tabs.dart:2090/2112` | `tab` + `tabBar`（还有 2376 的 `tabPanel`） |
| `navigation_bar.dart:294/305` | `tabBar` + `tab` |
| `navigation_rail.dart` | **一个都没有** |
| `bottom_navigation_bar.dart` | **一个都没有** |

于是 `one_of_many` 多了一个 `role` 参数，
外层新增 `a_bar_of_choices`——两层都给或者都不给：
**读屏用户听到"第 2 个，共 5 个"，却没有任何东西说这 5 个是什么，
那句话只讲了一半。**

### 外层必须是注解，不能是折叠

这几轮一直在把注解换成折叠（第 381、382、383 轮都是），
这里是**唯一反过来的一次**：折叠会把整条 bar 收成一站，
标签就不再是可以逐个走过去的东西了——那正好是标签条的反面。
里面每个标签仍然折叠，因为一个标签是"一个记号加一个词"合起来的**一样**东西。

### `BottomNavigation` 差点接错——证据在函数体里，不在名字上

它用 `Destination`，读起来像 Material 3 的 `NavigationBar`（那个**是**给 role 的）。
但它的高度是 `kBottomNavigationBarHeight`，
里面的注释引的是 `_BottomNavigationTile.build`——**两处都是 Material 2 的 `BottomNavigationBar`**，
那个文件一个 role 都不给。所以它也不给。

### 变异扫描 6 个，第一遍 4 红、2 绿

* “没有 kind 的条也照样包一层”**绿**——因为那条 `if !role.is_set()` 的早返回
  **根本到不了**：当时只有 `TabBar` 调这个包装函数。
  我为一个不会发生的情况写了一道守卫。
  改法不是删掉它，而是**让三个条都走同一个包装**——
  内层本来就共用了，外层也该共用，这样"不给 role"才是这个条**做的决定**，
  而不是一行没人写的代码。
* 另一条绿是因为我的辅助函数**把空标签的节点滤掉了**——
  而 tabBar 那个节点正是"有 kind、没有词"。
  两处都改：过滤条件加上 role，另加一个**完全不过滤**的 `stop_count`，
  因为"一个没词也没 kind 的节点"恰恰是所有过滤器都看不见、
  而读屏用户会站上去听到一片安静的东西。

第二遍**六个全红**。

尺子：十六把全部 exit 0。门：Rust 6377 通过、`cargo fmt --check` 干净；
C++ 34 个 gtest 全过；三个输出目录与三个测试二进制全部重建。
清单：`TabBar -> "" role=TabBar, "Tab 1 of 2\nHome" role=Tab ...`，
另外两个条逐字未变。

**下一步**：`tabs.dart:2376` 还有第三层 —— `TabBarView` 的每一页是
`Semantics(role: SemanticsRole.tabPanel)`。**先查本项目有没有 `TabBarView`**：
...
## 第 388 轮：模态终于关得住读屏用户——以及一个"谁都没在看"的位

按“下一步”查 `dropdown`：**`dropdown.rs` 里一个 `Component` 都没有**，
`DropdownMenuItem` 是纯数据（`child` 是个 id，不是 widget）。
顺手把最后一个候选也查了：`radio_group.rs` 的 `RadioGroup` 同样只有逻辑、没有 build。
**上游 role 的 23 处，本项目有对应控件的至此全部接完**——这是个可核对的结论，不是感觉。

于是改接第 384 轮**自己写下并推迟**的那一条：`scopesRoute`。

### 那条笔记有一半是错的

第 384 轮写的是"没有字段、没有位、**engine 那边也什么都没有**"。
最后半句错了：engine 一直带着 `SemanticsFlags::scopesRoute`
（`lib/ui/semantics/semantics_flags.h:37`），**缺的全在本项目这一侧**。
笔记改正了，并把"为什么值得留着"一起写进去：
**一条把缺口说得比实际更大的笔记，正是一轮的活被无限期推迟的方式。**

`names_route` 说的是"路由到了该念什么"，`scopes_route` 说的是"读屏用户现在被关在哪"。
没有它，读屏用户从对话框最后一个控件继续往下划，就**走回了对话框本该挡住的页面**，
然后在一个根本不听的界面上操作。

一路接穿：flag、`1 << 26`、`rust_app_api.h`、`runtime_controller.cc`、
以及 `SemanticsFlags::merge`。接在 `announced` 里——
**跟着 label 走，不是独立成立**：上游 `dialog.dart:935/1365` 把 `scopesRoute`
放在 `if (label != null)` **里面**，紧挨着 `namesRoute`；
而同一个函数里的 role 是无条件的。**查过，不是照猜。**

### 变异扫描 5 个，第一遍 3 红、2 绿，两个都不是小事

**“位撞到邻居身上”绿了。** 把 `SCOPES_ROUTE` 改成 `1 << 24`（`NAMES_ROUTE` 的位），
**整道门全绿**：Rust 测试读的是它自己写下的那个常量（循环），
而 C++ 那个 FFI 测试**是用 C++ 常量自己造结构体**的，根本不碰这 27 个 Rust 常量。
去实测了一遍才敢这么说（第 383 轮的教训：没亲眼看它抓过错的仪器，只给数字不给事实）。
也就是说：**27 个位常量里任何一个写错幂次，都没有任何东西会发现**——
错的位不会"没传过去"，它会**冒充邻居传过去**。
补了一条 `no_two_flag_bits_are_the_same_bit`：两两不同 + 每个只有一位。
这不是全部契约（和头文件逐条对上才是），但**是能从这一侧检查的那一半，
也是失手时实际会犯的那一半**。

**第二条绿，把一个更大的缺口照了出来。** 我给"折叠后仍然关得住"写了断言——**它失败了**。
原因不在这个 flag：**walk 的折叠（`open` 的 merging 分支）会把标签、tooltip、role
从被折叠的后代带上来，flags 一个都不带**——所以折进去的按钮也不再说自己是按钮。
上游是并集（`flags = flags.merge(node._flags)`），
而本项目**确实有这条规则**，写在 `SemanticsConfiguration::absorb` 上——
**正是那条 walk 从不经过的路径**。规则在、生产者不在，这是第三次。
断言撤掉、把发现如实写进代码注释和这里，**没有顺手扩大这一轮**：
给每次折叠加上 flags 并集会改变相当多现有站点说的话，那是它自己的一轮。

尺子：十六把全部 exit 0。门：Rust 6384 通过、`cargo fmt --check` 干净；
C++ 34 个 gtest 全过（两侧 FFI 测试都加了这个位，一端置位一端不置）；
三个输出目录与三个测试二进制全部重建。

**下一步**：就做上面照出来的那个——**折叠时把后代的 flags 并上来**。
先跑一遍清单存档当前 20 行，**改完逐行对比**：
预期会变的是"折叠里本来就该带 flag"的那些（`Section` 里的 `ListTile`、
菜单条目下面的东西），**任何一行出乎意料的变化都要先解释再接受**。
...
## 第 394 轮：底栏终于画出来了——缺口两端都在，中间没有人画

按“下一步”先查两件事，两件都查对了：

* `bottom_bars::BottomAppBar` 有 `shape` 和 `notch_margin`，
  `ResolvedBottomAppBar::cuts_a_notch(has_fab)` 也在；
* `borders::NotchedShape::outer_path(host, guest)` **会画那条带缺口的轮廓**，
  而且有测试。

**两端都齐，中间空着**：没有任何东西画一条底栏，
所以那个缺口谁也看不见。这就是这一轮。

### 用渲染对象，不用"上一帧报回来的尺寸"

轮廓的路径**取决于底栏自己的尺寸**，而尺寸要到布局才知道。
`ink_well` 那边的水波纹只能用"上一帧报回来的尺寸"，代价是一帧画错；
一个已经布局完再绘制的对象，**要用的时候尺寸就在手上**。
所以写了个小的 `NotchedSurface`：布局交给孩子，绘制时用 `self.size` 算路径。

### 停靠位置由调用者给，而这正是上游的形状

上游的底栏也**不自己算** FAB 在哪：它从
`Scaffold.geometryOf(context)` **读**，再交给 `_BottomAppBarClipper`。
本项目 `fab_location.rs` 会算位置，但**没有东西发布结果**——那条通道没移植。
所以 `docked_at(rect)` 是那道缝：通道landing 之后是**填进来**，不是替换掉。

### 变异扫描 7 个，第一遍 3 红、4 绿，而四个绿都指向同一件事

**画布桩记的是路径的包围盒，而缺口是往里凹的**——
所以"边距没留"和"洞开在别处"这两条，
**在任何看绘制调用的测试里都长得一模一样**（永远是 `(0, 0, 400, 80)`）。
我第一版的边距测试正是这样：跑通了，什么也没量到。

改法和第 393 轮同一个手法：**把画布看不见的那一步命名出来**——
`BottomAppBar::notch_rect()`（按钮矩形向外扩 margin，
上游 `_BottomAppBarClipper.getClip` 做的同一件事）和
`NotchedSurface::guest_at(offset)`（把 bar 坐标系里的洞挪到画布上）。
两个都不是"测实现"，它们和 `cuts_a_notch` 一样，
是**这条几何链上有名字的中间量**。

另外两条绿是测试太弱：底栏没孩子（画不出孩子也看不出来）、
没断言高度来自主题。以及一个我自己的错：
**布局给的是 loose**，于是底栏收缩到 32 像素宽，
而停靠矩形在 x=180——根本不在栏上，两种边距当然画得一样。
改成**宽度 tight**，也就是 scaffold 实际给底栏的约束。

第二遍**七个全红**。第七条（"洞开在别处"）第一次还是绿的：
我只往下移了 bar，而变异清的是**横向**偏移——**一个轴不测等于半条线没测**，
两个轴都断言之后转红。

### 报告自己也漏了一次，同一类

`shells.py` 仍然列着 `BottomAppBar`——因为我写的是
`impl crate::framework::Component for`，**带路径**，
而它的模式只认光秃秃的 `Component`。
和 `ClipOval` 当初那次（返回类型带路径）是同一类漏报，一并修了。
校准仍然通过：4 个已知壳子在表里，14 个已知会 build 的一个都没误列。
151 个 build（原 147）。

尺子：十六把全部 exit 0。门：Rust 6402 通过、`cargo fmt --check` 干净；
C++ 34 个 gtest 全过；gallery 354 通过；三个输出目录与三个测试二进制全部重建。

**下一步**：把那条缺失的通道补上——**scaffold 把 FAB 的矩形交给底栏**。
先查本项目 `Scaffold` 现在怎么摆 FAB：是不是已经用了
`fab_location.rs` 算出的偏移；如果是，那 rect 已经在手里，
...
## 第 396 轮：Scaffold 有了底栏槽位——而"键盘加底栏"是**取大**，不是相加

按“下一步”补底栏槽位。上游 `_ScaffoldLayout` 的顺序是：
**先量底栏**（全宽），把高度记进 `bottomWidgetsHeight`，
放在 `bottom - bottomWidgetsHeight`，body 拿上面剩下的。

于是上一轮那个 `FloatingButtonOver` 扩成 `ScaffoldFloor`，
一个对象同时管三样：页面、底栏、按钮——
**这正是上游的安排**（一个 layout 同时知道两者），
而不是把底栏塞进 column、再想办法把高度传出来。
...
**取大，不是相加。** 键盘是**盖在**底栏上的，不是摞上去的；
按二者之和去避让，浮动按钮会浮在键盘上方"一个底栏那么高"的位置，
中间空一条。这条被写进注释，也被单独测了一条。

三样东西的顺序（画、访问、命中）抽成了一个 `visit_floor`：
**画和命中如果差一个底栏的高度，就是一个"答在自己没画的地方"的控件。**

### 变异扫描 7 个，第一遍 4 红、3 绿，三个都是真的

* **键盘和底栏相加**——没有任何测试同时给了两者。
* **底栏不满宽**——上游用 `fullWidthConstraints`；给松的最小宽度它就缩到内容那么宽，
  旁边露出一条页面。**一条不到两边的栏就不是栏。**
* **手指落在底栏上却到了页面**——命中测试完全没测。

三条补齐，第二遍**七个全红**。

尺子：十六把全部 exit 0。门：Rust 6412 通过、`cargo fmt --check` 干净；
C++ 34 个 gtest 全过；gallery 354 通过；三个输出目录与三个测试二进制全部重建。

**下一步**：三样东西现在在**同一个对象**里了，第 394 轮备下的那道缝终于两端齐全——
`ScaffoldFloor` 量完按钮和底栏之后，**把按钮的矩形交给底栏**，
让 `BottomAppBar::docked_at` 有人调用，缺口自己就出现了。
但**先查一件事**：`docked_at` 是 `BottomAppBar` 上的**构建期**方法，
而矩形要到**布局期**才知道——这两个时机对不上。
所以先确认怎么把布局期的值送进一个已经建好的子树：
...
## 第 400 轮：选中的目的地要写得更大——而"没有任何测试能说出文字画多大"

按“下一步”查回退值，**查出来的比预想的严重**：
上游的 `selectedFontSize: 14` / `unselectedFontSize: 12` 是**控件自己的默认值**
（不是主题的），本项目**两个都没移植**，而这条底栏把所有标签写死 `11.0`。

也就是说：**11 既不是 14 也不是 12，而且选中与未选中一样大。**
一条固定底栏本来用**两种方式**告诉你在哪一页——颜色和字号——
只画一种，等于只答了一半。

三样一起补：两个字号常量、解析器带上它们、控件改用解析出的 style。

### 上游那条规则在这里问不出来

`_effectiveTextStyle` 是 `textStyle.fontSize == null ? copyWith(fontSize:) : textStyle`。
**本项目的 `TextStyle.font_size` 不是 `Option`**——每个 style 都带着一个尺寸，
分不出"特意设的"和"默认就有的"。
所以规则落到这一侧唯一存在的"未设"信号上：**style 那个 `Option`**——
没给 style 就用字号，给了就整份采用。
代价写在文档里：一个只想设字体族、把字号留给 `selectedFontSize` 的主题，
**在这里说不出来**。那是 `TextStyle` 少一个 `Option`，不是这里的决定。

顺带补了 `BottomNavigationBarThemeData::with_label_styles`——
那两个字段**根本没有 setter**，解析器读的东西永远只能是 `None`。

### 桩子里也有一个"量不到"

写测试时发现：**画布桩记了段落的颜色，没记字号**——
于是"这条底栏所有标签一样大"这件事，**crate 里没有任何测试能说出来**。
而桩子一直有那个数（`StubParagraph.font_size` 就是用它量每一行的），只是丢掉了。
补上，理由和它旁边 `argb` 那段注释一模一样
（tick 176 之前"没有任何测试能说出文字是什么颜色"）。

### 变异扫描 5 个，第一遍 1 红、2 绿 + 2 个 MISS

* **“两个字号互换”绿**——我的断言用的是**那两个常量本身**，
  互换之后两边一起换，永远相等。**改成写死 12/14**：
  拿被测的常量去断言被测的常量，是循环。
* **“给了 style 却被字号盖掉”绿**——没有测试装过主题 style（因为没有 setter）。

第二遍**五个全红**。

尺子：十六把全部 exit 0。门：Rust 6423 通过、`cargo fmt --check` 干净；
C++ 34 个 gtest 全过；gallery 354 通过；三个输出目录与三个测试二进制全部重建。

**下一步**：这条线索只剩 `selected_icon_theme` / `unselected_icon_theme`，
而**它们接不了**——本项目没有图标字体（`Destination.mark` 是一两个字符），
`IconThemeData` 的 size/opacity 没有东西可以套。**先把这条写进代码注释**
（"规则在、宿主未到"，和 `to_properties` 那两处同样的记法），
...
## 第 403 轮：列表的每一行终于说得出自己是第几行

按“下一步”查 `IndexedSemantics` 有没有生产者。**没有**——
`RenderIndexedSemanticsBox` 只有自己的测试在用。

上游的生产者很明确：`SliverChildBuilderDelegate` 默认给每个孩子套一层
`IndexedSemantics`（`addSemanticIndexes`），**和它更出名的 `addRepaintBoundaries`
写在同一个构造器里、同样默认为真**。
而本项目的 `LazyList` **只套了重绘边界那一层**——
那行代码旁边的注释甚至引了同一个 delegate。

于是下面整条链都在、就差最上面这一行：
`RenderIndexedSemanticsBox` 在、走查的 `pending_index` 在、
`RfSemanticsNode::scroll_index` 第 383 轮就已经过 FFI 了。
补上之后，走查里每一行都带着自己的序号。

**这个位置无法在下游补救**：走查跑起来的时候，滚出视野的行已经不在了，
数还活着的那些，会把**第一个可见行编成 0**。

### 但它只答了半句，而这半句是被断言下来的

跑完之后去看了实际产出：**整棵树里没有任何节点被标为可滚动**——
`RenderSliverViewport`（`LazyList` 用的那个）**根本没有 `describe_semantics`**，
只有盒子版的 `RenderViewport` 有。
所以这些序号还没有一个"可滚动节点"可以汇总上去，平台仍然听不到"第 3 项，共 40 项"。

**没有含糊过去**：测试里把这件事**断言**成当前事实，
并写明"哪天那个注解落地，这条测试会指出是哪一半动了"。

### 变异扫描 3 个，全红——但第三条第一次是绿的

“序号从第一个可见行开始数”一开始没红：**我的测试停在偏移 0**，
那里"在列表中的位置"和"在幸存行中的位置"是同一个数字。
**一条只在静止时检查的列表，分不出绝对序号和相对序号。**
把测试改成滚动到 500 之后转红。

尺子：十六把全部 exit 0。门：Rust 6429 通过、`cargo fmt --check` 干净；
C++ 34 个 gtest 全过；gallery 354 通过；三个输出目录与三个测试二进制全部重建。

**下一步**：就补上面那半句——**给 `RenderSliverViewport` 一个 `describe_semantics`**。
盒子版 `RenderViewport` 的那一段可以照抄形状（位置、min、max、方向），
但**有两处必须先查**：
一是 `semanticChildCount`——盒子版传的是 `None` 并注明"这个视口只有一个孩子"，
而 `LazyList` **知道自己有几项**，那个数得从列表传到视口，先看有没有路；
二是 `min` 那个字面 0——盒子版注释说是因为它自己的算术假定如此，
**sliver 视口是不是同一个假定要重新确认，不能顺手抄。**

---

## 第 404 轮：sliver 视口终于说自己是一个列表——顺带纠正我上一轮写错的原因

按“下一步”给 `RenderSliverViewport` 补 `describe_semantics`，两个数**先查后写**：

* **最小值那个字面 0**：sliver 视口的偏移同样跑在 `0..=max_scroll_extent()`，
  被它自己布局算出的 `content_scroll_extent` 夹住——和盒子版同一个假定，
  **确认过，不是顺手抄的**；
* **项数**：盒子版只能给 `None`（它只有一个任意大小的孩子），
  而 sliver 视口的 `children` 是**熬过窗口的那些**——
  数它们等于拿"屏幕上有几个"去回答"整个列表有多长"。
  所以数字从知道它的人那里来：`SliverListView` 的 `child_count`。

### 上一轮那句话的原因是错的

第 403 轮我写的是"`LazyList` 没有可滚动节点，因为 sliver 视口没有 `describe_semantics`"。
补完之后回去看，**`LazyList` 那棵树里仍然没有可滚动节点**——
因为 `LazyList` **根本不是视口**：它建一列，视口是调用者套上去的。
那不是缺口，是边界。注释和断言都改了，并写明**当初错在哪、怎么发现的**。

### 变异扫描 4 个，第一遍 3 红，而那个绿的是真 bug

**“列表从不说自己多长”绿**——追下去发现：**`update_from` 没有接这个字段**。
也就是说，滚动一次（列表每次滚动都重建，走的是 `update_from` 那条路而不是新建）
之后，视口会**一直沿用第一次的项数**：一个从 40 项变成 3 项的列表，
会继续宣告"共 40 项"。和第 397 轮 `RenderMergeSemanticsBox` 同一个毛病。

接上之后转红，四个全红。**并且刻意不把它算进 `changed`**：
项数是"告诉读屏用户的话"，不是量出来的尺寸，只改它不必重新布局。

测试也跟着改成**先重建再读**——只在"出生时"看一眼，两条路径长得一模一样。

尺子：十六把全部 exit 0。门：Rust 6430 通过、`cargo fmt --check` 干净；
C++ 34 个 gtest 全过；gallery 354 通过；三个输出目录与三个测试二进制全部重建。

**下一步**：`LazyList` 那一半仍然没人接——它建的那一列，
外面套的是**盒子** `RenderViewport`，而那个视口的 `semantic_child_count` 现在还是写死 `None`
（注释说"这个视口只有一个孩子，没有数目可报"）。
**先查一件事**：`LazyList` 是不是总被同一个地方套进 `RenderViewport`
（`ScrollView`？`scrolling.rs` 里的哪个控件？）——
如果是，那和这一轮一样，把 `child_count` 从列表传到那个视口即可；
**如果 LazyList 可以被任意人套进任意视口**，那这条路就不通，
得换成"列表自己带一个可滚动注解"，那是另一种做法、另一种大小。

---

## 第 405 轮：gallery 真正在滚的那个列表，终于报得出"共几行、这是第几行"

按“下一步”查 `LazyList` 是不是总被同一处套进视口。**答案是：它没有任何调用者**——
`lib.rs` 导出了它，crate 和 gallery 里一个用处都没有，只有自己的测试。
盒子 `RenderViewport` 同样只在测试里被构造过。

**所以那条路根本不存在**，而查这件事时露出了真正该做的那个：
gallery 滚的是 `widgets::ListView`（`app.rs:912`），
它内部合成一个 `RenderViewport`——**这才是有真实用户的那条路**，
而它此前：行没有序号、视口不报项数。

上游 `ListView(children:)` 用的是 `SliverChildListDelegate`，
`addSemanticIndexes` 默认开、`semanticChildCount` 就是 `children.length`。三处都接上了。

### 而真正挡住这件事的是第三样东西

接完之后测试仍然报"没有节点说自己是列表"。原因：
**`ListView::visit_children` 交给走查的是那一列的孩子，不是视口本身**，
所以视口的注解永远没人问。
而它旁边的 `describe_approximate_paint_clip` **早就为同一个理由转发了**——
注释写着"裁剪列的是那扇窗，而窗在里面一层"。
`describe_semantics` 照同样的理由转发，答案立刻就出来了。

也就是说：一个 `ListView` 此前到达读屏用户那里是**一个普通盒子**——
不说自己会滚、不说滚到哪、不说有多少行、也不提供滚动手势，
而里面那个视口四个答案全都备着。

### 变异扫描 5 个，第一遍 3 红、2 绿——而两个绿是同一件事

两条都在**重建路径**上。照第 404 轮的做法把测试改成"先用不同长度重建再读"，
**仍然绿**：这一次的重建**换掉了整个渲染对象**而不是更新它，
所以那条 `update_from` 分支根本没跑到。

**没有假装覆盖到**：把这件事写进代码注释——两行都按下面那条布局路径的样子写，
而"重建后的 `ListView` 到底是被更新还是被替换"是另一个问题，
不是这两行的问题。

尺子：十六把全部 exit 0。门：Rust 6431 通过、`cargo fmt --check` 干净；
C++ 34 个 gtest 全过；gallery 354 通过；三个输出目录与三个测试二进制全部重建。

**下一步**：就查上面那个悬着的问题——**重建一个 `ListView` 时它是被更新还是被替换**。
这不是"给它加个测试"，是先弄清楚事实：
`leaf(|| ListView::new()...)` 每帧都造一个新的 `ListView`，
而 `update_from` 里那一大段（`flex.reconfigure`、`composed.update_from`）
是**为"被更新"写的**。如果实际上总是被替换，那一段就是死码，
gallery 每一帧都在重建整棵列表子树——**那是个性能事实，不是语义事实**，
比这一轮的注释要紧得多。先量，再决定。

---

## 第 406 轮：先去量那件悬着的事——量出来的第一件事是**我的探针在量别的东西**

上一轮留了个问题：重建一个 `ListView`，渲染对象是被更新还是被替换？
写了个探针比对两次 `build_render_tree()` 的返回值：`same_object=true`，
又在 `update_from` 里打点，确认**走到了 staged 那条路**。
看上去结论清楚：更新路径是活的。

**然后一个直接实验把这个结论推翻了。** 把 staged 那行的项数改成 `None`，
再把探针放进视口的 `describe_semantics`——它仍然报 `Some(4)`。
也就是说：**走查读到的那个数根本不是 staged 那条路写的**。

回头看那个 `same_object=true`：我比的是**整棵树的根**，
而不是根里面那个 `ListView`。**根一样，说明不了里面一样。**
和第 384 轮那条"量了一个不走这段代码的控件"是同一族的错误——
这一次是量了一个不回答这个问题的对象。

### 所以这一轮的活变成了：把第二个写入者删掉

第 405 轮把项数穿进视口，穿了**两处**（布局路径和更新路径）。
实测下来，**更新路径那处从来没被读到过**——
"两个写入者、其中一个永远读不到"正是这些轮反复在拆的形状。

改成**只有一个来源**：`ListView::describe_semantics` 拿到视口的注解之后，
自己把 `scroll_child_count` 填成 `self.children.len()`。
它不可能过期，因为**它就是那些孩子**。
盒子视口那个字段随之删掉（连 builder 和 `update_from` 里那行），
`describe_semantics` 回到诚实的 `None`——
"这个对象被交了一个任意大小的孩子，从没人告诉它里面装了什么"。

变异扫描 4 个，**第一遍全红**（其中"项数改成 1"这条，
正是"数视口留下的那些"会得到的答案）。

尺子：十六把全部 exit 0。门：Rust 6431 通过、`cargo fmt --check` 干净；
C++ 34 个 gtest 全过；gallery 354 通过；三个输出目录与三个测试二进制全部重建。

**下一步**：上一轮那个问题**仍然没有答案**，而且现在知道它不能用"比根"来回答。
但它已经不挡路了——项数只有一个来源，无所谓对象是被更新还是被替换。
所以**先放下它**，回 `shells.py` 的表挑下一个（表头附近还有
