...
**Windows**:框架 399 个测试、FFI 17 个、flutter_gallery 21 个、相册 60 个
...
五个示例的无头渲染在这三件事之后仍然**逐像素相同**,在第二十一条之后
也仍然**逐字节相同**(同一批 `--png`,改动前后各渲染一次);UI Automation
客户端在第二十一条之后重跑:树、`Invoke`(0 → 1 taps)、`Toggle`(Off → On)、
`FromPoint` 都照旧,text_demo 的 `SetFocus` 之后用真键盘打进 `rustflutter`
并从 Value 模式读回,退格三次读回 `rustflut`。

**Android**(模拟器,Android 14,x86_64):11 个 APK 全部安装启动;九个示例的
截图在第二十一条之后与之前**逐像素相同**(状态栏以下),更早八个与持久化改动前
也是;showcase 的
...
第二十一条之后重跑了这一条,因为惰性列表的每一行现在都在一个边界下面,而边界
交回旧层就意味着读屏软件看不见那一行;
...
- **还没回答 `update_from` 的那几个**。`RenderClipPath` 是故意的(见第二十一
  条);`Container` 和 `ListView` 是两个自己组装子树的渲染对象,把新配置塞进
  一棵已经搭好的子树,比换一个新的更容易出错,所以它们还答 `None`。代价是
  这两个下面的子树每次重建都重做,而它们都不在惰性列表的行里。
