...
## 下一步:`Let`(2525,新队头)——还原 `??` 和 `?.`

已经量清楚了:77% 是 `a ?? b`(6764)和基于 null 测试的条件式(4838,多半 `?.`)。

**先做 `??`**,它占最大且形状最固定:

    let final T #0 = a in #0 == null ? b : #0

要识别的正是这个模式:一个临时变量,body 是以 `#0 == null` 为条件的三元,
且 else 分支就是那个临时变量本身。识别出来就还原成 `IrBinary('??', a, b)`——
后端已经会译 `??`(`a.unwrap_or(b)`),第 2 轮就有了。

**要当心一件事**:`a` 可能有副作用,而 `unwrap_or(b)` 会**无条件求值 `b`**。
Dart 的 `??` 是短路的。如果 `b` 有副作用或代价大,`unwrap_or` 是错的,
该用 `unwrap_or_else(|| b)`。**先量 `b` 里有多少是纯字面量**,再决定是
一律用 `unwrap_or_else` 还是分情况。这正是第 11 轮
`unwrap_or_default` 那类"能编译但语义悄悄不对"的陷阱。

之后 `?.`:Rust 是 `a.map(|x| x.b)` 或 `a.as_ref().map(...)`,
但要看清 CFE 那 4838 个的确切形状再动手。

## 当前队头

**Kernel(发布路径,664 库 / 3665 类 / 12151 次拒绝)**

| 次数 | 要建的东西 |
|---|---|
| 2525 | CFE 的 `Let`(77% 是 `??` 和 `?.`) |
| 1443 | super 构造调用带参数 |
| 908 | 闭包字面量 |
| 559 | 调用函数值 |
| 395 | `BlockExpression` |
...
1. **"零拒绝"(Kernel 765 / 3665)不等于"翻译好了"**:引用其他库的东西仍靠手写桩。
2. **`testdata` 的桩已经吃力**。真正的答案是翻译 `dart:core` / `dart:ui` 的最小子集。
3. **哪把仪器在守哪条守卫,要记清楚。** 有些只有断言抓得到
   (第 11 轮 `unwrap_or_default`),有些只有前端对照抓得到
   (第 18 轮 `null ==` 左值),有些只有 rustc 抓得到(第 12 轮可变性)。
   变异存活时,先问的是"该用哪把仪器",而不是"是不是等价"。
