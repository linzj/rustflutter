...
| | |
|---|---|
| `const instance of X, which is not in this file` | 484 |
| `field never initialised` | 438 |
| `call to X, which was not translated` | 128 |
| `operator X has no Rust name` | 121 |

前两条**多半也是第 50 轮那种"跨模块"的假设**:常量实例的字段名要查那个类,
而现在查得到了——**下一轮先确认这两条是不是同一个原因**。

错误 7292:`E0425` 5444、`E0433` 728、`E0405` 573。
