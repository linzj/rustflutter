...
## 下一步:`?.`(剩下 1573 个 `Let` 的主体)

第 18 轮量到 4838 个"基于 null 测试的条件式",多半是 `a?.b`:

    let final T #0 = a in #0 == null ? null : #0.b

和 `??` 的区别在 **then/else 哪一边是那个临时变量**:
`??` 的 else 是临时变量本身,`?.` 的 then 是 `null`。

Rust 这边是 `a.map(|x| x.b)`——但要当心:
`a?.b?.c` 会嵌套成 `map` 里再 `and_then`,而且 `a?.b` 的结果是 `Option`,
**和 `??` 一样需要知道结果的可空性**。这一轮加的 `IrIfNull` 是同一类问题的先例。

**先 dump 那 4838 个的确切形状**,别照 Dart 源码猜——
第 8、18 轮都是先 dump 再写,两次 IR 都一次对上。

## 当前队头

**Kernel(发布路径,664 库 / 3665 类 / 11690 次拒绝)**
...
1. **"零拒绝"(Kernel 802 / 3665)不等于"翻译好了"**:引用其他库的东西仍靠手写桩。
2. **`testdata` 的桩已经吃力**。真正的答案是翻译 `dart:core` / `dart:ui` 的最小子集。
3. **哪把仪器守哪条守卫,要记清楚。** 断言、rustc、前端对照各有各的盲区。
   变异存活时先问"该用哪把仪器",而不是先假定等价。
4. **短路语义是这条路上最容易静默出错的一类。** `??` 错了十七轮,
   因为 `unwrap_or` 能编译、能过测试,只在右边有副作用时才不同。
   `?.` 是同一族,做的时候要专门为它造能观察到求值的 fixture。
