...
**下一轮**:先看那 157 个整类被拒的是什么理由——现在数量小了,
应该能一眼看出还有没有别的"单位不对"的地方。
然后是 `E0659`(718)名字歧义,那是 glob 导入的代价,可能要改成具名导入。

**`late`**:读的那一侧仍未解决(`Box<dyn Trait>` 不是 `Clone`),
但**它现在只拒构造函数,不再拒整个类**,优先级下降了。
