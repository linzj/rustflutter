...
## 下一步:`dart:core` 挑着翻,和 `dyn`

按 8177 个错误里的点名:

1. **`Object`(546)**、`Set`(194)、`Duration`(127)、`Future`(128)、
   `DateTime`(112)——`dart:core` **挑着来**,不能整个照搬:
   `String`/`int`/`double` 后端已映射到 Rust 原生类型。
2. **`E0782`(802):trait 对象要写 `dyn`。** 这条纯粹是后端的拼写问题,
   不是缺东西——`Box<Trait>` 该是 `Box<dyn Trait>`,应该是最便宜的一大块。
3. `Infinity`(183)是 `double.infinity`,一行。
4. `TextStyle`(367)在 `dart:ui` 里被拒了,查一下为什么。
