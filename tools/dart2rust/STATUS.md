...
躲不掉了,它是队头而且遥遥领先。**和可变性纠缠**:
Dart 的闭包捕获可变状态,Rust 的 `Fn`/`FnMut`/`FnOnce` 有借用规则。

**先量再动**(第 3、10、14、15 轮都证明这一步值)。要分的四类:

1. 不捕获任何东西 → 直接是 `fn`
2. 只捕获不可变的 → `Fn`,按值或按引用捕获
3. 捕获可变状态 → `FnMut`,而且捕获物的生命周期要说清
4. 捕获 `this` → Flutter 的回调几乎全是这种,**最难**:
   `this` 是 `&self`,闭包要活得比调用久,需要 `Rc`/`Weak` 或别的所有权安排

第 4 类大概率压倒多数。**这可能不是一轮能做完的**,量清楚后先切第 1、2 类。

## 也可以先做的两块(都比闭包便宜)

- **顶层函数调用(964)**:`identical`、`clampDouble`、`debugCheckX`……
  它们是自由函数,Rust 也有。难点是**跨库依赖**,和常量那轮一样。
- **mixin 成员(391)**:mixin 在 Kernel 里已经展开成合成类(第 7 轮量到 871 个),
  所以 Kernel 前端可能几乎白得——**先查 Kernel 侧这 391 是不是已经不存在了**。

## 当前队头(685 文件 / 3331 个类 / 9155 次拒绝)
...
1. **"零拒绝"(878 / 3331)不等于"翻译好了"**:引用其他库的东西仍靠手写桩。
