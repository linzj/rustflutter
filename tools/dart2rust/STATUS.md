...
## 下一步:捕获 `this` 的闭包(878,新队头)

这是躲不掉的那件事,也是**这条路上第一个真正的所有权问题**。

Flutter 的回调都是这个形状:

    onPressed: () { setState(() { _count += 1; }); }

闭包读写 `this` 的字段,而且**被存起来**(挂在 widget 上、给按钮),
所以它活得比 `build` 那次调用久。Rust 这边 `&self` 借用做不到。

可能的答案,**先各自量清楚代价再选**:

1. `move` 闭包 + 值语义:结构体是 `Copy` 就能捕获副本,但**写回去就丢了**
2. `Rc<RefCell<Self>>`:手工移植那边就是这么做的(`src/flutter/rust/rustflutter`),
   而且第 5 轮记过——**五处重入,五种不同策略**,不是一条规则能覆盖的
3. 闭包接收 `&mut Self` 作参数,而不是捕获它:改变签名,但 Rust 味道最正

**这一轮之前先做别的**:队头第二到第六(`BlockExpression` 450、
方法撕下 446、剩余 `Let` 441、`for` 369、字符串拼接 355)加起来 2061,
每个都不需要所有权模型。**先把它们清掉,再用一整轮专门规划 `this`。**

## 当前队头

**Kernel(发布路径,664 库 / 3665 类 / 10329 次拒绝)**

| 次数 | 要建的东西 |
|---|---|
| 878 | 闭包捕获 `this`(要所有权模型) |
| 450 | `BlockExpression` |
| 446 | 方法撕下来当值 |
| 441 | 剩下的 `Let` |
| 369 | `for` 语句 |
...
1. **"零拒绝"(Kernel 855 / 3665)不等于"翻译好了"**:引用其他库的东西仍靠手写桩。
2. **`testdata` 的桩已经吃力**。真正的答案是翻译 `dart:core` / `dart:ui` 的最小子集。
3. **哪把仪器守哪条守卫。** 断言、rustc、前端对照各有盲区。
4. **短路语义最容易静默出错**(`??` 错了十七轮)。
5. **fixture 里的值要能分辨错误配对。** 一个参数的闭包分辨不出乱序,
   两个相同的 super 参数分辨不出反序——**每次都要问"这个用例能不能分辨错的那种"**。
