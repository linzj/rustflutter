...
## 下一步:`EqualsNull`(2524)+ `Let`(2077)

Kernel 队头的前两名,**都不是 Dart 的概念,是 CFE 降级的产物**。

- **`EqualsNull` 就是 `x == null`**,Rust 里是 `x.is_none()`。便宜,先做。
- **`Let` 是 `??`、`?.`、级联的降级临时量。** 要先**读懂 CFE 的降级模式**,
  再决定是**还原**成源码形态(`a ?? b`)还是直译成临时变量。

第 8 轮把运算符从 `InstanceInvocation` 还原成二元表达式,是同一类工作,
而且证明了**还原比直译好**——直译会产出没人认得的 Rust,
而这个项目的产出要能和上游对照着读。

**先 dump `Let` 的实际形状再动手**(第 8 轮就是先 dump 节点再写代码,
那一轮的 IR 一次就对上了)。

## 当前队头

**Kernel(发布路径,664 库 / 3665 类 / 13045 次拒绝)**
...
**analyzer**:队头仍是闭包 900 + 638。

## 三条要记住的

1. **"零拒绝"(Kernel 732 / 3665)不等于"翻译好了"**:引用其他库的东西仍靠手写桩。
2. **`testdata` 的桩已经吃力**。真正的答案是翻译 `dart:core` / `dart:ui` 的最小子集。
3. **dill 路线的两项代价已经量到**:没有文档注释,常量名被折叠掉。
   两者都让产出更难和上游对照。analyzer 前端因此仍然值得留着,
   而不只是作为对照组。
