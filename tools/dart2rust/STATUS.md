...
- **当前读数（ws1124）**：拒绝 **9**（ws1101 的 12 起算;ws1123 去掉 `Platform.toJson`,ws1124 去掉 `_addLicenses` ×2），桩 **18**（ws1098 的 36 起算，每轮 gone N / new 0，`stubdiff.sh` 逐字节；ws1122 → ws1124 桩集合不变），unstubbable **0**，可达 **69**，0 error；渲染树连采五次 **707 节点 / 0 panic / 0 类型差异**；夹具 **125 个，124 AGREE + `ffistruct` 故意红**；panic 尺子 **「程序还能走、这边走不了」的中止点 = 0**（ws1121 的 `fold` 两处 `expect` 让它变 2,ws1122 改成不中止的写法后归 0）。
...
- **18 条桩,逐条写下理由(ws1121;ws1119 的 21 条去掉 `_initLicenses`、`LicenseRegistry.licenses`、`_PackagesViewState.licenses`)**。按「那个程序」的理由分:
  ...
  - **本编译器的欠账,8 条,不是程序的理由**:dart:io 的 `HttpClient`/`Stream.pipe` 不在 prelude(`io_client send`);`TypedData` 在 prelude 是空 struct,`TypedDataBuffer._buffer as TypedData` 无处落(`typed_buffer` ×2);擦除的 `T?` 字段的 `?` 只活在字段拼法里(`scheduler_binding.scheduleTask` ×2、`_RouteEntry.handleDidPopNext`、`push_named`,见下面两条痕迹);槽位 mixin 三处改动要一起做(`slotted…debugDescribeChildren`)。
