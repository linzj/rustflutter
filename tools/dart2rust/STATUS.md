...
(525 库 / 2743 类 / 1196 个零拒绝的类 / 11428 个成员发出 / 3523 次拒绝)

| 次数 | 要建的东西 |
|---|---|
| 576 + 420 | **闭包捕获 `this` / 撕方法——同一件事,见第 30 轮的测量** |
| 141 | 写另一个对象的字段(穿过形参那半) |
| 140 | 非 const 静态字段(`LazyLock`) |
| 120 + 91 + 49 | `MapLiteral` / `ListConstant` / `MapConstant` |
| 66 | 没有函数体 |
| 62 | `RecordLiteral` |
| 44 | `await` |

**下一轮**:集合的**字面量**(`MapLiteral` 120、`ListConstant` 91、`MapConstant` 49
合计 260)——类型和方法都通了,literal 还没有;`ListConstant` 里有一部分在
`static const` 位置上,那需要 `&[T]` 而不是 `Vec<T>`,要分开处理。
