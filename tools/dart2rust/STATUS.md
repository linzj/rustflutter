...
- **体积:还是没量成——我拿 debug 比了 release,这个数作废**(2026-09-11,ws1066)。
  `/tmp/work.md` 要的基准是 **release**:`.text` **73,071,360** / 文件 **214,887,120**
  (2026-09-10 19:52)。我量的是 `.crate-ws/target/debug/dart_main`
  (`.text` 108,733,298 / 文件 352,917,896),**两个 profile 不是一回事,比了等于没比**。
  盘上那个 release 二进制是 **09-10 17:55** 建的(`.text` 77,147,472 / 文件 230,489,072),
  **早于本会话的每一轮**,所以它也答不了这个问题。
  `bin/run_main.sh` 和 render_ruler 跑的都是 debug 构建
  (`workspace.py` 自己写着:那把仪器跑在 debug 上),release 只有要量体积时才建。
  **结论:这条账还欠着**,要答就得现建一次 release。
  两个 profile 各自的基准也要分开记,别再混。
