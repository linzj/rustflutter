...
它现在是队头,而且是**便宜的那种**:Dart 的 enum 和 Rust 的基本对得上,
`identifier of EnumElementImpl` 只是还没接。
先做它,把闭包那块大的留到后面单独规划。

要注意两点:

1. Dart 的 enum **可以有字段和方法**(增强枚举),Rust 的也可以(带数据的变体
   + `impl`)。但简单 enum 和增强 enum 是两种形状,**先量清楚各占多少**,
   别把它们当一件事——这是第 3、10 轮反复出现的教训。
2. `index` 和 `values` 是 Dart 每个 enum 都有的隐式成员。

## 之后:闭包(841 + 610 = 1451)

最大的一块,而且**和可变性纠缠**:Dart 的闭包捕获可变状态,
Rust 的 `FnMut` 有借用规则。这一轮和上一轮建的 `_mutating` 分析会用得上,
但捕获是跨函数的,比类内传播难。**单独规划,别顺手做。**

## 当前队头(685 文件 / 3331 个类 / 9939 次拒绝)
...
| 412 | super 构造函数调用 |
| 379 | 方法引用(撕下来的方法) |
| 358 | 赋值给 `a.b`(另一个对象的字段) |
| 313 | 字符串插值 |

第 358 条是可变性剩下的那半:**穿过别的对象的字段写**(量到 3869 次),
需要可变的接收者。它比 `&mut self` 难,而且和闭包的借用问题是同一族。

## 两条要记住的警告

1. **"零拒绝"(838 / 3331)不等于"翻译好了"**:引用其他库的东西仍靠手写桩。
