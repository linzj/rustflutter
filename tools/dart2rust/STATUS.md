...
- **当前读数（ws1123）**：拒绝 **11**（ws1101 的 12 起算;ws1123 去掉 `Platform.toJson`），桩 **18**（ws1098 的 36 起算，每轮 gone N / new 0，`stubdiff.sh` 逐字节；ws1122 → ws1123 桩集合不变），unstubbable **0**，可达 **69**，0 error；渲染树连采五次 **707 节点 / 0 panic / 0 类型差异**；夹具 **124 个，123 AGREE + `ffistruct` 故意红**；panic 尺子 **「程序还能走、这边走不了」的中止点 = 0**（ws1121 的 `fold` 两处 `expect` 让它变 2,ws1122 改成不中止的写法后归 0）。
...
- **11 条拒绝(ws1123;ws1101 的 12 条去掉 `Platform.toJson` 的 `JsonEncoder`,另补上当时漏记的 `GetQueue._check`)**。5 个机制;4 条有关于「那个程序」的理由,其余 7 条是本编译器的欠账:`GZipCodec` 的 const(`_addLicenses` ×2,背后还有 `gzip.decode` 的 inflate 与 `compute` → `Isolate.run`,三件事换两条)、`identical` 作用在非引用上 ×2、`super.==`/`hashCode` ×2、`GetQueue._check` 里 `item.completer.completeError(e)` 是对 `dynamic` 接收者的调用(queue 包把 completer 存成 `dynamic`)。
