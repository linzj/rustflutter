...
- **当前读数（ws1122）**：拒绝 **12**，桩 **18**（ws1098 的 36 起算，每轮 gone N / new 0，`stubdiff.sh` 逐字节；ws1120 → ws1122 gone 2：`LicenseRegistry.licenses`、`_PackagesViewState.licenses`），unstubbable **0**，可达 **69**，0 error；渲染树连采五次 **707 节点 / 0 panic / 0 类型差异**；夹具 **123 个，122 AGREE + `ffistruct` 故意红**；panic 尺子 **「程序还能走、这边走不了」的中止点 = 0**（ws1121 的 `fold` 两处 `expect` 让它变 2,ws1122 改成不中止的写法后归 0）。
