...
| ws643 | 链：stub **602**（-12），拒绝 229。 | |
| run643 | `_removeChild` 过了，**进到 gallery 自己的 `RootPage.build`**；下一站 `layout_adaptive.isDisplayDesktop` 的 stub：`getWindowType(context) >= AdaptiveWindowType.medium`——用户类声明的 `operator >=` 被拼成 Rust 的 `>=`（无 `PartialOrd`）。`stdOperators` 之外的运算符在翻译类上是方法（`ge`/`lt`…），前端不再走 `IrBinary`。夹具 cmpop SAME。 | ws644 量；run644 看下一站。 |
| ws644 | 链：stub **599**（-3：`isDisplayDesktop`、`indicatorRect`、`basic.paint`），拒绝 229，可达 64。 | |
| run644 | `RootPage.build` 过了，下一站 `LayoutBuilder`：`_LayoutBuilderElement<Rc<dyn Constraints>>::renderObject` 的 `as RenderAbstractLayoutBuilderMixin<LayoutInfoType, RenderObject>` 转型 `unwrap` 到 None。根因两层：① `ConstrainedLayoutBuilder<ConstraintType extends Constraints>` 的参数擦除到界，`LayoutBuilder` 因而是 `AbstractLayoutBuilder<Rc<dyn Constraints>>`，元素也在 `<Constraints>` 上实例化，可 `_RenderLayoutBuilder` 只实现了 `<BoxConstraints>`——普查（census）从没见过 `RenderAbstractLayoutBuilderMixin<Constraints, ..>`，因为超类型子句是逐个参数拼的，绕过了 `_type` 的普查，且带类型参数的实例化被整个跳过。修（通用）：超类型子句进普查；含**已擦除**参数的实例化按「Rust 实际持有的样子」——参数换成界——再普查一次（`_atErasedBounds`，`_censusMembers` 顺着走到 `createRenderObject` 的返回类型）。② 类自己的实例化也得按 Rust 的路径算（`_asRustInstance`：沿 extends/with/implements 逐层把擦除参数换界再代入），否则 Dart 视角的 `Builder<BoxC>` 与 Rust 已写的 `Builder<Rc<dyn Constraints>>` 各写一份 impl（E0119）。③ coerce：同一 trait 不同**具体**实参之间原本「无转型」直接放行（rustc 报 mismatched → stub），现在是 `dart_cast_to`（对象经更宽 impl 应答）；实参含类型参数仍放行。夹具 atbounds SAME（gentrait/projarg/traitset/restoreprop 回归 SAME）。 | 链 ws645 |
| ws645 | 链：stub **592**（-7：`Tween<T>` 进 `Tween<Object>` 一族，同 trait 异实参现在是转型），拒绝 229。 | |
| run645 | 同一 panic：`_RenderLayoutBuilder` 仍无 `RenderAbstractLayoutBuilderMixin<Constraints>` 的宽 impl——AOT dill 把 `AbstractLayoutBuilder.createRenderObject` 的覆盖摇掉了，`RenderAbstractLayoutBuilderMixin<LayoutInfoType, ..>` 只剩 `_LayoutBuilderElement<LayoutInfoType>` 里有，而它是 `createElement` **体内**构造的。修（通用）：`_censusMembers` 顺带走类体内构造的泛型类型（`_constructedIn`，每类一次；构造调用与工厂调用），代入实例化后再普查；`ConstructorInvocation` 的构造类型也进普查。夹具 atbounds 改成无覆盖版 SAME。 | 链 ws646 |
| ws646 | 链：stub **592**（持平），拒绝 229；`_RenderLayoutBuilder` 有了 `<Rc<dyn Constraints>>` 的 impl。 | |
| run646 | **LayoutBuilder 过了，整棵 widget 树建完，layout/paint 跑到 `compositeFrame`**；下一站 `_updateSystemChrome` → `Layer.find<S>` 的 stub（欠账 run553）：`findAnnotations<S>(AnnotationResult<S> result, ..)` 经擦除孪生时 `result` 是 `AnnotationResult<Rc<dyn Object>>`，与调用方的 `AnnotationResult<S>` 是两个 struct（出参要**同一个**对象）。修（通用，三段）：① 协变分析 `covariance.dart` 把「trait 类泛型方法的签名经孪生变成实参=界」算作一次 `C<A>`→`C<B>` 流（`_throughTwin`），`AnnotationResult.T` 因而擦除；擦除再沿成员签名传播（`Bag<T>` 擦了则 `add(Entry<T>)` 里的 `Entry.T` 也擦，`_spelledInto`，只进可擦参数，定点）。② 前端：构造调用的槽按声明类型（含擦除参数时）拼，不按调用实参；`_argumentBinding` 对擦除参数不绑；无规则可读的提升局部仍带声明类型。③ `is S`/`as S` 也算 `S` 的「值用法」（`_TypeLiteralFinder`），孪生因此带隐藏 `__ty_i`；`Type` 带 `id: Option<TypeId>`（按名相等），`dart_type_of` 填 id，`is S` 走 `dart_is_type(obj, __ty_i)`（对象自己的 cast 表按 id 答）。夹具 outparam SAME（`hi,null,3`），10 个相关夹具回归 SAME。 | 链 ws647 |
