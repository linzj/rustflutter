...
| 整包,glob | ~13 分钟 | 10277 |
| 整包,精确 | **11 分 25 秒** | **10422**(+145) |

切片上是明显的赢,整包上只快一成多、错误还多了 145。
**留着**,因为它结构上是对的(精确的导入强过 glob,顺带整类歧义不复存在),
但它没有解决整包这一档,得说清楚。
...
**下一轮**:E0425(整包 9137,占九成)。先用 `--slice core`(4 秒)看清形状。
另外还欠一笔:精确导入之后剩下的 E0432 差额没查清。
