...
- **当前读数（ws1118）**：拒绝 **12**，桩 **22**（ws1098 的 36 起算，每轮 gone N / new 0，`stubdiff.sh` 逐字节；ws1116 → ws1118 gone 2：`RenderPointerListener.handleEvent`、`ChangeNotifierProvider.value`），unstubbable **0**，可达 **69**，0 error；渲染树连采五次 **707 节点 / 0 panic / 0 类型差异**；夹具 **120 个，119 AGREE + `ffistruct` 故意红**；panic 尺子 **「程序还能走、这边走不了」的中止点 = 0**。
