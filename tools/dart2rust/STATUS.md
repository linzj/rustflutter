...
## 下一步:异常 → `Result<T, E>`(用户已定方向)

**第一步是量传染范围,不是写代码。** 具体要产出三个数:

1. 从 831 个 `throw` 出发沿调用图向上传播,**有多少函数会变成返回 `Result`**?
   如果是"几乎全部",那 `Result` 会渗透到每个签名,得先知道这一点。
2. **一个函数能抛几种错误类型?** 474 次 `FlutterError` 占 57%,
   如果多数函数只抛一种,`Result<T, FlutterError>` 就够,不必造枚举。
3. `try/catch` 的 171 处**能不能截断传播**——`catch` 之后调用者就不用返回 `Result` 了,
   这是传染的边界。

量完再决定切多大一块。**先做"函数体里 throw、同一函数内 catch"这种不跨签名的**,
再处理跨签名的传播。

## 当前队头

**Kernel(发布路径,664 库 / 3665 类 / 9997 次拒绝)**

| 次数 | 要建的东西 |
|---|---|
| 905 | 闭包捕获 `this`(要所有权模型) |
| 461 | 方法撕下来当值 |
| 401 | `for` 语句 |
| 372 | 字符串拼接 |
| 312 | switch 当表达式(`Let` 里绑定无初值) |
| 298 | 顶层调用 `identical` |
| ~258 | **异常**(throw 191 + try/catch 41 + try/finally 26) |

## 七条要记住的

1. **"零拒绝"(Kernel 912 / 3665)不等于"翻译好了"**:引用其他库的东西仍靠手写桩。
2. **`testdata` 的桩已经吃力**。真正的答案是翻译 `dart:core` / `dart:ui` 的最小子集。
3. **哪把仪器守哪条守卫。** 断言、rustc、前端对照各有盲区。
4. **短路语义最容易静默出错**(`??` 错了十七轮)。
5. **fixture 里的值要能分辨错误配对**——已经栽了五次:
   单参数闭包、相同的 super 参数、只替换第一处、没人碰 `Season`、
   没有字段同时有声明值和构造函数值。
6. **变异存活时,先查我改的那行和测试断言的是不是同一件事。**
7. **探针能分对"是什么"却分错"长什么样"。** 第 23 轮探针把级联的绑定
   当成块的第一条语句,而真实形状是 `Let` 包着块——
   **两前端对照才是发现这件事的仪器**。
