...
(525 库 / 2743 类 / 1094 个零拒绝的类 / 11078 个成员发出 / 3834 次拒绝)

| 次数 | 要建的东西 |
|---|---|
| 520 | 闭包捕获 `this`(要所有权模型) |
| 387 | 方法撕下来当值(**978/1018 接收者是 `this`,同一个问题**) |
| 174 | 写另一个对象的字段 |
| 140 | 非 const 静态字段 |
| 117 | `MapLiteral` |
| 108 | `StaticTearOffConstant` |
| 93 | 字符串插值 |

**下一轮该做所有权**:队头前两项加起来 907 条,是同一件事。
这个仓库自己的手写移植当初是"五个重入点,五种策略",没有通解——
所以先量:这 907 处闭包**存到哪里去了**,立即用掉的有多少。
