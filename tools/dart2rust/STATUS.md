...
- **当前读数（ws1120）**：拒绝 **12**，桩 **20**（ws1098 的 36 起算，每轮 gone N / new 0，`stubdiff.sh` 逐字节；ws1119 → ws1120 gone 1：`_PackageLicensePageState._initLicenses`），unstubbable **0**，可达 **69**，0 error；渲染树连采五次 **707 节点 / 0 panic / 0 类型差异**；夹具 **122 个，121 AGREE + `ffistruct` 故意红**；panic 尺子 **「程序还能走、这边走不了」的中止点 = 0**。
...
- **20 条桩,逐条写下理由(ws1120;ws1119 的 21 条去掉 `_initLicenses`)**。按「那个程序」的理由分:
