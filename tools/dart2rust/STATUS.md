...
拒绝 3272,**整类被拒 0**。真实分布见第 54 轮。

| 理由 | 次数 | |
|---|---|---|
| 闭包捕获 `this` + 撕方法 | 768 + 478 | **所有权,第 30 轮量过** |
| 顶层调用(别的库的) | 565 | 跨模块的顶层函数没接上 |
| 没有函数体 | 231 | external / abstract,**本来就该拒** |
| 写另一个对象的字段 | 155 | 要 `&mut` 形参 |
| `identical` 不是引用 | 145 | |
| `is` | 115 | 要类型层次 |

**下一轮**:顶层调用 565 —— 第 36 轮做了顶层函数,但只认**本库**的;
现在整个 package 一个 crate,别的模块的顶层函数也够得到。
**和第 42、50 轮同一种"假设过期了"**,应该是最便宜的一块。
