...
字段赋值做完了,setter 是它的另一半,而且**需要跨类信息**:
Dart 的 `a.x = 1`,若 `x` 有 setter 就得变成 `a.set_x(1)`——
**得知道 `x` 有没有 setter**,而 `a` 的类可能在别的文件里。

Kernel 这里又占优:`InstanceSet.interfaceTarget` 直接说是 `Field` 还是
`Procedure`,不用查。analyzer 那边靠 `writeElement.isSynthetic`。
两边都已经能分辨了(这一轮就是靠它拒绝 setter 的),
所以下一轮是**把拒绝改成生成**:声明 `set_x(&mut self, v)`,调用点改成 `a.set_x(1)`。

## 当前队头(685 文件 / 3331 个类 / 10626 次拒绝)
...
**闭包(822 + 602 = 1424)现在是最大的一块,而且它和可变性纠缠**:
Dart 的闭包捕获可变状态,Rust 的 `FnMut` 有借用规则。
这一轮建的 `_mutating` 闭包分析会用得上,但捕获跨函数,比类内传播难。

**枚举(820)是便宜的一块**,可以插在闭包之前:Dart 的 enum 和 Rust 的
基本对得上,`identifier of EnumElementImpl` 只是还没接。

## 两条要记住的警告

1. **"零拒绝"(828 / 3331)不等于"翻译好了"**:引用其他库的东西仍靠手写桩。
2. **`testdata` 的桩已经吃力**。下一堵墙是依赖图,真正的答案是
   翻译 `dart:core` / `dart:ui` 的最小子集。
