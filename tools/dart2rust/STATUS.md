...
## 下一步:super 构造函数调用带参数(1481,新队头)

Dart 的 `Sub(...) : super(a, b)` 要把参数传给基类的构造函数。
Rust 没有构造函数继承,所以基类的字段得**在子类的结构体里**,
而基类构造函数的**函数体要能被子类的构造函数调用**——
这和第 6 轮 `super` 方法调用是同一类问题,那次的答案是**自由函数**。

要先回答的:

1. **基类字段在哪?** 现在抽象基类变成 trait,而 trait 不持有存储,
   所以基类的字段**已经被报为 NOT TRANSLATED**(第 5 轮就是这么记的)。
   super 构造函数要传参给它们,就必须先决定这些字段落在哪——
   最直接的答案是**扁平化到每个子类**。
2. **具体基类**(不是抽象的)怎么办?现在它生成 struct,子类另一个 struct,
   两者没有关系。扁平化同样适用,但会复制字段。

**先量**:1481 次里,基类是抽象的占多少、是具体的占多少。
这两种情况的答案可能不同,而现在它们记成了一条——
第 10、15 轮都证明了合并的条目会把人送去建错东西。

## 当前队头

**Kernel(发布路径,664 库 / 3665 类 / 11025 次拒绝)**
...
1. **"零拒绝"(Kernel 814 / 3665)不等于"翻译好了"**:引用其他库的东西仍靠手写桩。
2. **`testdata` 的桩已经吃力**。真正的答案是翻译 `dart:core` / `dart:ui` 的最小子集。
3. **哪把仪器守哪条守卫。** 连续三轮,Kernel 侧的守卫都只有 `fixtures.py` 抓得到。
4. **短路语义最容易静默出错**(`??` 错了十七轮)。做这类的时候,
   fixture 必须能观察到"求值了没有",光看答案对不对分辨不出来。
