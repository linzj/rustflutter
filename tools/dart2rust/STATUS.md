...
(525 库 / 2743 类 / 1251 个零拒绝的类 / 11786 个成员发出 / 3185 次拒绝)

| 次数 | 要建的东西 |
|---|---|
| 588 + 428 | **闭包捕获 `this` / 撕方法——同一件事,见第 30 轮的测量** |
| 141 | 写另一个对象的字段(穿过形参那半) |
| 66 | 没有函数体(external / abstract,**这条大概本来就该拒绝**) |
| 45 | `await` |
| 44 | `LocalInitializer` |
| 38 | `TypeLiteralConstant` |

**节奏**:一轮攒几个 blocker,最后统一验一次;`fixtures.py` / `regen.py` 并发;
变异扫描只碰它够得到的 fixture(`bin/mutate.py`,一轮不到一分钟)。
