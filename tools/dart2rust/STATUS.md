...
## 下一步:给 Kernel 独有的路径一条回归通道

这一轮暴露的问题比它修的那个 bug 更重要:
**Kernel 前端有 analyzer 前端遇不到的形状**(枚举常量、`EqualsNull`、`Let`、
`BlockExpression`),而现有的验证全走 analyzer——
fixture 是源码文件,`agree.py` 只比一个库。
所以 Kernel 侧的 bug 可以存活任意多轮,直到有人碰巧去量。

要建的:**给 fixture 也造 dill**。
`bin/dill.py --build` 已经能为任意入口产出 dill;
fixture 是不依赖 Flutter 的纯 Dart 文件,应该能用同一套工具链编。
建成之后每个 fixture 都能**两个前端各跑一遍**,
`agree.py` 的对照从"一个库"变成"整套 fixture"。

**先验证 fixture 能不能编出 dill**(修订版、target、package_config),
编不出来就得另想办法,而不是写完前端才发现。

## 之后:`EqualsNull`(2505)+ `Let`(2046)

它们是 Kernel 队头的前两名,**都不是 Dart 的概念,是 CFE 降级的产物**:

- `EqualsNull` 就是 `x == null`,Rust 里是 `x.is_none()`。便宜。
- `Let` 是 `??`、`?.`、级联的降级临时量。要看懂 CFE 的降级模式,
  再决定是**还原**成源码形态(`a ?? b`)还是**直译**。
  第 8 轮把运算符从 `InstanceInvocation` 还原成二元表达式,是同一类工作,
  而且证明了还原比直译好——直译会产出没人认得的 Rust。

## 当前队头

**Kernel(发布路径,664 库 / 3665 类 / 13317 次拒绝)**
...
**analyzer(685 文件 / 3331 类 / 9155 次拒绝)**:队头仍是闭包 900 + 638。

两把尺子都要留着:一个是发布路径,另一个产出**带文档注释、更像源码**的 Rust,
而且它们互相能抓对方的错(第 12 轮 `agree.py` 抓到过我在 Kernel 前端的语法错)。

## 两条要记住的警告

1. **"零拒绝"(Kernel 723 / 3665)不等于"翻译好了"**:引用其他库的东西仍靠手写桩。
