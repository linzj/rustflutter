...
## 下一步:可变性(赋值 + setter,2443 次)

私有成员进来之后的真队头(685 文件 / **3331 个类** / **11869 次拒绝**):
...
赋值(1631)+ setter(812)= **2443,是同一件事:可变性**。
Rust 这边要 `&mut self`,还要一个"什么时候可变"的模型——
这比之前任何一轮都大,**动手前先规划**,别直接改。

要先回答的三个问题:

1. 一个 Dart 方法写了字段就要 `&mut self`,而 `&mut` 会传染到调用者。
   谁来判断传染边界?
2. `impl Add for Alignment` 这类 trait 方法签名是固定的(`self`,不是 `&mut self`),
   如果里面有赋值怎么办?
3. Dart 的字段可以从外部赋值(`a.x = 1`),Rust 的 `pub` 字段也可以——
   但 setter 有逻辑,不是纯赋值。setter 变成 `set_x(&mut self, v)`,
   那外部的 `a.x = 1` 就要变成 `a.set_x(1)`,**这需要知道 `x` 有没有 setter**。

先量清楚:1631 次赋值里,有多少是**方法内的局部变量赋值**(不需要 `&mut self`),
多少是**字段赋值**。这两个数差别很大,而现在混在一个条目里。
**这个数字本身要先被信任**,和第 3 轮 `abstract method` 是同一类问题。

## 一条要警惕的记录

"零拒绝"现在是 776 / 3331。它比以前诚实得多(私有类算进来了),
但仍然**不等于"翻译好了"**:私有成员现在会翻译,但引用**其他库**的东西
(`Object.hash`、`dart:ui` 的类型)仍然只能靠手写桩顶着。
`testdata` 里那些桩已经开始吃力了——这是下一个会撞上的墙。
