...
1. **枚举的值要从常量里恢复。** 整个 crate 一个 `pub enum` 都没有,
   因为 dill 里枚举的值字段被摇掉了(第 38 轮量的:`Axis` 0 字段)。
   第 24 轮已经用过办法——`InstanceConstant` 的 `_name`。
   `Axis` 一个影响 99 个模块。
2. **`dart:ui` 的最小子集**:`Color`(1558)、`Offset`(873)、`Size`(507)、
   `TextDirection`(389)、`TextStyle`(352)、`Rect`(336)、`Matrix4`(231)。
   这些不在 `package:flutter/` 里,永远不会自己出现。
3. 然后重新跑 `rustc` 数错误——**12807 是基线**。

`bin/compiles.py` 量单个库能不能独立编(115/525);
`bin/dart2rust_package.dart` 量整包一起编有多少错(12807)。两把尺子都留着。
...
8. **拒绝数升高可能是好事。** 第 9 轮(私有成员)和第 24 轮(构造函数体)
   ...
   **报进度时要说清是哪一种。**
9. **一个变异必须还编得过。** 让编译器自己编不过的变异什么也没证明。
   第 29 轮和第 31 轮各撞见一次:删掉一个分支会留下未用的变量或死代码里的
   类型错误。**改答案,不要改得编不过**。
10. **数字必须带量它的条件。** 第 24 轮记的 `10040` 没写 dill 和前缀,
