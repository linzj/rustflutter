...
- **体积:欠了两轮的那一次量,量了**(2026-09-11,ws1064 之后)。
  `/tmp/work.md` 第五节第 3 条要的就是这个数,而且写明「涨多少要量,不要估」。
  基准 2026-09-10 19:52:`.text` **73,071,360** / 文件 **214,887,120**。
  现在(`.crate-ws/target/debug/dart_main`,07:55 那次 render_ruler 建的):
  `.text` **108,733,298**(**+48.8%**)/ 文件 **352,917,896**(**+64.2%**)。
  **这个读数说不出涨在谁头上。** 两个时间点之间跑了 ws1053 到 ws1064 一共十二轮,
  里面既有 `?` 传播(ws1061/1062 换掉 22,108 处 `.unwrap()`,`?` 有传播代码、
  `Err` 有构造),也有身份令牌(每个带令牌的实例多一个字、多一条 vtable 方法)、
  动态成员分派(每个候选一条 downcast 臂)等等。
  **要分摊就得一轮一量**,而这两天没有一轮量过——这条账记在这里,
  下一轮起按 `/tmp/work.md` 第六节的要求,每一行都带体积前后数。
