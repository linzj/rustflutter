...
(525 库 / 2743 类 / 1193 个零拒绝的类 / 11380 个成员发出 / 3571 次拒绝)

| 次数 | 要建的东西 |
|---|---|
| 560 + 418 | **闭包捕获 `this` / 撕方法——同一件事,见第 30 轮的测量** |
| 141 | 写另一个对象的字段(穿过形参那半) |
| 140 | 非 const 静态字段(`LazyLock`) |
| 119 + 85 + 49 | `MapLiteral` / `ListConstant` / `MapConstant` |
| 66 | 没有函数体 |
| 62 | `RecordLiteral` |
| 44 | `await` |
| 剩下的 List 方法 | `map` / `where` / `toList` / `first` / `last` / `sort` … |

**下一轮**:`Map` 的表示(插入序,见上),以及剩下的 List 方法
——`map`/`where` 在 Dart 是惰性的 `Iterable`,在 Rust 是 `.iter().map(..)`,
链式调用要一起想清楚,不能一个一个映射名字。
