...
(525 库 / 2743 类 / 1043 个零拒绝的类 / 10470 个成员发出 / 4441 次拒绝)

| 次数 | 要建的东西 |
|---|---|
| 493 | 闭包捕获 `this`(要所有权模型) |
| 367 | `switch` 语句 |
| 364 | 方法撕下来当值 |
| 169 | 写另一个对象的字段 |
| 153 | 顶层调用 `max` |
| 140 | 非 const 静态字段 |
| 115 | `InstanceSet` 当表达式 |
| 111 | `MapLiteral` |

**节奏**:用户 2026-09-02 指出验证跑得太勤、进展太慢,以及一次起一个 Dart VM
太耗时。改成**一轮攒几个 blocker 一起实现,最后统一验一次**,且
`fixtures.py` / `regen.py` 都并发跑(25 秒 / 18 秒)。
