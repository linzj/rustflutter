...
| ws1070 | **`late` 读到没写过的值,Dart 抛 `LateInitializationError`,而这边一直是 `.unwrap()`**——`/tmp/work.md` 第六节第 1 步的头一批。一个 `late` 字段在这边**就是**它没值时的那个 `Option`,所以打开那个 `Option` 的地方是唯一能分辨的地方:十六处发射点全部改成 `.ok_or_else(|| dart_late_init_failed("Field"/"Local", "x"))` 再按上下文接 `?` 或 `.unwrap()`(`places.dart` 的 `_lateRead`)——trait 存取器 `emit_impl.dart:207/564`、字段读 `closures.dart:630/634/635`、`super.x` 和别的对象的字段 `super_calls.dart:339/349/373/382/410`、左值与读值两个 place `places.dart:573/719`。**`late` 局部变量那一处在前端**(`expression_raw.dart:124`):它自己造了 `IrCall(x.clone(), 'unwrap')`,改成 `!late` 伪调用交给后端,因为「空 `Option` 变成什么」是后端的事。夹具还挖出一个旧缺陷:**写*别的对象*的 `late` 字段没包 `Some`**(`b.s = 'written'` 把 `String` 写进 `Option<String>`,E0308),`_lateFieldOf` 按前端给的 owner、退到目标的静态类型去查,把读和写两侧统一了。`_optionRead`(`places.dart:282`)**没动**:它同时给 `is` 用,那里 `None` 的答案是 `false` 而不是抛。 | `.unwrap()` **7,350 → 4,154**(两次都在 `.crate/src` 上量,−3,196 / **−43.5%**;`.crate-ws` 4,139),`dart_late_init_failed` 出现 3,193 次;`panic!` 合计 7,219 = native 7,162 + dart2rust 41 + uncaught **16**(uncaught 一处没涨);拒绝 12、桩 **36 逐条相同**(gone 0 / new 0)、unstubbable 0、可达 69、0 error;run1070 连采五次 707 节点 / 0 panic / 0 类型差异;夹具 **83** 个,**82 AGREE** + `ffistruct` 那个故意的红。**欠一笔**:`panic!` 合计的「改之前」没在这棵树上量过(手上只有 ws1062 的 7,223),只有 `.unwrap()` 的前值是在 HEAD 上真跑了一次翻译量出来的 |
| ws1071 | **左值上的 `x!` 和 `removeLast()`,两条都是 Dart 会抛而这边在 panic 的**——work.md 第六节第 1 步的第二批。`xs!.add(1)` / `m[k]!.add(1)` 的 place 原来是 `.as_mut().unwrap()` / `.get_mut(&k).unwrap()`,改成 `.ok_or_else(dart_null_check_failed)` 接 `?`(`places.dart:547/557/609`);**`places.dart:542` 的 `??=` 那一处没动**,它前面刚写过 `if is_none { .. }`,证不出失败。`removeLast()` 改成 `.ok_or_else(dart_remove_last_empty)`,**而这个错不是 `Bad state: No element`**:夹具的第一版按空迭代器的 `StateError` 写,Dart 那边直接 unhandled——VM 的 `removeLast` 是 `_list[_length-1]`,抛的是 `RangeError (length): Invalid value: Valid value range is empty: -1`。**顺带揪出一个一直在吞错误的旧缺陷**:后端那条规则按名字匹配 `pop`,而 `SceneBuilder.pop()` 是 Dart 里真叫 `pop` 的方法——它返回的 `Result` 一直被 `.unwrap()` 吞掉。把 `removeLast` 的重命名改成伪调用 `!pop`(`ir.dart` 的 `listMethodNames`,只对 `List`/`Iterable` 的接收者生效),两者再不会撞;第一次测量正是这个撞出来的:**桩 36 → 49**(`rendering_layer.rs` 的 `add_to_scene` 十一个),改完回到 36。 | `.unwrap()` **4,154 → 3,991**(`.crate/src`;`.crate-ws` 4,139 → 3,976,−163),`.as_mut().unwrap()` 只剩 **1**(就是那个 `??=`);`panic!` 合计 **7,219 → 7,219** 一个没动(native 7,162 + dart2rust 41 + uncaught 16,这次前后都在同一棵树上量了);拒绝 12、桩 **36 逐条相同**(gone 0 / new 0)、unstubbable 0、可达 69、0 error;run1071 连采五次 707 节点 / 0 panic / 0 类型差异;夹具 **85** 个(新增 `bangplace`、`removelast`),**84 AGREE** + `ffistruct`;`test/member_names_test.dart` 的三个钉子 20/52/12 → 21/53/13 |
| ws1072 | **懒静态的初始化器抛了之后,Dart 下一次读会重跑**——work.md 第六节第 2 步,而第五节第 1 条那块板是先用 Dart 钉死的:`Bad state: not yet (1)|Bad state: not yet (2)|3|3|n=3`,**所以 `LazyLock<Result<T,E>>` 出局**,它会把第一个 `Err` 永远缓存。prelude 立了 `DartLazy<T>`:`fn() -> Result<T, DartError>` 加一个 `Isolate<OnceCell<T>>`,失败**不写**那个 cell,下一次读再跑一遍;`OnceCell` 而不是 `RefCell<Option<T>>`,是为了读能交出地址不动的 `&T`,`X.get()?.clone()` 和 `*X.get()?.borrow_mut() = ..` 都要这个。十四处发射点从 `(**X)` 改成 `X.get()` 加传播(读、可变读、左值、赋值、类静态与顶层两套声明),**初始化器的体放进 `_failure`**——这才是第 2 步真正的目标,那 1,171 个 `.unwrap()` 全在这些体里。一个新桩来自**手写的** `runtime/src/lib.rs:125`(`announce_view` 里 `*(**_IMPLICIT_VIEW_ID).borrow_mut()`),它不是发射出来的,改成 `.get()?` 之后桩回到 36。**没做**:Dart 的「初始化期间读自己」("Reading static variable 'x' during its initialization")这边不检测,第二次 `set` 直接输掉,记在 prelude 的注释里。 | `.unwrap()` **3,991 → 2,821**(`.crate/src`,−1,170,正好是计划里数的 1,171;`.crate-ws` 3,976 → **2,806**);`DartLazy::new` 388 处,`std::sync::LazyLock` 在生成代码里**归零**;`panic!` 合计 **7,219 → 7,219**(native 7,162 + dart2rust 41 + uncaught 16);拒绝 12、桩 **36 逐条相同**(gone 0 / new 0)、unstubbable 0、可达 69、0 error;run1072 连采五次 707 节点 / 0 panic / 0 类型差异;夹具 **86** 个(新增 `lazystatic`,头一次跑就 AGREE),**85 AGREE** + `ffistruct` |
| ws1073 | **trait impl 的转发体里,参数上的那个 cast 是 Dart 的,不是翻译器的**——剩下的 `.unwrap()` 抽样一看,压着的既不是 work.md 第 3 步也不是第 4 步,而是**第四个上下文**:`ProxyElement::update(self, new_widget.dart_cast_to::<dyn ProxyWidget>().ok_or_else(|| dart_cast_failed("ProxyWidget")).unwrap())`。`covariant` 参数在 Dart 里**就是调用处的一次向下转型**,转不过去 Dart 抛 `TypeError`;而整个 `impl` 块是在没有 `_failure` 的情况下发射的,所以每一处都 `.unwrap()`。**第一版把 `_failure` 加在整个 `_emitImplFor` 上,桩 36 → 128**:转发体的*返回值*走的是 `$call.map(|__v| ..)`,那个闭包返回裸值,`?` 进不去(92 个 E0277,外加几种同族的)。改成只包住 `_inherentCall` 那一步——**参数**,而且只在这个转发体自己返回 `Result` 的时候——桩回到 36。返回值那一侧仍然 `.unwrap()`,记在代码注释里,要动得先把 `.map` 换成能带 `?` 的形状。 | `.unwrap()` **2,821 → 1,711**(`.crate/src`,−1,110;`.crate-ws` 2,806 → **1,696**);`panic!` 合计 **7,219 → 7,219**(native 7,162 + dart2rust 41 + uncaught 16);拒绝 12、桩 **36 逐条相同**(gone 0 / new 0)、unstubbable 0、可达 69、0 error;run1073 连采五次 707 节点 / 0 panic / 0 类型差异;夹具 **87** 个(新增 `covariantparam`),**86 AGREE** + `ffistruct` |
| ws1074 | **Dart 的 `operator +` 会抛,而 `std::ops` 的签名说不了 `Result`**——work.md 第六节第 4 步。原来每个运算符都发一对:`impl std::ops::Add for X` 转发给内在的 `op_add`,而后者「签名是 std 的,里面失败就 unwrap」。前提本来就不成立——**那 60 个 `impl std::ops::*` 只有这个编译器自己的 `a + b` 在读**,而 `a + b` 是它自己选的拼法;`~/` 从写出来就走内在方法这条路(Rust 没有对应的 trait),形状是现成的。现在每个运算符都是普通内在方法(`_emitMethod`),impl 全删,调用点改发 `a.op_add(b)?`——抽象句柄那条本来就是,counted、按值、一元负号、trait 转发四条改过来。**两次返工,都是名字和接收者对不上**:① `_operatorName('&')` 是 `bit_and` 而调用点说 `op_bitand`,发的和叫的不是一个名字,99 个 E0599(`Offset & Size`),改成两边都用 `_methodName`;② counted 类的 `op_mul` 收 `self: &Rc<Self>`,而调用点还按老的 `impl for Struct` 发 `(*x).clone().op_mul(..)`,8 个 E0599(`Matrix4`/`Vector3`/`AttributedString`),改成在**句柄**上调。 | `.unwrap()` **1,711 → 1,617**(`.crate/src`;`.crate-ws` 1,696 → **1,602**);`impl std::ops::*` **60 → 0**;`panic!` 合计 **7,219 → 7,218**,其中生成侧 `uncaught Dart exception` **16 → 15**(第 5 步那 9 个里的一个就在运算符体里);拒绝 12、桩 **36 逐条相同**(gone 0 / new 0)、unstubbable 0、可达 69、0 error;run1074 连采五次 707 节点 / 0 panic / 0 类型差异;夹具 **88** 个(新增 `operatorthrows`),**87 AGREE** + `ffistruct` |
| ws1075 | **`__to_list` 的签名是这个编译器自己定的,所以它可以说 `Result`**——work.md 第六节第 5 步便宜的那一半。Dart 里没有任何成员叫 `__to_list`,它是给「翻译出来的类本身是个 `Iterable`」发的走查器;它要调 `iterator`,而 `iterator` 是 Dart 的 getter、会抛,可签名写成 `-> Vec<E>`,那个 `Result` 就无处可去——`panic!("uncaught Dart exception")` 是唯一出口,生成侧 7 处。改成 `-> Result<Vec<E>, DartError>` 加 `?`(结构体与 trait 默认两处发射点)。**夹具头一次跑是 DISAGREE,而且骗了我一下**:调用点在**前端**(`locals.dart:1367` 造的 `IrCall(.., '__to_list')`),它不知道被调的会失败,发出来是 `c.__to_list().clone().iter()`——`Result` 上的 `.iter()` 是合法的(产出 0 或 1 个元素),所以它编得过、跑出 `[[0,1,2]]` 和 `[]`,没有报错。给那个 `IrCall` 加上 `fails: true` 才对上。 | 生成侧 `uncaught Dart exception` **15 → 8**;`panic!` 合计 **7,218 → 7,211**;`.unwrap()` 1,602 不变(这一条不碰 unwrap);拒绝 12、桩 **36 逐条相同**(gone 0 / new 0)、unstubbable 0、可达 69、0 error;run1075 连采五次 707 节点 / 0 panic / 0 类型差异;夹具 **89** 个(新增 `iteratorthrows`),**88 AGREE** + `ffistruct` |
| ws1076 | **prelude 里那 32 个 `panic!("uncaught Dart exception")`,26 个改完了**——work.md 第六节第 6 步的大半。按 Dart 的错误类型分批:`Bad state: No element`(`first`/`last`/`single`/`reduce`/`removeFirst`/`removeLast`,Set、VecDeque、LinkedList 三套)、`FormatException`(`int.parse`/`double.parse`)、`ArgumentError`(`<<`/`>>`/`>>>` 的负位移)、`StateError`(`insertAfter`/`insertBefore`)、`TypeError`(`fillRange(.., null)`)。**其中 5 处是白送的**——函数本来就返回 `Result`,只是里面写了 `panic!`。发射侧配套:`_preludeFailing` 加六个名字、`_preludeFailingStatics` 加三个、位移加传播、**`Vec` 上的 `first`/`last` 原来发的是 `xs[0].clone()`**,空表时是越界 panic——同一条路以另一种方式丢掉,一起改成 `ok_or_else(dart_no_element)`。**夹具又抓到一次名字撞车**:Dart 顶层函数 `parseInt` snake 出来就是 `parse_int`,和 prelude 的同名,于是 `int.parse(text)` 在它自己体内**调了它自己**——而且编得过、跑得出东西。四个入口改成 `dart_` 前缀(这个 prelude 对外函数的惯例),前端 `static_calls.dart` 跟着改。 | **prelude `panic!` 合计 52 → 26,其中 `uncaught Dart exception` 32 → 6**;生成侧 `uncaught` 8、`panic!` 合计 7,211、`.unwrap()` 1,602 都没动(这一轮不碰生成侧);拒绝 12、桩 **36 逐条相同**(gone 0 / new 0)、unstubbable 0、可达 69、0 error;run1076 连采五次 707 节点 / 0 panic / 0 类型差异;夹具 **90** 个(新增 `preludethrows`),**89 AGREE** + `ffistruct` |
| ws1077 | **`raise(self) -> !` 存在的唯一理由就是 panic,所以它整个没了**——work.md 第六节第 6 步的剩下部分。`dart:io` 的 18 个同步调用(`createSync`/`openSync`/`readAsStringSync`/`writeAsBytesSync`/…)都走 `FileSystemException::raise`,而 Dart 从每一个里抛 `FileSystemException`——`on FileSystemException catch` 就是程序读一个不存在的文件的写法。18 个签名改成 `Result`,`raise` 删掉(它旁边本来就有 `failed(self) -> DartError`),名字进 `_preludeFailing`。外加 `utf8.decode(bad)` 的 `FormatException` 和 `Error.throwWithStackTrace`(它本来就是个 `throw`)。**今天第三次名字撞车**:`decode` 进表之后,`JsonCodec.decode` 也跟着传播了,两个新桩(`E0277`,`?` 用在 `Rc<dyn DartAny>` 上);改成**按接收者**认(`Utf8Codec`),照 `queueLike` 那条的先例。 | **prelude `panic!` 合计 26 → 23,其中 `uncaught Dart exception` 6 → 3**(只剩 JSON 那三处:`revive` 与两个 `json_key`);生成侧 `uncaught` 8、`.unwrap()` 1,602 不动;拒绝 12、桩 **36 逐条相同**(gone 0 / new 0)、unstubbable 0、可达 69、0 error;run1077 连采五次 707 节点 / 0 panic / 0 类型差异;夹具 **91** 个(新增 `syncthrows`),**90 AGREE** + `ffistruct`。夹具里那个文件用的是裸 `catch` 不是 `on FileSystemException`——这个编译器**拒绝**对 `FileSystemException` 做 `is`(它不是翻译出来的类),那是另一件事,记在夹具的注释里 |
| ws1078 | **`where` 的谓词会抛,而 Rust 的 `filter` 要的是 `bool` 不是 `Result<bool>`**——work.md 第六节第 3 步。链子原来发的是 `src.iter().map(..).filter(..)`,适配器的闭包只能返回裸值,所以每个 step 的体都是在**没有失败通道**的情况下写出来的(`values.dart` 里明写着 `_failure = null`),里面一个 `throw` 就是 panic。改成**发一个 `for` 循环**——循环能带 `?`——step 仍是同一个 `_stepClosure`,只是多了个 `fallible`:返回 `Result`、当场调用、`?` 传出去;`map`/`filter`/`filter_map`/`flat_map`/`for_each` 五种 step 各一行,`flat_map` 多开一层。**只在 `_failure != null` 时走循环**——没有外层可传的地方,循环也无处可去,那里适配器照旧。返工四次,四次都是「闭包上下文的哪一位该翻」:① 参数要**带类型**(没有适配器可推,而且item是值不是引用);② `_inFlowClosure` 要关掉,否则 `expand` 体里的 `return` 被拼成 try 体的 `Ok(Some(Ok(..)))`,step 的类型成了 `Option<Result<..>>`;③ `_spellsReturn` 要**留着**——它是给 step 的值加向上转型的那一位,关掉之后 17 个 E0308(`Rc<DiagnosticsProperty<..>>` 收在 `Rc<dyn DiagnosticsNode>` 里);④ 参数**不能**进 `_refLocals`,否则体里写出 `&**element`,多脱一层(E0614)。 | `.unwrap()` **1,617 → 1,180**(`.crate/src`,−437,计划里数的是 442;`.crate-ws` 1,602 → **1,165**);生成侧 `uncaught Dart exception` **8 → 7**,`panic!` 合计 7,211 → **7,210**;拒绝 12、桩 **36 逐条相同**(gone 0 / new 0)、unstubbable 0、可达 69、0 error;run1078 连采五次 707 节点 / 0 panic / 0 类型差异;夹具 **92** 个(新增 `stepthrows`),**91 AGREE** + `ffistruct`。`allfx.sh` 在这一轮抓到两个链子看不见的红(`shadowedcell`、`mixiniterable`),两个都是上面那四条里的 |
| ws1079 | **prelude 里的 `panic!("uncaught Dart exception")` 归零**——work.md 第六节第 6 步收尾,外加第 5 步那两处。JSON 那一对是最后三处:解码器的 `fail(&self, what) -> !` **存在的唯一理由就是 panic**,而 Dart 的 `jsonDecode` 对坏输入抛 `FormatException`;编码器的 `json_key` 遇到非字符串的键要抛 `JsonUnsupportedObjectError`,可它返回 `String`,没处放。整条链改成 `Result`:`JsonPiece::json_write`、`JsonKey::json_key`(共十五个 impl)、解析器的 `fail`/`expect`/`revive`/`value`/`object`/`array`/`number`/`hex4`/`string`、`JsonCodec::encode`/`decode`、顶层 `jsonEncode`/`jsonDecode`。生成侧那两处 `NoSuchMethodError`(槽分派里一个类没有 `[]=`)是**前端自己写的 `panic!` 字面量**,改成 `IrThrowValue`,交给后端的 `_thrown` 按上下文拼。**又一次名字撞车,这次最贵**:我把 `encode` 和 `decode` 一起按接收者放行,可 `Utf8Codec.encode(String) -> Vec<u8>` 根本不会失败——`?` 加上去之后 `leaf_dart_ui` 编不过,**可达 crate 69 → 25**,桩「少」了 28 个(它们跟着 44 个 crate 一起没了)。只放行 `JsonCodec.encode` 之后回到 69。 | **prelude `uncaught Dart exception` 3 → 0**(计划起点是 32),prelude `panic!` 合计 23 → **19**;生成侧 `uncaught` **7 → 5**,`panic!` 合计 7,210 → **7,208**;`.unwrap()` 1,165 不变;拒绝 12、桩 **36 逐条相同**(gone 0 / new 0)、unstubbable 0、可达 69、0 error;run1079 连采五次 707 节点 / 0 panic / 0 类型差异;夹具 **93** 个(新增 `jsonthrows`),**92 AGREE** + `ffistruct` |
| ws1080 | **最后那 5 处 `uncaught Dart exception` 根本不是 Dart 的异常,是标签贴错了**——work.md 第六节第 5 步收尾。槽分派(`IrDynamicDispatch`)的 `else` 分支写着「a dynamic slot held an unexpected type」:可那些 arm **就是这个编译器的槽普查给出的答案**,落到 else 说明**普查不全**——按 work.md 自己的判据(「这个 panic 说的是那个程序的事实,还是这个翻译器的事实?」)这是翻译器的事实,该炸,但该说自己是谁。改成 `panic!("dart2rust: a dynamic slot held a type the census did not predict")`。**这一条没有夹具,因为它什么行为都没改**——panic 还是 panic,只是不再冒充 Dart 的异常;能量的是标签本身,下面那两个数就是。 | **生成侧 `uncaught Dart exception` 5 → 0**(work.md 起点 16),`panic!("dart2rust:` 41 → 46,`panic!` 合计 7,208 不变 = native 7,162 + dart2rust 46;`.unwrap()` 1,165 不变;**至此 work.md 六步全部落地:`uncaught Dart exception` 生成侧 16 → 0、prelude 32 → 0,`.unwrap()` 29,439 → 1,165(−96.0%)**;拒绝 12、桩 **36 逐条相同**(gone 0 / new 0)、unstubbable 0、可达 69、0 error;run1080 连采五次 707 节点 / 0 panic / 0 类型差异;夹具 93 个,92 AGREE + `ffistruct` |
| ws1081 | **体积尺子(`/tmp/work_size.md` 第 §0 节)**——四把尺子没有一把量体积,而唯一一次手工量就量错了(拿 debug 比 release,`8d3b5e51` → `cda614f9` 作废)。`bin/size_ruler.py`:release 构建 → 拷出来 strip → 报段大小、按 crate、逐字节重复、单态化多出。两条规矩写进脚本的文档里:**release 只跟 release 比**(`run_main.sh`/`render_ruler.py` 跑的是 debug),**它要跑 cargo,所以不许跟链子/`allfx.sh`/`render_ruler.py` 并排**。产物进 `.build/size/`(不出工程目录)。计划 §九 的两个坑都照抄进来了:`readelf` 的 `[ 6]` 那一列要先剥掉(位数不同会错列),`nm --size-sort -t d` 只有三列(求和用 `$1`)。**这一步不改编译器一行代码**,所以另外三把尺子没跑。 | 基线(release + strip):`.text` **75,158,528**、`.rodata` 4,154,464、`.data.rel.ro` 12,226,408、`.eh_frame` 9,998,024 + hdr 2,298,868、文件 **135,833,000**(未 strip 218,010,464);符号 405,637 个 / 84,097,538 字节,前三名 `scc_flutter_widgets` 25.4%、`gallery_above` 23.5%、`scc_gallery` 16.7%;**逐字节重复 135,326 份 / 11,473,321 字节 / 13.6%**;**单态化多出 121,402 个 / 14,050,471 字节 / 16.7%**。和计划里手量的那组(135,005 / 11,363,057 / 13.5% 与 121,973 / 13,976,452 / 16.7%)对得上,差的是这中间落地的九轮 |
| ws1082 | **F:release 不发展开表**(`work_size.md` 基建组)。`panic = "abort"` 之下没有任何东西会 unwind,而 `.eh_frame` + hdr 是 stripped 文件的 9.1%。Cargo 没有对应的 profile key,所以这面旗只挂在**尺子自己那次 release 构建**上(`-C force-unwind-tables=no`)——**不许进全局 `RUSTFLAGS`**,那是 cargo 指纹的一部分,会把链子的增量 debug 全打翻。**计划估 12.4 MB,实际 6.6 MB,差的一半是 std**:`force-unwind-tables=no` 只作用于这次编译的 crate,`core`/`alloc`/`std` 是预编译进来的,它们的表还在(要拿掉得 `-Z build-std`,不在本计划里)。 | `.eh_frame` **9,998,024 → 4,833,472**、hdr **2,298,868 → 865,988**(合计 −6,597,432);stripped 文件 **135,833,000 → 129,235,576**(−4.9%),未 strip 218,010,464 → 211,412,760;**`.text` 75,158,528 一个字节没动**(计划预言的就是这个),`.rodata`/`.data.rel.ro`、重复 13.6%、单态化 16.7% 全部不变。另外三把尺子没跑:这一轮只动 `bin/size_ruler.py`,而它们量的是 **debug** 那份产物,这面旗碰不到 |
| ws1083 | **E:链接期折叠逐字节相同的函数**(`work_size.md` 基建组)。盘上有 `/usr/bin/ld.gold`(PATH 上那个 `lld` 是 Android SDK 的,别用),`-C link-arg=-fuse-ld=gold -C link-arg=-Wl,--icf=safe`。**计划 §八 诚实地记着「实际能折的是那 11.47 MB 的一个子集,没量」,现在量了:`safe` 只折掉 11%**。`all` 也量了(计划说「先 safe,再考虑 all」),它折掉 4.9 MB——四倍。**但 `all` 没有采纳**,理由不是体积而是**没有尺子能验它**:夹具各自建自己的 crate(debug、不 icf),渲染树尺子跑的也是 debug,**release 的链接这四把尺子一把都碰不到**。风险本身我是读代码核过的:prelude 里**每一处身份比较都是数据指针**(`Rc::as_ptr`、`&self as *const Self`,连 `dart_function_same` 拿的都是闭包数据的 `Rc`),而 ICF 折的是代码——两者不相干。但「读过」不是「量过」,所以尺子的默认停在 `safe`,`all` 的读数记在这里。 | `safe`:`.text` **75,158,528 → 73,912,722**(−1,245,806 / −1.66%),stripped 文件 129,235,576 → **128,326,632**(−0.7%),`.rodata` 4,154,464 → 4,431,560(gold 的布局不一样),符号 405,637 → 404,625。`all`(量了未采纳):`.text` → **70,242,786**(−4,915,742 / −6.5%),stripped → **124,259,304**(−3.85%),符号 → 367,677。另外三把尺子没跑:这一轮只动 `bin/size_ruler.py`,它们量的是 debug |
| ws1084 | **A(`work_size.md` 翻译组第一条)量完之后不成立:那条规则这个发射器**早就**在做了**。计划说「一个字符串 27 个函数」,其中 9 份是「逐字节相同、一个子类一份」的体——**那 9 份是上游自己写的**:Flutter 的 gen-l10n 把**每一条消息都写进每一个 locale 类**,`gallery_localizations_en.dart` 里 `String get deselect => 'Deselect';` 出现 **9 次**,九个类**每一个都 `@override`**。夹具 `inheritedmember`(基类一个 getter、三个子类不 override、第四个 override)把发射器的行为钉死了:`base_super_label` 一个自由函数、trait 里一个默认体,**`First`/`Second`/`Third` 三个类一个字节都没发**,只有 `Fourth` 有自己的体加一个转发。`emit_impl.dart` 的 `required` 本来就是「抽象的 + 这个类自己 override 的」,不 override 的落在 trait 默认体上。 | 编译器自己的倍数是 **3.00×**,不是 27:`l10n_gallery_localizations_en.rs` 有 **21,660** 个 `fn`,上游那份 Dart 声明了 **7,218** 个成员,21,660 ÷ 7,218 = 3.00——一个体 + 每个它要答的 trait 一个转发(自己的和祖父的)。`fn deselect` 出现 26 次 = 9 体 + 17 转发。**所以 A 没有代码改动**,`.text` 一个字节没动(75,158,528),重复 13.6%、单态化 16.7% 原样;夹具 **94** 个(新增 `inheritedmember`,AGREE)——其余三把尺子没跑,这一轮没动编译器一行代码。**连带作废计划里 A 的 ~10 MB 预估,和「B 之前 A 必须先落地」那条前置条件**(A 不改任何东西,读数污染不存在) |
| ws1085 | **C:一串同构的构造调用变成一张 `static` 表加一个循环**(`work_size.md` 翻译组)。代码浏览器是 126 个函数、13.8 MB `.text`(全部函数符号的 16.4%),每一个都是同一个形状:一行源码一个 `TextSpan::new(Some("<源码>"), None, code_style.<某个样式>, None×8)`,每个 span 302 字节机器码。**判据先写下来再写代码**(计划反复警告这条最容易写成 code-viewer 特判):≥16 个元素、每个都是同一个类同一个构造函数同样参数个数;每个参数位置要么处处同文、要么处处是 Rust 字符串字面量(载荷,至多一列)、要么至多 8 个不同的文本(选择,至多两列);至少有一列真的在变。**第一次跑全 gallery 只出了 1 张表**——没猜,加了 `DART2RUST_TRACE_TABLE` 插桩,一行就看明白了:**69 个长列表卡在「first is IrUpcast」**,元素外面裹着列表元素类型的向上转型(`List<InlineSpan>` 装 `TextSpan`)。穿过那层之后 **64 张表**。 | **`.text` 75,158,528 → 61,533,728(−13,624,800 / −18.1%)**;按计划要求两边一起报:`.rodata` 4,154,464 → 4,173,032(+18,568)、`.data.rel.ro` 12,226,408 → 13,386,776(+1,160,368,表里是 `&str` 指针,带重定位),**三段净省 12.44 MB**(计划估 9–11 MB);stripped 文件 129,235,576 → **118,041,912**(−8.7%);`gallery_above` 19,733,825 → 6,057,229;生成的 `codeviewer_code_segments.rs` 源码 6,539,515 → **767,343** 字节(−88%);拒绝 12、桩 **36 逐条相同**(gone 0 / new 0)、unstubbable 0、可达 69、0 error;run1085 连采五次 707 节点 / 0 panic / 0 类型差异;夹具 **95** 个(新增 `constanttable`:20 个合判据的 + 20 个参数是调用的不合判据的,生成的 Rust 里正好一张 `static __TAB`),**94 AGREE** + `ffistruct` |
| ws1086 | **`--icf=all` 收下了,因为终于有尺子能验它**。ws1083 把它量出来却没敢采纳,理由是「四把尺子一把都碰不到 release 的链接」——那就给尺子加一条:`size_ruler.py --run` **拿 release 那个二进制**跑 `DART2RUST_OS=android DART2RUST_DUMP_RENDER_TREE=1`,按 `render_ruler.py` 一模一样的口径(去掉 `size=`/`offset=` 那两个 headless 产不出的布局数)和参照树对。先验 `--icf none` 的 release 自己走不走得通(**707 / 0 / 0**,走得通——顺带说明 release 版本身就能当尺子用,`workspace.py` 里那条「要 release 尺子得重新考虑 `panic = "abort"`」的注记可以放下了),再验 `all`。 | **`--icf=all` 的 release 跑出 707 节点 / 0 panic / 0 类型差异**——这比计划要的「身份夹具不变红」更强:验的就是这面旗改的那个产物本身。体积(都在 C 之后、都带 `-C force-unwind-tables=no`):`.text` **61,533,728 → 56,555,618**(−4,978,110 / −8.1%),stripped 文件 118,041,912 → **113,003,496**(−4.3%),符号 406,651 → 368,325,`.rodata` 4,173,032 → 4,449,648(gold 的布局)。**从 work_size.md 的起点算起:`.text` 75,158,528 → 56,555,618(−24.8%),stripped 135,833,000 → 113,003,496(−16.8%);对官方 `libapp.so` 的 `.text` 12,291,088 从 6.11× 降到 4.60×** |
| ws1087 | **D 和 B 的头寸量清楚了,两条都没做,理由是价钱不是机制**——`work_size.md` 第八节「没量的」里点名的两项(「D 里有多少 `T` 真能擦」「B 跟 A 的读数互相污染」)。**先把「单态化」这个数拆开**:计划里那 13,976,452 字节是按「剥掉泛型实参之后同名」分的组,而那样会把**同一个函数在多个 crate 里的副本**(`ThemeData::clone` 21 份、`ColorScheme::clone` 34 份——它们根本不是泛型)算成单态化。按「组内的**全名**是否真的不同」分开之后:真单态化 = std 3.34 MB + **翻译代码 3.07 MB** + prelude 1.35 MB;跨 crate 重复的那一半是 E(ICF)的活,`--icf=all` 已经收掉了。**D 的真实上限是 3.07 MB,占 `.text` 的 5.4%,不是计划写的 14 MB。**排头的全是 Flutter 的值类型 widget(`_RadioListTileState<T>::build` 6 份 348 KB、`PopupMenuButtonState<T>` 5 份、`_RadioState`/`_RadioGroupState`/`_AnimatedEvaluation`),`T` 确实只被「存、比、传」——**机制是对的**。不做的理由是:擦一个类型参数要在**每个读出点插入向下转型**(擦进去是向上转型,读出来 Dart 那边的静态类型是具体的),那是新的一整套 coercion,而押着的是 5.4%,对面是 36 桩 / 69 crate 这个均衡。**B 同理**:l10n 是 **19,437,406 字节,占全部函数符号的 28.8%**,C 和 ICF 一个字节都碰不到它(每个体的字符串都不一样),而 `.to_string()` 在生成代码里有 **139,450 处**——每个体已经接近「分配 + memcpy」的下限了,**省不下来的是 `String` 这个表示本身**,所以 B 只能是那个全程序类型改动(计划自己说的「跟 `6d65920e` 同一个形状同一个量级」),再加上 prelude 里整片 Dart `String` API 的重写。**这一轮不改一行代码**。 | 读数全部取自 C + `--icf=all` 之后那次 release:`.text` 56,555,618;真单态化(翻译)**3,074,216**、prelude 1,345,197、std 3,343,231;l10n **19,437,406 / 28.8%**;生成代码里的 `.to_string()` **139,450** 处。尺子一格没动(没有代码改动),夹具 95 个 94 AGREE |
| ws1088 | **D 动手了,四刀,尺子四次都不收,撤回**(`work_size.md` 翻译组)。好消息是机制现成:擦除**本来就在**(`_erasedParameter`),只是判据是「程序把这个参数**协变**地用过」——那是**正确性**规则(Rust 的 trait object 不协变),不是体积规则。D 要的是把同一套擦除按体积也用上。四刀,每一刀都是尺子教的:① 凡是界是句柄的都擦 → **桩 655、可达 39**(方法自己的类型参数 `mergeSort<T>` 是靠实参推出来的,擦了就没得推);② 只擦**类**的参数 → 桩 139、unstubbable 12、可达 67(`cannot multiply f64 by Rc<dyn DartAny>`——有实例化给的是数,而体里在算术,正是计划说的「要具体布局就不擦」);③ 加一趟 `scalarParameters` 预扫,标掉任何被 `int`/`double`/`num`/`bool` 实例化过的参数 → 桩 79、unstubbable 5、可达 66;④ 标记沿继承链两个方向传到不动点(`ConstantTween<T> extends Tween<T>`,擦了子类没擦基类,`ConstantTween` 就不再 `Animatable<f64>` 了) → **桩 57、unstubbable 1、可达 66**。收敛是真的(655 → 139 → 79 → 57),但**没到 36**,而剩下的错(`no field 'text' on Rc<dyn DartAny>`、`no method 'insert'`)要的是计划里那句话的正脸:**体里有没有用到 `T` 的具体布局**——那是一趟带静态类型的体扫描,又是一个新分析。押着 3.07 MB(5.4%),对面是 36 桩 / 69 crate,**收手**。 | 四次读数:桩 **655 / 139 / 79 / 57**、unstubbable **—/12/5/1**、可达 **39/67/66/66**;基线 36 / 0 / 69。撤回后重跑确认:**拒绝 12、桩 36 逐条相同(gone 0 / new 0)、unstubbable 0、可达 69**。`scalarParameters` 那趟预扫也一并撤了(量不出来的改动不留下);要接着做的话,前置测量、四个失败模式、和判据的正脸都在这一行里 |
| ws1089 | **B 动手了,走了一大半,撤回**(`work_size.md` 翻译组最后一条)。**先把账算准**:计划说 l10n 的 19.4 MB 是 165,135 个体各自 `String::from` 一个字面量。按符号形状拆开:**trait impl 161,701 个、15,616,914 字节(80%)、每个 96 字节**;`_super_` 自由函数 326 个、2.3 MB;内在体 3,108 个、1.5 MB。反汇编一个「转发器」,它根本不是转发:`movabs $0x7463656c65736544`(「Deselect」那八个字节)、`alloc`、`memcpy`、写三个字的 `String`——**被内联了,所以 15.6 MB 全是字面量分配**,B 的前提成立。做法:`DartStr = Cow<'static, str>`,字面量发 `dart_text("..")`(一个指针加一个长度,不分配)。**prelude 那边比想象的小得多**:Dart 的 String API 是**一个 trait 一个 impl**,把 `impl DartString for String` 改成 `for str`(`Cow` 靠 deref 就到),`dart_nullable!(DartStr)` 一行给齐 `DartNullable` + `FromDynamic`;发射器只有六处(字面量、插值、`join`、`_downcastNames`、`_copyText`、前端那个 `"null"`)。五轮下来夹具 **60 → 64 → 70 → 71 → 更多**,但剩下的是 prelude 里 70 个 `-> String` 和 51 个 `String` 参数——哪些是「Dart 的字符串」哪些是内部缓冲要一个一个认——**而夹具通了之后,gallery 那条尾巴还没开始**。**收手,整轮退回**:留一半在树上就是把尺子扔了。 | l10n 拆账:**trait impl 161,701 / 15,616,914 字节 / 96 字节一个**、`_super_` 326 / 2,290,166、内在体 3,108 / 1,530,326;中途读数:夹具 95 个里 AGREE **60 → 64 → 70 → 71**,prelude 自己 0 error;撤回后重跑确认:**拒绝 12、桩 36、unstubbable 0、可达 69**。**要接着做的话,难点已经定位**:不是 prelude 的 API(那是一个 impl),是 `String` 和 `DartStr` 的边界——prelude 的 121 处签名,加上 gallery 自己的那条尾巴 |
