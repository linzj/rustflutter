...
| ws282–ws286 | 950 叶子错 | **closed world 的 trait 转换**：`DartAny` 加 `fn dart_cast(&self, TypeId) -> Option<Box<dyn Any>>`（默认 `None`），每个 struct 的 `impl DartAny` 里生成：自己的类型给 `dart_self_ref().get()`（非泛型值类 `Rc::new(self.clone())`），`_abstractAncestors` 里每个 trait 给 `Box::new(self.dart_self_<trait>())`；扩展 trait `DartCastExt::dart_cast_to::<dyn X>()`，对 `S: DartAny`、`Rc<S>`、`Option<S>` 都实现（方法调用，接收者是句柄/引用/值都行；`x is Foo` 对 `Foo?` 问的是里面的值）。用处三个：`is` 对 trait 不再拒绝（`_isTest`，refusal 1047→887）；局部变量提升到抽象/开放类后的读（`if (x is Foo) x.bar`）包 `IrCastTo`；抹掉参数后 `widget` 静态类是开放类时的读也走它。**第一版给 `Rc<T>` 也做了 `impl DartAny`，与 `Object` 的 blanket 在同一层撞出 5136 个 `as_any` 二义性——去掉，方法调用自动解引用就够。** 读数（主线，不抹）：ws279 5247 stub / 1047 refusal → ws286 **5381 / 887**：少拒 160 个函数，其中 134 个编不过——`stub+refusal` 6294→6268。抹掉打开：5341→5443/883。**尺子从这轮起记两个数**：refusal 也是没译出的函数。还剩的 cast 失败 56 个里 50 个接收者是 `Rc<dyn Object>`：`Object` 是 blanket impl，没法按类型答 `dart_cast`——要一个运行时的 `TypeId → 转换函数` 注册表，`dart_rc` 和 `!rc_object` 处登记（下一步）。 |
| ws287 | 950 叶子错 | **`dyn Object` 上的 trait 转换**：`Object` 是 blanket impl，答不了 `dart_cast`；prelude 加一张 thread-local 的 `TypeId → fn(&dyn Any, TypeId) -> Option<Box<dyn Any>>` 表（`DART_CASTS`），`dart_register::<T>()` 登记（`dart_rc`、`dart_object`——非 counted 值共享成对象时的 `Rc::new`——以及每个非 `const` 构造器第一行），`impl DartCastExt for dyn Object` 查表转。closed world 里每个 `is Trait` 能碰到的对象都是这几条路造出来的（`const` 构造的值除外——它们的 `is` 多半是对具体类的 `downcast_ref`，不走表）。主线 **5381→5366 / 887**；`dart_cast_to` 相关的错 102→47，其中 `Rc<dyn Object>` 接收者 50→5。剩下 17 个是老 bug 露头：`{ let __set = v; x = __set; __set }` 对非 `Copy` 值先搬后用（E0382）。 |
| ws288 | 950 叶子错 | 赋值当值用的两处（局部 `x = v` 和 `a.b = v`）存进去的改成 `__set.clone()`，最后交出 `__set`：**5366→5358 / 887**（E0382 那 17 个没了；多了 13 个 "type annotations needed"——`let __set = None; x = __set.clone()` 这种，`None` 的类型要从用处推，下一步看）。 |
| ws289–ws290 | 950 叶子错 | **抹参数的三刀补完，翻默认**。ws289：① 泛型值 struct（`Router<T>`）的下转不再 `.clone()`（派生的 `Clone` 要 `T: Clone`），当引用读，字段读处本来就克隆字段（`_narrowingCast`）；② 闭包参数：槽的声明类型是被抹参数时，闭包按上界取参（`_closureParamType`），读该参数时按声明的类下转/转 trait（`VariableGet` 看 `_retyped`）；位置实参的槽是提到被抹参数的函数类型时用声明类型而不是实例化后的（`_mentionsErased`）。抹开：5443→5381/883（closure mismatch 95→55，move out of shared ref 39→7）。ws290：读的结果类型是读者自己的被抹参数（`ImplicitlyAnimatedWidgetState<T>` 读 `State<T>.widget`，`T` 是 `TypeParameterType` 不是类）时按其上界转：**5352 / 883**，`Rc<dyn StatefulWidget>` 上的字段读 80→0。主线 ws288 是 5358/887——抹参数两个数都赢了，`DART2RUST_ERASE` 默认开（`=0` 关）。剩：tear-off 适配闭包（kernel 合成的 `#wc0_formal`，按方法的 `Intent?` 取参）对被抹的槽（`Rc<dyn Intent>`）仍不匹配（E0631 里 `fn(Rc<_>)` vs `fn(Option<Rc<_>>)` 9 + `TapUpDetails` 5）。 |
| ws291 | 950 叶子错 | **trait 上的泛型方法**（`where Self: Sized` 让它们在 `dyn` 上消失，180 个 stub：`BuildContext.dependOnInheritedWidgetOfExactType<T>` 89、`Animation.drive<U>` 46、`invokeMethod<T>` 19…）。先用 probe 数 closed world 里的实现体：35 个这样的方法，`BuildContext` 那一族都只有 `Element` 一个体，`drive` 只有 `Animation` 一个，`getElementForInheritedWidgetOfExactType`/`invokeMethod` 各两个。于是：调用点的接收者静态类是 trait、方法是泛型、全程序恰好一个体且那个体所在类也是 trait 时，前端发 `IrSuperDispatch`——后端印成那个体的 super fn 调用 `element_super_depend_on_..::<_, T>(&*recv, args)?`（`__Self` 和类参数留 `_` 推，方法参数写实参；接收者不是该类的子类型时先 `dart_cast_to::<dyn Element>()`）。用 kernel 的 `ClosedWorldClassHierarchy.computeSubtypesInformation().getSubtypesOf` 找体，`isSubInterfaceOf` 判是否要转。**5352→5214 / 882**（那一类 180→28）。剩：两个体的（31）要 `IrDynamicDispatch` 逐个 `downcast_ref` 派发、且具体 struct 的 inherent 泛型方法要 turbofish；跨模块的 super fn 名字（`canonicalized_map_super_cast`，24）import 没跟上——import 是按模块正文里出现的名字在**已引用模块**里找的，被调的 super fn 所在模块本来就不在引用列表里。 |
| ws292–ws294 | 950 叶子错 | 三件小事加一件结构的。ws292：① import 解析在**已引用模块**里找不到定义者、但整个 crate 只有一个模块定义该名时就用它（`rendering.dart` 这种 barrel 库降成空模块，可见链在它这儿断了；`VerticalDirection` 48、`BorderStyle` 33…，"cannot find type" 125→31）；② `_narrowingCast` 的下转目标过 `_rustScalar`（`downcast_ref::<int>` 31）；③ `!rc` 对 `this`：trait 体里给 `dart_self_<trait>()`，counted 给 `dart_self_ref().get()`：5214→**5154**。ws293：`!rc_object`（`this` 共享成 `Object`）同样改走自己的句柄——`Rc::new(this_)` 82 处没了（"lifetime may not live long enough" 168→87），但换成 81 个 non-primitive cast：`Rc<dyn Simulation> as Rc<dyn Object>` 不行，因为 `Object` 不是 trait 的超 trait——之前每个 trait 都发了一份 `impl Object for dyn X`，那只是 impl，给不了 unsize 强转。ws294：**`DartAny: Object`**（超 trait；`as_any` 归 `Object` 的 blanket，`DartAny` 不再声明；`impl Object for dyn X` 撤掉——有超 trait 时它是 E0371；blanket 的 `runtime_type` 去掉泛型实参只留 struct 名，和 `dart_runtime_type` 一致）：**5153→5071 / 882**（non-primitive cast 105→21）。 |
| ws295 | 950 叶子错 | "the trait bound `Rc<X>: Trait` is not satisfied" 606 处里 510 处是一个形状：`state_super_dispose(self)`——super fn 要 `&__Self`，而这个方法的 `self` 是 `&Rc<Self>`（`_receiverOf` 给交出 `this` 的方法用句柄接收者），得 `&**self`；闭包里的 `__me` 是句柄（counted）要 `&*__me`，值类要 `&__me`。`_superCall` 按 `_selfName`/`_selfIsHandle`/`counted` 拼接收者，和 `_call` 的 qualified 路径同一套：**5071→4712 / 882**（trait bound 739→282）。剩 96 个 `Rc<..>: Trait`，79 个是 `.as_ref().map(\|it\| ..)` 里 `it` 是 `&Rc<..>`、`&*it` 少解一层引用。 |
| ws296 | 950 叶子错 | qualified 调用的接收者拼写补两种：`IrCastTo` 的结果永远是句柄（`&*`）；null-aware 里绑定的 `it` 是 `&T`——句柄类要 `&**it`，值类直接传 `it`：**4712→4652 / 882**（`Rc<..>: Trait` 96→8）。 |
| ws297–ws298 | 950 叶子错 | 三刀。① 工厂构造器带着类参数的副本（kernel 里 factory 的 `typeParameters` 是类的拷贝，`declaration` 是 Procedure 不是 Class）——`_erasedParameter` 对 factory 按下标查类参数是否被抹；静态/实例方法的 `typeParameters` 也过这一层：`global_key_new<T>` 的 `T` 没了（"cannot infer T" 87→0）。② `IrCall.typeArguments`：译出类上的泛型方法（`bucket.remove<P>(id)`，`P` 只出现在返回值）调用时印 turbofish；`_construct` 把 `constructedType` 的（过滤后的）实参带进 `IrNew`，`_new` 本来就会印 `Foo::<T>::new`。③ `this.x` 而 trait 声明了 `x`、**降到当前类里的**类覆盖了它（Rust 会解析到 inherent 的窄类型，kernel 按声明类型算——`String? get restorationId` 被覆盖成 `String`，`.unwrap()` 找不到 72）：`_qualified` 里用 `_lowering`（mixin 的体是降进应用它的类里的，`_member` 指向 mixin 自己）判有覆盖就 `Trait::x(self)`。**4652→4574→4423 / 882**。turbofish 暴露了 43 个 `ObserverList::<Rc<dyn Fn>>::new()`——`impl<T: PartialEq>`，`dyn Fn` 不能 `PartialEq`（orphan 规则，ws60 就撞过）；监听器列表要按 `Rc::ptr_eq` 比，独立一刀。 |
| ws299 | 950 叶子错 | 闭包身份比较那一族的便宜半步：prelude 的 `Map`/`Set` 把不比键的方法（`new`/`len`/`keys`/`values`/`entries`/`for_each`/`clear`…）挪进只要 `Clone` 的 impl；后端对带键界（`T: PartialEq`）的泛型 struct 把构造器和常量放进不带键界的 impl 块，方法留在带键界的块里——`ObserverList<Rc<dyn Fn>>` 在每个持有者那里都**造得出来**了，只有比较元素的方法够不着：**4423→4388 / 882**（"associated function exists but trait bounds" 43→2）。`Set::of` 去重、要比，第一次挪错了（prelude 多 1 个 stub），挪回（prelude 单独 check 只剩 `run_until_idle` 里那两个老错）。真正的解法记着：`Rc<dyn Fn>` 的 `PartialEq` 撞 orphan 规则，要么回调类型换成 prelude 自己的 `dyn DartFn<A, R>`（blanket impl 接住闭包，`impl PartialEq for dyn DartFn` 按身份），要么 `Map`/`Set` 改用 `DartEq` 比键（blanket `impl<T: PartialEq> DartEq for T` 和 `Rc<T>` 那条会重叠）。 |
| ws300 | 950 叶子错 | setter 调用的二义性（`RestorableValue` 和 `_RestorablePrimitiveValue` 都声明 `set value`，`RestorableBool` 两个 impl 都有 `set_value`，77）：`IrSetter` 加 `qualifier`/`receiverClass`，前端在三处构造（表达式形、语句形、`this` 上）用 `_qualifierFor(接收者静态类或 `_lowering`, interfaceTarget)` 算，后端有 qualifier 时走 `_call`（`Trait::set_x(&*recv, v)?`，接收者拼写沿用）：**4388→4347 / 882**（E0034 124→47，`set_value` 归零）。 |
| ws301 | 950 叶子错 | 两件小事。① `_operatorTraits` 只映射了算术五个：`Offset & Size` 发了 inherent 的 `bit_and` 却没有 `impl BitAnd`，调用点又写成 Rust 的 `a & b`（103 个 E0369）——补 `&`/`\|`/`^`/`<<`/`>>` 到 `BitAnd`/`BitOr`/`BitXor`/`Shl`/`Shr`。② prelude 的 `DartDouble` 和 `Comparable<f64>` 都给 `f64` 一个 `compare_to`（22 个二义性）——`DartDouble` 的去掉。**4347→4264 / 882**。E0034 还剩 30（`SourceLocation`/`SourceLocationMixin` 的 `source_url`，`CupertinoThemeData`/`NoDefaultCupertinoThemeData` 的 `text_theme`，`ModalRoute`/`PageRoute` 的 `fullscreen_dialog`——都是 getter 在两个 trait 里各声明一次，`_qualifierFor` 该管却没管到，下一步看是不是 `_instanceGetRaw` 的 `IrField` 路径绕过了它）。 |
| ws302–ws304 | 950 叶子错 | **类的类型参数要 `Clone`**（`T`/`Option<T>` 上 `.clone()` 找不到，约 240 个 stub）。ws302 第一刀把 `_generics(static)`、`_implGenerics`、super fn 的类参数都写成 `Clone + 'static`：读数掉到 2794 但只到 33 个 crate——`trait _CallbackHookProvider<T: Clone>` 被 `_CallbackHookProvider<Future<bool>>` 命名（`Pin<Box<dyn Future>>` 不 `Clone`），trait 声明外的错整 crate 死。ws303：trait 头上不放 `Clone`，需要克隆的**默认方法**各自 `where T: Clone`（`_traitWhere`，和 `Self: Sized` 合并）；super fn 的方法级参数也补 `Clone`。还剩两个：trait 访问器返回 `Vec<TweenSequenceItem<T>>`，struct 声明上的 `T: Clone` 在任何**提到**该类型的地方都被要求。ws304：声明层（struct/trait/标记 impl）一律只 `'static`，`Clone` 只在真会克隆的 impl 块（`_implGenerics`、super fn、`_traitWhere`）上：**4264→4213 / 882，138 个 crate 全到**（"no method named clone for type parameter" 82→6）。附带：`FixedScrollMetrics with ScrollMetrics` 的 trait 没把 mixin 当超 trait（39 个 `__Self: ScrollMetrics`）——`supers` 加 `cls.mixins`。新露出："the method exists for reference but its trait bounds were not satisfied" 17→208。 |
| ws305 | 950 叶子错 | ws304 露出的 208 个 "method exists for reference but trait bounds"：`impl<E: 'static> DartAny for DefaultEquality<E>` 里的 `dart_cast` 调 `self.dart_self_equality()`，而 `impl<E: Clone> Equality<E> for DefaultEquality<E>`——`DartAny`/`DartSelfRef` 的 impl 块改用 `_implGenerics(cls, keyed: false)`（和 inherent impl 一样带 `Clone`）：**4213→4025 / 882**。下一刀：`_listeners`/`_count` 在 `ChangeNotifier` 和应用它的开放类（`FocusNode` 等，mixin 字段降进了它的 `fields`）两个 trait 里各有一个访问器，trait 体里 `self._listeners()` 二义（E0034 57 里 ~50）——`IrField`/`IrAssignField` 要记声明类，访问器模式下按声明的 trait 限定。 |
| ws306–ws307 | 950 叶子错 | `_listeners` 的二义（50）：mixin 的字段降进了应用它的类，那个类又是开放类→trait，于是 `trait FocusNode: ChangeNotifier` 里又声明了一遍 `_listeners` 访问器。改成 trait 不声明抽象上位类型（超类/mixin/接口，传递闭包）已经声明的字段访问器，实现者的 impl 块也照此跳过（第一次只改了 trait 那头，impl 里多出的 `fn` 成了 "not a member of trait"，63 个声明外的错把 widgets crate 整个弄死——读数 2836 是假的，`cargo check` 只到 33 个 crate；两头一起改后）**4025→4008 / 882，138 个 crate 全到**。 |
| ws308–ws311 | 950 叶子错 | 剩下的 E0034（39）两类。ws308 试了让 trait 不再声明抽象上位类型已要求的**方法**（`SourceLocation.sourceUrl` 在 `SourceLocationMixin` 里又要求一遍，13）——前端已经把 qualified 调用写成 `ScrollPosition::jump_to(this_)`，trait 上一撤，路径就成了 "expected a type, found a trait"（+220，4179），**撤回**。ws309–ws311：trait 体里对 `this` 的访问器读/写/`_cell` 按声明者限定（`_accessorQualifier`）——读同名的 getter 覆盖取**最近**的 trait（Dart 的虚分派），字段的访问器取**最上面**的声明者（ws306 起 trait 不重复声明上位类型的字段，`FocusNode.fields` 里虽有 mixin 的 `_count`，声明它的是 `ChangeNotifier`；第一版取了 `FocusNode`，163 处 `FocusNode::_count(this_)` 找不到项）；写和 cell 只在那个 trait 真声明了 `set_x`/`x_cell` 时才限定：**4008→3996 / 882**（E0034 39→16）。 |
| ws312–ws313 | 950 叶子错 | 两刀一撤一留。撤：`_widened` 里 counted 类的值进 trait 槽时包 `IrUpcast(handle: true)`（`(x as Rc<dyn T>)`）——`_closureCallsMethod` 说的是类 counted，表达式却可能是 struct 值（`ScaffoldState as Rc<dyn State>`，79 个 non-primitive cast），而目标 `Set<Ticker>.remove(&ticker)` 那 45 处根本不走 `_widened`（是 `!contains`/`remove` 那条路），4060 撤。留：局部变量提升到**枚举**也下转（`if (x is _MediaQueryAspect) switch (x)` 之前对着 `Rc<dyn Object>` 匹配枚举臂，29 处）：**3996→3995 / 882**——那 29 处集中在几个函数里，尺子只动一格。`IrUpcast.handle` 留在 IR 里备用。 |
| ws314–ws319 | 950 叶子错 | **`Object` 上界的协变**（`AssetImage: ImageProvider<Rc<dyn Object>>` 44、`RestorableX: RestorableProperty<..>` 77）试了一刀：上界是 `Object`/`Object?` 的 trait 类参数，closed world 里有子类把它固定成具体类型（`_fixedBelow`，`getClassAsInstanceOf`）就抹。三个坑顺手填了：① 只对译出的类（prelude 的 `Sink<T>` 别名被抹成无参，11 个声明外的错整个叶子 crate 死）；② 抹成上界后读处的下转，标量（`double` 在 kernel 里是抽象类）要 `downcast_ref::<f64>` 不是 `dart_cast_to::<dyn double>`（133）；③ 匿名 mixin 应用（`dart:mixin_deduplication`）自己的类型参数要映射到 mixin 的——dedup 出来的应用类 `mixedInType` 是 null，关系在 `implementedTypes` 里；映射不到时按自己的上界判（`ChildType extends RenderBox`）。全 crate 到齐后读数 **4435 vs 3995**：`Animation<double>` 的 `T` 也被抹了，值都进了 `Rc<dyn Object>`，每次读都要下转回来（mismatched +461）——**门控** `DART2RUST_ERASE_OBJECT=1`，默认关，回到 **3995 / 882**。②③ 是对的，留下。 |
| ws320–ws321 | 950 叶子错 | `Set<Ticker>.remove(ticker)` 那 45 处：Dart 的 `Set.remove(Object?)` 参数不是 `E`，`_widened` 不会往元素类型共享；prelude 的 `remove(&T)` 拿到 `&Rc<_WidgetTicker>` 又不能隔着引用 unsize。加 `_intoElement`：集合的元素实参，元素类型是 trait、实参是它的具体子类时——counted 的句柄 `IrUpcast(handle: true)`（`(x as Rc<dyn T>)`），值类 `!rc`；只对具名的值（`this` 可能是 `&self` 背后的 struct，ws312 的教训）。挂在 `contains`、`List.remove/indexOf/lastIndexOf` 和单独一条的 `Set.remove`（第一次漏了它，读数没动）：**3995→3950 / 882**。 |
| ws322 | 950 叶子错 | **两次 OOM 把 WSL2 打死了**（用户报的）。第一次：stubs.py 的 `cargo check` 全并行（32 核）旁边我又开了一个数可达 crate 的 `cargo check`；第二次 `-j 6` 也没保住——ws321 在全并行下是好的，所以是 ws322 那刀（具名参数按类型序，见下一行）让某个合并 crate 的 `rustc` 自己跑飞了。处置：① `stubs.py` 加 `-j`（`DART2RUST_JOBS`，默认 8），可达 crate 数从它自己最后一次 check 的 artifact 里数，**不再另起 check**；② `bin/run_chain.sh`：整条链在一个看 `MemAvailable` 的 watchdog 下跑（没有 sudo，用不了 cgroup），低于 `DART2RUST_MIN_FREE_GB`（默认 16）就杀掉 rustc/cargo/stubs.py，日志里记一行 `OOM-GUARD`（附正在编译的 crate 名）；③ 可疑的那刀先 stash。基线在 guard 下（`-j 4`）重跑：**3950→3943 / 882，138 个 crate，峰值 20 GB**（−7 是 prelude 给 `Duration` 补了 `Mul<f64>`：`kThemeChangeDuration * timeDilation`）。 |
| ws323–ws325 | 950 叶子错 | 第二次 OOM 的真凶不是 `rustc`：guard 触发时（可用内存跌破 24 GB）活着的 rustc 只有 1.4 GB，而 `cargo check --message-format=json` 的输出到 50 GB 还在长——gallery 里一行 1.7 MB 的 `TextSpan` 树上有几千个错，每个错的 `rendered` 都带整行；`stubs.py` 用 `capture_output` 把整轮输出攥在内存里。改成流式读、每条 `rendered` 只留 8 KB、一轮超过 `DART2RUST_DIAG_BUDGET_MB`（512）就丢掉剩下的（日志里有一行）。那几千个错本身是 ws322 那刀造的：把所有具名参数按类型序排，定义/直接调用/构造器/tear-off 适配器几处不一致（构造器那处漏了），全程序 8789——**撤回**。原问题（34 个 `FontWeight` 落进 `locale` 槽）另解：`GoogleFonts.libreFranklin(..)` 在 kernel 里是对一个**常量 tear-off** 的 `FunctionInvocation`（TFA 把 getter 折成了 `StaticTearOffConstant`），按函数类型（具名参数排过序）传参，被调的却是按声明序定义的自由函数——这种调用直接降成 `IrStaticCall`，实参按被调者自己的顺序：**3943→3935 / 882，138 个 crate，峰值 4 GB**（`Locale`/`FontWeight` 归零）。顺手留下的：静态 tear-off 流进**保留具名参数**的函数槽时包一个按类型序取参、按声明序调用的适配器（这批代码没走到，读数没动）。 |
| ws326–ws327 | 950 叶子错 | 两件小事，都在 guard 下量（峰值 4–5 GB）。① 抛 prelude 的异常：`Exception::new(..)` 是静态调用不是 `IrNew`，`_boxedThrow` 没包 `Rc::new`（29）——按 owner 在 prelude 异常名单里就包。② 值局部变量的字段写：`childParentData.offset = Offset(..)`，局部的类 `_ToolbarParentData` 是 counted（字段是 cell）却走了"裸局部、直接赋值"那条（只看了字段所在类），写进 `Rc<Cell<Offset>>`（23）——接收者的类 counted 或字段所在类是 trait 时不走那条；第一版把 trait 字段的写落进了 `_rootedAtThis` 的拒绝（refusal 882→901），补一条：字段所在类是 trait 就 `IrAssignField(owner: trait)`，后端走 setter。**3935/882 → 3888/879，138 个 crate**。 |
| ws328 | 950 叶子错 | 三件小事。① 省略的可选实参的默认值直接走了 `_intoObject`，prelude 的 `StringBuffer([content = ''])` 拿到 `Rc<dyn Object>`（21）——改走 `_intoDynamic`（它知道 `dart:` 的被调者把 `Object` 参数拼成什么）；② `Object` 进 `Object?` 槽要 `Some`（`list[i] = key` 对 `List<Object?>`，36）——`_intoObject` 里 `given` 是 `Object` 时原来原样放过；③ 抛一个**值类**的局部（`throw error`，`FlutterError` 在手里，18）：前端按静态类型包 `!rc_object`，构造出来的仍由后端 `_boxedThrow` 包。**3888→3864 / 879，138 个 crate**（峰值 57 GB，guard 没触发）。三种形状各剩一部分（14/33/13），下一轮看剩的走的什么路。 |
| ws329 | 950 叶子错 | ws328 剩的：`throw FlutterError('..')` 的构造器是 **factory**，降成静态调用不是 `IrNew`，后端 `_boxedThrow` 不认；语句形的 `throw` 又没走我在表达式形加的包法。合成一个 `_thrownValue`：抛的是译出**值类**的表达式（局部、工厂调用、方法调用）就包 `!rc_object`，`IrNew`/`IrUpcast` 留给后端，两种 `throw` 都走它；`dynamic`（`Rc<dyn Object>`，`Null` 也是对象）进 `Object?` 槽包 `Some`（`MethodCall.arguments` 进 `writeValue`）。**3864→3853 / 879，138 个 crate，峰值 4 GB**（`FlutterError` 14→1，`Option<Object>` 33→25）。 |
| ws330–ws331 | 950 叶子错 | 一撤一留。撤：`Option<Option<T>>`（`WidgetStateProperty<Color?>` 的 `T?`，约 80 处 `?` 不匹配）想在 `type()` 里把套两层的 `Option` 折成一层——错在层面：泛型 trait 用可空实参实例化时，Rust **自己**要求 impl 写 `Option<Option<..>>`（`MessageCodec<Object?>::decodeMessage`），折了 2 个声明外的错整个 widgets crate 死（只到 32 个 crate）；转发器本来就在边界上 `.flatten()`，正解在值那层，不在类型打印。留：运算符方法按 std 签名发（不带 `Result`），调它的地方却按会失败的传播——运算符体里 `_failure == null`，传播就是 `.unwrap()`，`_Vector` 的 `self[i] * a[i]` 对着裸 `f64` 解包（73）：`_fails` 对 `ProcedureKind.Operator` 一律 false。**3853→3857 / 879，138 个 crate**（+4 是首错归属挪了：`Color <- Option<Color>` 那批本来就在，只是原先被别的错挡在前面）。剩 71 个 `f64` 上的 `unwrap` 是另一形状——`as T` 把标量运算结果转成类型参数（`Tween.lerp`、`EdgeInsets.lerp`），下一轮。 |
| ws332–ws334 | 950 叶子错 | `f64` 上的 71 个 `unwrap`：以为是 `lerpDouble(..)!` 被 TFA 内联后 `NullCheck` 落在算术上——补了两刀（直接的算术、`let` 里的算术）都没动读数（3857）；probe 一看 `EdgeInsets.lerp` 的 kernel：`!` 被 TFA 改写成 `unsafeCast<double>(lerpDouble(..))`，走的是 `as double`（`double?` → `double` 同类）那条，`IrNullCheck` 套在我们把 `lerpDouble` 降成的裸算术上。抽一个 `_neverNullHere`（`lerpDouble` 调用、或 TFA 判非空的数值算术，`let` 剥掉看体），`NullCheck` 和同类 `as` 都用它：**3857→3814 / 879，138 个 crate，峰值 4 GB**（那 71 个剩 1）。 |
| ws335 | 950 叶子错 | "lifetime may not live long enough"（91）：super fn 里 `this_: &__Self`，闭包开头 `let __me = this_.clone()` 克隆的是**引用**，进 `'static` 的 `Rc<dyn Fn>` 就活不够长。`_selfHandle()`：trait 体里 `dart_self_<trait>()`，counted 类 `dart_self_ref().get()`，否则才 `clone()`；闭包前奏用它，`IrThis` 印在 `this_` 上也一律走句柄。**3814→3752 / 879，138 个 crate，峰值 5 GB**（lifetime 91→25）。剩 20 处 `this_.clone()` 走的是别的路（`objectRuntimeType(this, ..)` 的实参那种），下一轮看。 |
| ws336 | 950 叶子错 | 剩的 20 处 `Rc::new(this_.clone())` 走的是第三种共享（`!as_object`，`objectRuntimeType(this, ..)` 的实参）——`this` 是 `this_` 时和前两种一样交出 `dart_self_<trait>()`，counted 给 `dart_self_ref().get()`；`!rc` 那条也不再只看访问器标志。**3752→3733 / 879，138 个 crate，峰值 4 GB**（lifetime 25→5）。 |
| ws337 | 950 叶子错 | `_publicItemsIn` 把 `pub async fn`/`pub const fn` 也算定义者（`haptic_feedback_selection_click` 那类调用此前一个也没导入）。**3733→3714 / 879**，但 workspace 只剩 **127+1 个 crate**：多出来的边（`widgets_window` 导 gallery 的 `main`，`material_navigation_rail` 因为一个叫 `destinations` 的**字段**导 gallery 的 `destinations`，`rendering_view` 因为局部变量 `is_android` 导 `google_fonts`）把 gallery 和 `flutter_localizations` 并进 widgets 大 crate（378k→581k 行，**峰值 57 GB**）。这些边来自 ws291 的"唯一定义者"兜底，与 Dart import 图无关。 |
| ws338 | 950 叶子错 | 唯一定义者兜底只在库能看见的 **package** 内生效（`_packageOf`：`package:flutter`、`dart:ui`）；barrel 被 CFE 消掉的情况都在同一 package 里，`VerticalDirection` 那类仍能导。**3714→3714 / 879，138+1 个 crate**，widgets crate 378k→325k 行（多出一个 `merged_cupertino_material` 95k），峰值抽样 ≤19 GB。 |
| ws339 | 950 叶子错 | `is T`/`as T`，T 是**类型参数**：调用方传的是整个类型（`Rc<ScaffoldState>`、`Rc<dyn State>`、`i64`），所以按 `TypeId` 问对象——prelude `DartCastExt::dart_cast_any::<T>()`（`dart_cast` 之外再兜 `Any` 的按值克隆），生成的 `dart_cast` 也应答 `Rc<Self>`/`Rc<dyn A>` 的 id；后端记住当前方法的类型参数（`_methodTypeParams`），`_isTest`/`IrCastTo` 走 `dart_cast_any`。前端：`value is! T` 在 `T?` 上只是判空，`null is T` 为 false（gallery 没有 `of<X?>`），`T? as T` 是 `!`。**3714→3702 / 872，138+1 个 crate**（cannot find function 37→13：`find_ancestor_state_of_type`、`provider_of` 那几个 super fn 有了正文）。 |
...
| ws345 | 950 叶子错 | 基 trait impl 里类自己没有正文的方法，沿 superclass 链找最近有正文的（open 类的 trait 默认方法），`Base::name(self, ..)` 转发（`_inherited`/`_inherentCall(via)`）；转发时比基更窄的 trait 形参 `dart_cast_to::<dyn RenderBox>()`。`todo!("X does not translate Y yet")` **26199→2747**（剩的多是 mixin 的私有字段访问器：`_tickerModeNotifier` 198、`_child` 180、`_bucket` 110）。**3351→3283 / 872，138+1 个 crate**（mismatched −76，arguments incorrect −26，type annotations needed +34）。 |
...
| ws355 | 950 叶子错 | 两件：`T extends num` 不再当 `f64`（`_RestorablePrimitiveValue<T>` 收 `RestorableInt` 的 `i64` 又吐 `f64`，51 处）——`T` 留给调用方；`super.value = v`（SuperPropertySet，9 个拒绝）走基类 setter 的 super fn（`IrSuperCall(isSetter)`，`<base>_super_set_x`），基类字段则是摊平后的同一存储。**3214→3187 / 804→795，138+1 个 crate**（mismatched −21，operator incompatible −13；新增 `set_begin`/`set_end`/`value` 各 1——`Tween<T extends num>` 那边 `T` 泛型化后的界约束，trait bound +5）。 |
...
| ws367 | 950 叶子错 | `IrUpcast` 印法先看值记录的 `rustType` 再看节点形状（局部变量、调用结果此前一律 `Rc::new`，不注册）。**3070→3103**：枚举也被 `dart_object` 了（不是 `DartAny`，+49）。 |
| ws368 | 950 叶子错 | 枚举走普通句柄。**3103→3071 / 795，138+1 个 crate**（与 ws366 持平 +1，印法按类型是对的，留下）。 |
...
| ws400 | 950 叶子错 | `T?` 投影的第二、三步：前端把泛型声明（方法、顶层函数、构造、forwarder、stub）签名和字段里**本声明自己的**可空、未 erase 的类型形参标成 projected（`<T as DartNullable>::Or`），并在六个边界插 `IrNullableOf`：参数入口（函数体前 `let p = T::option(p)` 同作用域重绑）、本成员的 `return`（闭包不算，函数类型仍拼 `Option<T>`）、字段读/写、字段初始化、翻译过的被调用者的 `T?` 实参和 `T?` 结果（类形参按接收者代入，方法形参按调用实参代入）。`_typeKept` 和后端 `_substituteType` 代入时按 Dart 折叠 `T?[T:=X?]=X?`。所有类型形参加 `DartNullable` bound。驱动输出：478 处 projected 签名，148 个 option、161 个 from_option 转换。`WidgetStateProperty.lerp` 本身仍是旧的「方法形参遮蔽类形参」拒绝。 |
| ws400 结果 | 950 叶子错 | **构建断了：1691 / 34 个 crate，23 个函数外错**（`scc_flutter_widgets` 没编过，上面全不可达）。四类根：(1) `<T as DartNullable>::Or: Clone` 不满足——struct 的 `derive(Clone)`/字段 clone 要求 Or 是 Clone；(2) impl 签名和 trait 不符——trait 的 `T?` 投影后，impl 代入具体类型（`FlagsSummary<T>: DiagnosticsProperty<Map<String,T?>>`）拼成了 `Option<Map<..>>`，rustc 比签名不做规范化；方法形参被改名 `T_` 时前端没投影（名字检查）；(3) 给所有类型形参加 `DartNullable` bound 把 future 挡在外面（`_CallbackHookProvider<Future<bool>>`）；(4) 调用点转换按代入后的类型判断，`Equality<E?>.equals(E?)` 代 `E?` 时也转了（被调用者那边 Or 已是 `Option<E>`）。 |
| ws401 | 950 叶子错 | 四个通用修法：trait 里 `type Or: Clone`，泛型 impl 加 Clone bound，翻译的 struct 只在 derive Clone 时发 impl；projected 打印推广到任意类型 `<X as DartNullable>::Or`，后端 `_substituteType` 代入后保持 projected（impl 签名照 trait 拼）；`DartNullable` bound 精确到"自己或所实现的 trait 有投影"的类和"签名有投影"的方法；`_projectedSlot` 改查"是否按 bound 拼写"而不是名字；使用点（字段读写、调用实参、调用结果）按**代入物**判断：只有代入的是本作用域非空类型形参 `U` 才转换（`Or<U>` ≠ `Option<U>`），代入 `U?` 或具体类型时槽位已经是 `Option`。带投影字段的 struct 不 derive PartialEq（手写 impl 加 `Or: PartialEq` where）。驱动：797 处投影，146 option / 147 from_option，`widgets_router` 的 DartNullable 44→11。 |
| ws401 结果 | 950 叶子错 | **还是断的：1681 / 34 个 crate，11 个函数外错**。三类：(1) 拍平进子类的投影字段代入复合类型（`DiagnosticsProperty<T>._value: T?` 进 `IterableProperty<T>` 是 `<Vec<T> as DartNullable>::Or`），`Vec<T>: DartNullable` 要 `T: Clone`，而 struct 的形参 bound 故意不带 Clone；(2) 手写 PartialEq 的 where 子句只写了字段类型的名字（`<Map as DartNullable>::Or`）；(3) 投影槽代入**可空**类型时（`RestorableValue<T?>`）仍拼投影 `<T as DartNullable>::Or`，rustc 那边 trait 侧已规范化成 `Option<T>`；(4) 精确 bound 行不通：`SlottedRenderObjectElement<SlotType>` 被一个自己不投影的 trait 命名，Rust 要求命名处重复 bound。 |
| ws402 | 950 叶子错 | bound 回到所有类型形参（future 也实现 `DartNullable`：`Or = Option<Rc<Pin<Box<F>>>>`，读回时 `try_unwrap`），`_needsNullable` 只用来决定投影 struct 的形参加 Clone；where 子句拼完整类型；代入可空类型时折叠为它本身，非空时保持投影（impl 签名照 trait 拼）。 |
| ws402 结果 | 950 叶子错 | **还是断的：113 / 32 个 crate，1 个函数外错**：`Equality<Vec<E>>` 作 supertrait，`Vec<E>: DartNullable` 要 `E: Clone`（`Or: Clone` 逼的），而 trait 形参没有 Clone。 |
| ws403 | 950 叶子错 | `Or` 不再要求 Clone（`Vec<T>`/`Map`/`Set`/元组/future 的 impl 都无条件），要 clone 的地方在 bound 上说：所有带 Clone 的形参 bound 改成 `Clone + DartNullable<Or: Clone> + 'static`（关联类型 bound），struct/trait 的无 Clone 形参只 `DartNullable + 'static`；每个翻译的 struct/enum 都发 impl。 |
| ws403 结果 | 950 叶子错 | **还是断的：1865 / 54 个 crate，9 个函数外错**：投影 struct 自己的形参上带了 `Clone + DartNullable<Or: Clone>`（为了 derive(Clone)），Rust 要求每个命名它的声明重复这个 bound（`_FutureBuilderState<T>` 持有 `AsyncSnapshot<T>`、`_DelegateState<T>` 命名 `_InheritedProviderScopeElement<Option<T>>`），而那些 struct/trait 的形参只有 `DartNullable + 'static`。 |
...
| ws408 | 950 叶子错 | ws407 剩下最大的一族（106 个）：`WidgetStateProperty<T?>` 作类型实参/函数类型形参时 `T?` 没投影，代 `Color?` 后 `dyn WSP<Option<Option<..>>>` 和 `Fn(Option<Option<..>>..)` 与调用方不是同一个类型。`_type` 加深度：**类型内部**（类型实参、record、函数类型的形参和返回）的 `T?` 一律投影，顶层裸 `T?` 仍是 `Option<T>`（签名处由 `_edgeType` 投影）；`_typeKept` 代入的类型实参按实参深度算；`IrNullableOf` 结果类型按方向定；`coerce.dart` 加一条规则：同名 `T?` 一边投影一边不投影就插转换（函数类型里的差异靠 `projectionDiffers` 让适配闭包生效）；经函数值的调用按函数 Rust 类型的形参槽位 coerce、结果按其返回定型。投影 797→974。新拒绝 1（`_TaskEntry` 的 `FutureOr<T>` 走到了 `_type`）。 |
| ws408 结果 | 950 叶子错 | **断：1 个函数外错**——`dyn WidgetStateProperty<<T as DartNullable>::Or>` 要求 `Or: DartNullable`（trait 形参的 bound），预料中的"投影套投影"。 |
| ws409 | 950 叶子错 | prelude：`type Or: DartNullable<Or = Self::Or>`——`Or` 自己也是 `DartNullable` 且 `Or` 不变，rustc 由此把 `<<U as DN>::Or as DN>::Or` 规范化成 `<U as DN>::Or`（`T?` 代 `U?` 还是 `U?`，Dart 的折叠在 Rust 里可证）。 |
| ws409 结果 | 950 叶子错 | **2639→2650 / 796，138+1 个 crate**：嵌套投影净 +11，待看新旧各是什么。ws410 用 `DART2RUST_ERASE_OBJECT=1` 重量协变那一族（ws318 时 +440，那是形状适配器时代）。 |
| ws409 细看 | 950 叶子错 | 修 46 新 52。`Option<Option<` 452→126（嵌套投影起效）。新的三类：(1) `_typeKept` 对具体类也打了 projected（`<GestureBinding as DartNullable>` 把 trait 当类型，E0782 15 个）；(2) 泛型体里 `Vec<T?>` 现在是 `Vec<Or>`，`List.filled(n, null)`、元素上的 `!`/`== null`/`??`/`==` 直接碰 `Or`（priority_queue 7 个、segmented control 14 个）；(3) `this_.begin()?.unwrap()` 之类结果没转就 unwrap。 |
| ws411 | 950 叶子错 | 三个通用修法：`_typeKept` 只在代入物是本作用域裸类型形参时投影；`coerce.dart` 任何值进投影槽先 coerce 到 `Option<T>` 再 `from_option`、投影值出来先 `option` 再 coerce（`None` 进 `Vec<T?>` 元素不再是 rustc 推不出的 `Option<_>`）；后端在 Option 级操作（`!`、`is_none`、`?.`、`??`、`==`/`!=`）的操作数是投影类型时先 `option()`（`_plain`）。转换 157/126→167/195。 |
...
| ws413 结果 | 950 叶子错 | **断：80 个函数外错，34 个 crate**。两类：(1) `impl Tween<Object> for IntTween` 要求 supertrait `Animatable<Object>` 也有 impl（61 个 E0053 里一部分、19 个 bound）；(2) 投影的 `T?` 代 `dynamic` 时后端走了"dynamic 已含 null"的老捷径拼成 `Rc<dyn Object>`，trait 侧规范化后是 `Option<Rc<dyn Object>>`。 |
...
| ws416 结果 | 950 叶子错 | **2973→2632 / 796，138+1 个 crate**，比协变前的 ws411（2638）低 6，目前最低。 |
| ws416 细看 | 950 叶子错 | 修 123 新 64；协变 trait bound 187→65（剩的：泛型类 `WidgetStatePropertyAll<T>` 用作 `WSP<T?>` 12 个——要按类自身的具体实例化发 impl；`Rc<dyn X>: DiagnosticableTree` 之类 handle 进 handle 的 20 个）。新错里 `cannot find type T` 43→63：基类 `T? x = null` 的字段初始值拍平进子类时 `IrNullableOf` 的 `T` 没代入。E0034 21 个（`mark_needs_layout` 两个 trait 都声明）是老的。 |
| ws417 | 950 叶子错 | `_allFields` 拍平基类字段时初始值也经 `_substitute` 按基类绑定代入（`IrFieldDecl.substituted` 加 `initial`）。 |
| ws417 结果 | 950 叶子错 | **2632→2609 / 796，138+1 个 crate**。 |
...
| ws421 结果 | 950 叶子错 | **2770→2605 / 796，138+1 个 crate**，目前最低（比 ws417 低 4：setter 修正 + `Match`）。 |
| ws422 | 950 叶子错 | 实例调用也进 `_withGenericArgs`（方法自己的类型形参按调用实参绑定，`maybePop<T>` 里 `pop<T>(result)` 的投影槽位才知道 `T` 代什么，19 个 `Or <= Option<T>`）；`async` 成员的 `return` 边按 `Future<X>` 的 X 判断投影。驱动输出对 ws421 差 3 文件。 |
...
| ws423 结果 | 950 叶子错 | **2602→2583 / 796，138+1 个 crate**。 |
| ws424（含 ws425 的三条） | 950 叶子错 | 再加：prelude `UnimplementedError::new(Option<String>)`——Dart 的 `[String? message]` 本来就可空，调用方传 `None`（10 个 `String <= Option<_>`）。 |
| ws425 | 950 叶子错 | 运算符两端不一致：trait 里的 `[]`/`~/`/比较是 `Result` 方法，struct 上的 inherent 版本不返回 `Result`，而前端对所有运算符调用都不 `?`（`MaterialColor.shade50` 的 `index_of(50).unwrap()` 拆的是 `Result`，20 个）。统一：只有 `std::ops` 有 trait 的运算符（`stdOperators`，ir.dart 一张表）保持 std 签名不传播，其余运算符两端都是普通的失败方法（后端 inherent 版本走 `_emitMethod`）。**拒绝 796→693**（运算符体内失败调用之前拒绝）。驱动输出对 ws424 差 136 文件。 |
| ws424 结果 | 950 叶子错 | **2578**（ws423 2583），139 crate。去 19 来 19：来的是 ws424 自己的两处——(1) `List<Hct>.from(hctsByHue)`：`_mentionsDynamic` 的门让参数先 coerce 进声明的 `Iterable<dynamic>`（逐元素 upcast），prelude 的 `List.from` 是 `.clone()`，没有再 cast 回来（temperature_cache 3、balanced_layout、l10n `HashSet.from` 3 个 static）；(2) `Timeline.instantSync(arguments: <String, dynamic>{..})`：Map 规则打印的 `Map::from(vec)` 而 prelude 的 `Map::from` 只收数组，且 map 字面量首项没有显式 upcast（image_cache 4）。 |
| ws426 | 950 叶子错 | 三条：`preludeSiblings` 一张表——prelude 按*同类兄弟*的声明实现的 dart:core 成员（`List/Set/Map/HashSet/LinkedHashSet/HashMap/LinkedHashMap.from` → `of`），槽位类型取 `of(Iterable<E>)` 的，参数不再 upcast 进 `dynamic`；map 字面量首项的隐式 upcast 显式拼出（同 `vec![..]` 首元素规则）；prelude `Map::from` 收 `IntoIterator<Item=(K,V)>`（数组和 `Vec` 都行），`Timeline.startSync/instantSync`、`TimelineTask.start/finish` 的 `Map?` 和 `postEvent` 的 `Map<Object?,Object?>` 按 Dart 声明拼（键不再是 `String`）。驱动输出对 ws425 差 46 文件，拒绝 693。 |
| ws425 结果 | 950 叶子错 | **2566**（ws424 2578），139 crate，拒绝 693。去 21 来 11：来的 4 个 `op_eq` 是原来 `eq` 的改名（同样失败），其余是 ws424 挪 crate 的 table 5 个再计一次。 |
| ws427 | 950 叶子错 | 四条通用：提升/`as` 的 downcast 带上泛型类的类型实参（`other is AsyncSnapshot<T>` 读成 `downcast_ref::<AsyncSnapshot<T>>`，16 个 E0107）；借用下的操作数把隐式 upcast 拼出来（`&` 后面没有 unsizing：`map.get(&Rc::new(key))` 是 `&Rc<String>`，19 个 `&Rc<dyn Object>`）；prelude `Set::len()` 对齐 `Vec`/`Map`（`length` 一律打印 `len()`，6 个）；prelude `DartNullable::dart_null()`（`Option` 给 `Some(None)`，其余 `None`）+ `DartListLength::set_length`：`List<T?>.length = n` 补 null，非空元素列表增长是 `RangeError`（1 个）。驱动输出对 ws426 差 13 文件。 |
| ws426 结果 | 950 叶子错 | **2526**（ws425 2566），139 crate。去 46 来 7（来的是 table 5 个又换 crate、plural_rules 一个 static、scaffold 一个 `Rc<dyn Widget>: Widget`）。 |
| ws427 结果 | 950 叶子错 | **2513**（ws426 2526），139 crate。 |
...
| ws428 结果 | 950 叶子错 | **2388**（ws427 2513，−125），139 crate。去 114 来 6：「is not a future」118→24、E0381 125→19。来的：`widgets_binding._handle_commit_back_gesture`（async fn 不 await 直接当值返回，`?` 用在了 future 上）、`services_asset_bundle.from_option`（`?` 的错误类型转不成 `Exception`），table 4 个换 crate 再计。 |
| ws429 结果 | 950 叶子错 | **2391**（ws428 2388，+3），**140 crate**（多的是 `dart_main`）。prelude 自己被打了一个桩：`impl<T> Default for Completer<T>` 少了 `T: DartNullable`（本轮补上）。testdata 副本的 `FormatException::new` 与 prelude.dart 早已不一致（`Option<Rc<dyn Object>>` 对 `Rc<dyn Object>`），以 prelude.dart 为准改回。 |
...
| ws430 | 第一次构建（run429，在 ws429 的打桩工作区上）：`dart_main` 构建先因 prelude 那个桩失败，工作区里补上 bound 重建。路径上下一项已知：`GetStorage._internal` 的 `Future<bool>(() async {..})` 被拒（`Future.` 构造需要执行器）。通用：`FutureOr<T>` 成为类型（prelude enum，`FutureOr::value/future`，可 await；`coerceInto` 把 `T`/`Future<T>` 送进去）；prelude 调度器加 `tasks` + waker（`dart_spawn`：eager future），`Future()/microtask/sync/delayed/wait/error` 成为 prelude 函数；没 await 的 `async fn` 调用值是 `Box::pin(call)`（不 `?`）。拒绝 692→666，驱动输出对 ws429 差 42 文件。 |
| run429 | 构建本身成了尺子的第一读数：`gallery_above` 一个 crate 的 codegen 单线程跑了 30 分钟、27 GB 没完（`cargo check` 只要 3 分钟）。原因：code viewer 的 `codeviewer_code_segments`（6.7 MB，63 个函数）里一个函数是 3038 个 `TextSpan::new(..)?` 的扁平 `vec![..]`——每个 `?` 出口都要 drop 前面所有临时值，drop 代码是平方的（约 460 万个 drop）。两条通用：工作区 `[profile.dev] panic = "abort"`（翻译代码从不捕获 panic，Dart 异常是 `Result`；换成 abort 后重建 11 分钟仍没完，landing pad 不是主因）；**元素可能失败的列表字面量逐个 `push` 构建**（`{ let mut __v = Vec::new(); __v.push(a?); .. __v }`，失败时只 drop 一个部分 `Vec`；`_WalkSelf.failing` 判定，483 处）。驱动输出对 ws429 差 210 文件。 |
| ws430 结果 | 编译尺子 **2411**（ws429 2391，+20），140 crate；gallery crate 的 check 从 194s 降到 33s。去 11 来 27：来的几乎都是 `FutureOr` 让 27 个原来被拒的成员翻出来后不编译的（`_internal`、`schedule_task`、navigator 几个、`compute`…），不是回归。 |
| run430 | **第一次跑起来了**：逐个 push 的列表后 `cargo build` 整个工作区 + 运行只要约 100 秒（run429 单个 crate 30 分钟没完）。第一个 panic：`main` → `GetStorage.init` → 读字段 `initStorage`（`Future<bool>`）时 `.clone()`——`Pin<Box<dyn Future>>` 不是 `Clone`。同时修掉的：`_internal` 里 `() async {..}` 进 `FutureOr<bool> Function()` 槽：`_withBorrowing` 重建闭包时丢了 `isAsync`（`await` 出现在非 async 闭包里），且 async 闭包体的期望返回应取 awaited 类型（`FutureOr<T>` → `T`），外层 future 由 adapter `FutureOr::future(..)` 包。 |
| 下一步 | 字段/局部里的 future 是 Dart 的**共享、eager、可 clone 的值**，`Pin<Box<dyn Future>>` 是惰性、独占、借用 `&self` 的——模型不对，会一直咬（存字段、`.then`、`Future.wait` 里的方法 future、不 await 的调用不执行）。通用解：prelude `DartFuture<T>`（`Rc<RefCell<..>>` 的共享任务句柄，`Clone`，`impl Future`，结果 clone 给每个 awaiter），`Future<T>` 类型一律拼成它；async 方法/函数/super 函数不再是 `async fn`，而是 `fn .. -> DartFuture<T> { let __self = self 的句柄; DartFuture::spawn(Box::pin(async move { .. })) }`（eager、`'static`）。 |
| ws431 | 上一行做完：prelude `DartFuture<T>`（`Rc<RefCell<{result, wakers}>>`，`Clone`/`PartialEq` 按身份/`Debug`/`DartNullable`，`impl Future`（结果 clone 给每个 awaiter），`pending/ready/resolve/spawn/then`）；`Completer<T>` 就是一个 `DartFuture<T>` 的句柄（`complete_error` 收 `Rc<dyn Object>`，走 `Err`）；`Stream.first/toList`、`StreamSubscription.cancel`、`Future.*` 构造、`FutureOr::Future` 都换成它。后端：`Future<T>` 类型一律 `DartFuture<T>`；async 方法/顶层函数/super 函数发两份——`name__body` 是原来的 `async fn`，`name` 是包装：clone 出接收者句柄（`_selfHandle()`；super 函数用 `this_.dart_self_<trait>()`，body 以 `dyn Trait` 实例化），参数 move 进 `async move` 块，`DartFuture::spawn`；没翻出来的只发包装（panic）。trait 声明的 async 方法返回 `Result<DartFuture<T>, E>`，forwarder 不再 `Box::pin`；async 闭包同样是普通闭包 + spawn（内部再 clone 一遍捕获）。prelude 单独 check 通过。驱动输出对 ws430 差 134 文件，拒绝 666。 |
| ws431 结果 | 作废：**34 crate**，1511。一个函数外的错把 `scc_flutter_widgets` 整个拦下：`WidgetsFlutterBinding` 派生 `Clone` 而字段 `Option<TimelineTask>` 的 prelude 类型没有 `Clone`（之前这个 struct 没派生，改了 future 模型后派生了）。prelude `TimelineTask` 派生 `Clone/Debug/PartialEq`，重跑为 ws432。 |
| ws432 结果 | 编译尺子 **2383**（ws430 2411，−28），但 **139 crate**：`workspace.py` 找入口还认 `pub async fn main()`，入口现在是 `pub fn main() -> DartFuture<()>`，`dart_main` 没生成（run432 跑的是旧二进制）。去 97 来 75；来的里可修的：await 前 `?` 的规则——只有*inherent* 到达的 async fn 是 `DartFuture` 本身，经 trait（qualifier/asTrait，或继承自 open 基类如 `MethodChannel.invokeMapMethod`）到达的是 `Result<DartFuture>`，先 `?`，await 不再压 `?`（9 个「is not a future」）；`await` 一个 `Future<T>?` 是 `match`（`proxy.send()` 返回 `Future<ByteData?>?`）；counted 类的 `__self` 字段 `pub`（其他模块拼常量实例的 struct 字面量，4 个 E0451）；prelude `Uri::parse(text, start, end)` 按 Dart 声明（2 个）。 |
| run432 | **走过了 `GetStorage.init`**（DartFuture 模型生效），下一个 panic：`WidgetsFlutterBinding.ensureInitialized` → `WidgetsFlutterBinding::new` 不存在——构造函数在前端被拒。类内拒绝原来不带成员名，加了：`refuse(member, ..)` 记名，`DART2RUST_TRACE=<Class>` 打栈。两条通用：(1) 泛型函数*类型*自己的类型参数按 bound 拼（`StructuralParameterType` → bound；带 typeParameters 的 `FunctionType` 先以 bound 实例化）——`SchedulerBinding._taskQueue = HeapPriorityQueue(_taskSorter)` 的比较器类型；(2) 常量接收者上的方法 tear-off（`const GZipCodec().decode`，`_addLicenses`）是不捕获任何东西的闭包。拒绝 666→656，驱动输出对 ws432 差 380 文件。 |
| ws433 结果 | 编译尺子 **2444**（ws432 2383，+61），140 crate（入口回来了）。来 69：51 个 mismatched，最大簇 `Future.value(false)` → `DartFuture::ready(Ok(Some(false)))`（参数按 Dart 的 `[FutureOr<T>? value]` 进 `Option`）：prelude `future_value(Option<T>)`（无值取 `T::dart_null()`，`()` 的 null 是 `()`）。 |
...
| ws435 结果 | 作废：**33 crate**，122。`JsonCodec` 常量能翻之后 `JSONMessageCodec._decoder = utf8.decoder.fuse(json.decoder)`（一个 static，函数外）引用了 prelude 没有的 `Converter<S,T>` 类型。prelude 加 `Converter<S,T>`（持一个转换闭包：`convert`、`fuse`，`Clone`/身份 `PartialEq`/`DartNullable`），`Utf8Decoder.fuse`、`JsonCodec.decoder/encoder`。 |
| ws436 | **mixin 链的 super 函数**：CFE 把 mixin 的方法体复制进匿名 application 类（`dart:mixin_deduplication`，去重后有的 application 是空壳），mixin 声明留成 abstract。驱动收集每个 mixin 的 application 列表（`implementedTypes`），前端 lower mixin 声明时，abstract 方法从任一保留了实现的 application 取体（`_appliedBody`）——trait 得到默认实现和 super 函数（`scheduler_binding_super_frames_enabled` 等 36 个），`WidgetsFlutterBinding` 的 14 个「super call not translated」剩 2。拒绝 624→617，驱动输出对 ws435 差 452 文件（多是 `use` 列表）。 |
| ws436 结果 | 编译尺子 **2427**（ws434 2339，+88），140 crate。来 99：几乎全是 mixin 体第一次发出来的 super 函数不编译（widgets_binding 21、services_binding 10…）——拒绝换成桩。其中通用的：借来的体里 `this` 的静态类是匿名 application 类，`_qualifiedRaw` 把它当作 mixin 的 trait（9 个 E0782 `ServicesBinding::x(this_)`）；prelude `DartList::index_of(value, start)` 按 Dart 声明（省略的 `start` 由前端补 0，13 个）。 |
| run436 | 过了 `SemanticsBinding` 的 `ObserverList`，下一个 panic：`HeapPriorityQueue::new`（`SchedulerBinding._taskQueue`）——泛型类里 `List<E?>` 的投影：`List<E?>.filled(n, null)` → `vec_of_nones::<E>(n)` 返回 `Vec<<E as DartNullable>::Or>`（类型实参拼出来，推不出）；`_queue[i]`（`IrIndex`）按列表元素类型定型（投影的），`??` 前 `_plain` 才生效；`null as E` → prelude `dart_null_as::<E>()`（`E::dart_null()`，无 null 的类型 panic 即 Dart 的 TypeError）。预修：`this` 作为值逃逸（传参/返回/存字段/进字面量）的类计数（`_ThisEscapes`；`addObserver(this)` 的 `Rc<dyn WidgetsBindingObserver> <= _WidgetsAppState`，12 个）。拒绝 617→609。 |
...
| ws439 结果 | 作废：**60 crate**，2171。struct 头的类型参数 bound 是 `DartNullable + DartEq + 'static`，trait 默认方法要 `Or: DartEq`（`_nb`），forwarder 里调不动——struct/trait 头改成 `DartNullable<Or: DartEq> + DartEq + 'static`（`Or: DartEq` 对所有类型都成立）。 |
...
| ws448 结果 | 编译尺子 **2631**（ws447 2698，−67），141 crate：去 42 来 0。 |
| run448 | `WidgetsFlutterBinding::init_instances` 过了，进到 mixin 的 super 函数链：`semantics_binding_super_init_instances` 是桩——`__Self: PaintingBinding` 不满足。mixin 体里的 `super.initInstances()` 派发到 application 的**上一个** mixin（`_realOwner`），而 super 函数只以自己的 trait 约束 `__Self`，`on` 子句没提到的 trait 就缺 bound（Semantics→Painting、Scheduler→Gesture）。通用：super 函数的 `__Self` 加上体内 `super` 调用到达的、本类不在其下的 trait。同路径的第三个桩 `services_binding_super_init_instances`：`BasicMessageChannel<String?>(.., StringCodec())` 要 `StringCodec: MessageCodec<Option<String>>`——普查（`addWiderImpls`）只收 `_type` 经手的实例化，`MessageCodec<String?>` 是把 `String?` 代入 `codec` 字段类型才出现的。通用：普查对每个具名实例化闭包，把它代入该类成员的声明类型再普查（每个实例化一次）。驱动 diff 1456 行：39 个新的宽 impl（`ValueListenable<Object?>` 16、`Animation<double?>` 9……），refusal 521 不变。 |
| ws449 结果 | **作废**：34 crate（widgets crate 被一个函数外错误切断，1902 不可比）。`local_history_route_super_did_pop` 的新 bound 打成裸 `TransitionRoute`（E0107）：`LocalHistoryRoute<T>` 的 `super.didPop` 经 application `ModalRoute<T>` 到达 `TransitionRoute<T>`，而 `_traitArgsOf` 只沿本类的超类型找实参，找不到。通用：`IrSuperCall` 带上 base 的类型实参（`baseArguments`）——从 application 看 base 是 `TransitionRoute<T_app>`，再经 applied 类型 `LocalHistoryRoute<T_app>` 把 application 的参数映回 mixin 自己的；映不回去（实参不是裸参数）就留空。bound 排除根 `Object`。驱动 diff 2 行。 |
| ws450 结果 | 编译尺子 **2646**（ws448 2631，+15），141 crate：去 7（那 7 个 super 函数编过了）来 15——错误上移一层：trait 默认方法 `fn init_instances(&self) { scheduler_binding_super_init_instances(self) }` 里 `Self: GestureBinding` 不满足，且 `renderer_binding_super_init_instances` 传递地要 `PaintingBinding`。方法级 `where Self: X` 在可派发的 trait 方法上是 E0038，走不通。 |
| run450 | 第一个 panic 同族：`renderer_binding_super_init_instances` 桩（`__Self: PaintingBinding`）。通用：mixin 的 trait 以**每个** application 都放在它下面的东西为 supertrait（`_appliedOver`：沿 application 的超类链取 mixin 和第一个真实类，各 application 求交，实参按第一个 application 映回 mixin 的参数）——mixin 的体本来就取自 application，`super` 也按 application 的链派发，trait 的要求只是把这件事说出来。查 application 时发现 `dart:mixin_deduplication` 里三种匿名类：真 application、mixin 声明 `on` 子句的持有者（`Object&BindingBase&SchedulerBinding`，超类 Object，子类是 mixin 声明本身）、去重后没人继承的死类；后两种不是 application，驱动建表时排除。23 个 trait 头多了 supertrait（`SchedulerBinding: BindingBase + GestureBinding`，`LocalHistoryRoute<T>: Route<T> + TransitionRoute<T>`……），`todo!` 数和 fn 数不变，refusal 521 不变。 |
| ws451 结果 | 编译尺子 **2638**（ws448 2631，+7；ws450 2646），141 crate：`init_instances` 全族清零；去 13 来 13，其中 5 个只是 table 两个模块换了 crate，真新 8 个：`Animation<Option<f64>>` 宽 impl 让 `animation_super_to_string_details(self)` 的 `T` 推不出（3）；`<__Self as CupertinoPageRoute>` 少实参——qualifier 是本类自己时 `_traitArgsOf` 没答案（1）；`_TextFormFieldState` 的 `dart_cast` 里出现匿名 application 类名 `_MixinApplication37&State&RestorationMixin<..>`——普查闭包后 `addWiderImpls` 把匿名祖先也当 trait（4，同一处的 `_value` 惰性读还带着基类的 `T`）。 |
| run451 | **mixin super 链全部跑完**（Widgets→Renderer→Semantics→Painting→Services→Scheduler→Gesture→BindingBase）。第一个 panic：`BindingBase._initListenable` 桩——`AssertionError(message: final Object? msg)` 模式：prelude 的 `AssertionError.message` 是 `String`，Dart 声明是 `Object?`，读出来 `.unwrap_or_else` 打在 `String` 上；另一处 `dynamic` 临时量提升到 `Object?` 打成 `downcast_ref::<Object>()`。通用：prelude 按 Dart 声明改 `message: Option<Rc<dyn Object>>`（`new(String)` 仍在，包成对象）；提升到 `Object` 不是 cast；连带修上一行的三家：`_traitArgsOf` 对本类返回自己的参数；`addWiderImpls` 跳过匿名 application；`_lazyRead` 经 impl 转发时把 `_implBinding` 代入初始值（`_substitute` 也代换 `IrCastTo`/`IrDowncast` 的类型）。super 函数调用的 `T` 歧义（3）留到下轮。驱动 diff 214 行，refusal 521。 |
| ws452 结果 | 编译尺子 **2604**（ws451 2638，−34），141 crate：去 38 来 5（5 个只是 table 模块换 crate）。上一行的 super 函数 turbofish（c825895c）还没进这条链。 |
| run452 | 还是 `_init_listenable` 桩，换成 mismatched types：`isSet ? #0#2{String} : (#0#2 = error.message)`——赋值表达式的值。`dynamic` 临时量存 `String` 时 store 装箱成 `Rc<String>`，而表达式的值按 Dart 是赋的那个 `String`。原来只对 `Some(..)` 的 store 做「先 hold 再存 clone 再产出」，通用化：store 与原值不同就这样做（字面量除外，会求值两次）。 |
| ws453 结果 | 编译尺子 **2581**（ws452 2604，−23），141 crate：去 18 来 0。 |
...
| run455 | **`_initListenable` 过了，整个 binding 构造（含 `initInstances` 全链、`BindingBase` 构造体）跑完**。第一个 panic 到了引擎边界：`SchedulerBinding.addTimingsCallback` → `PlatformDispatcher.onReportTimings` setter → `__nativeSetNeedsReportTimings`，`external` 成员的 `todo!("external .. is the engine's to provide")`。dart:ui 里这样的 external 有 161 个（全树 632，只译前缀内的）。通用机制（按〈两条路〉A 的分工「VM 转发给 engine，生成代码只看见 Dart 那侧的签名」）：每个 external 译成 `dart_native(symbol, vec![args as Object], returns)`——symbol 取自 CFE 留下的 `@Native` 注解（`pragma("cfe:ffi:native-marker", Native<..>(symbol:..))`，转换过的 `$Method$FfiNative` 上是 `pragma("vm:ffi:native", ..)`），实参按 `coerce` 进 `Object`，返回值再从 `Object` 按声明类型 coerce 回来。prelude：`set_native_host(Box<dyn Fn(&str, Vec<Rc<dyn Object>>) -> Result<Rc<dyn Object>, _>>)` 装宿主（将来的 runtime crate → engine）；没装宿主时 **void native 跳过并记名**（`natives_skipped()`），有返回值的 native panic 报出 symbol——那就是无头尺子的终点、runtime 的起点。162 处全部走边界，`todo!("external` 清零。另修 `coerce`：裸 `Function` 类型进 `Object` 是它自己（后端把它拼成 `Rc<dyn Object>`），不再套一层 handle。 |
| ws456 结果 | 编译尺子 **2574**（ws455 2573，+1），141 crate：`todo!("external` 清零后来 1——`Gradient::Create` 的实参 `(gradient as Rc<dyn Object>)`：dart:ui 的 `Gradient` 是 struct，`package:flutter` 的同名类是 abstract，前端 `isTraitName` 先查 `abstractElsewhere`（按名字）就说它是 trait。通用：先看本库自己的同名类。 |
| run456 | **过了 `SchedulerBinding.initInstances` 里的 native**（`SetNeedsReportTimings` 无宿主跳过），下一个 panic：`ServicesBinding.initInstances` → `createRestorationManager` 编译桩（mismatched types）。`Ok(RestorationManager::new()?)` 本身类型是对的，错误只在后面的轮次出现，`rendered.txt` 按行号查不到（早先的桩把行挪了）。工具：`stubs.py` 另写 `<report>.detail.txt`，每个桩带上让它成桩的那条诊断。prelude：`run_main` 结束时报告无宿主跳过的 void native 名单。 |
| ws457 结果 | 编译尺子 **2573**（ws456 2574，−1），141 crate。`detail.txt` 第一次说了实话：`services_binding_super_create_restoration_manager` 被打桩的那条诊断在 `_handlePlatformMessage` 里（`method_call.arguments.clone()`）——**尺子自己的 bug**：同一文件同一轮按行号从高到低打桩，高处的桩把函数体折成一行后，低处（同一函数里）的 span 再查一次就落进了往上挪的、健康的下一个函数。修 `stubs.py`：本轮已折叠的函数体范围内的 span 一律算它的，不再查。多少个桩是这样误伤的，下一轮见分晓。 |
| ws458 结果 | 编译尺子 **2198**（ws457 2573，**−375**），141 crate：去 345 来 5（5 个是 `rendering_table`/`widgets_table` 两个模块在 `flutter_rendering_above`/`flutter_widgets_above` 之间来回换 crate 的老噪声，workspace.py 分区命名不稳定）。**尺子从 ws0 起一直虚高**：同轮同文件里被上面的桩挪了位置的 span 误伤了健康函数，到 ws457 累计 375 个。`create_restoration_manager` 不再是桩。 |
| run458 | `createRestorationManager` 过了（它本来就没错），进到 `RestorationManager()` 构造 → `initChannels` → `MethodChannel.setMethodCallHandler` 的 super 函数桩：`(ByteData? message) => _handleAsMethodCall(message, handler)`——`_handleAsMethodCall` 是 async 方法但 `fails=false`，前端在 `qualifier == null && !fails` 时原样返回不标 `asyncFn`，后端也就不打 `?`；而 super 函数里 `this_`/`__me` 是 `Rc<dyn MethodChannel>`，经 trait 调 async 方法拿到的是 `Result<DartFuture>`（trait 声明把每个 async 方法都包在 `Result` 里）。通用：`IrCall` 多带 `asyncTarget`（目标是否 async），`asyncFn` 对 `this` 上的调用在 open/abstract 类里也算「可能 inherent」，由后端按打印的是 trait 体（`_fieldsAreAccessors`）还是 struct 体决定：经 trait 的 async 调用一律 `?`。驱动 diff 34 行，全是 trait 对象接收者上的 async 调用补 `?`（`route.will_pop()?.await?`、`scroll_controller.animate_to(..)?`……）。中途误发了一条空改动的链（ws459 第一次），杀掉重发。 |
| ws459 结果 | 编译尺子 **2190**（ws458 2198，−8），141 crate。 |
| run459 | `setMethodCallHandler` 过了，下一个：`MethodChannel.binaryMessenger` getter 的 super 函数桩——`_profiledBinaryMessengers[this]`，Expando 的键要 `&Rc<dyn Object>`，`this` 是 `Rc<dyn MethodChannel>`。**改打法**（用户：为什么又慢了）：不再一次修一个，顺着路径把 `detail.txt` 里已知的桩一批处理。这一批：(1) async 闭包按函数值的统一形状返回 `Result`——槽要 future 就 `Ok(spawn)`，槽是 `void` 就 spawn 后 `Ok(())`（路径上 12 个 `expected closure to return Result<DartFuture..>` 桩：`initServiceExtensions`、`register*ServiceExtension`、`scheduleWarmUpFrame`、`setMessageHandler`……）；(2) prelude `Expando` 的键接受任何 handle（按地址）；(3) prelude 补 `DartFuture::when_complete`/`catch_error`、`Timeline::time_sync`、`UserTag`、`DartQueue::add_all`（`lockEvents`、`handlePopRoute`、`drawFrame`、`_handlePointerDataPacket` 的桩）；(4) 泛型 struct 的键参数不再加 `PartialEq` 块（`ObserverList<VoidCallback>.add` 被它挡住；Map/Set 早已按 `DartEq` 比键）；(5) `dart_cast_to::<dyn CanonicalizedMap<_, _, _>>` 泛型 trait 的 cast 写出推断实参。驱动 diff 339 行，refusal 521。 |
| ws460 结果 | 编译尺子 **2114**（ws459 2190，−76），141 crate：去 74 来 12（5 个 table 噪声，7 个是去掉键参数 `PartialEq` 后 `T?` 上的 `==`——通用：类型参数上的 `==` 打成 `dart_eq`）。 |
| run460 | 还停在 `binaryMessenger`：prelude `Expando` 的键改成按值接收（调用点传的是句柄本身）。顺路修的一批（都在 `detail.txt` 的启动路径上）：`Object`/`dynamic` 到 `Map`/`List`/`Set` 的 `as` 带实参 downcast（`systemMessage as Map<String, dynamic>`，之前原样透传）；super 函数体里闭包内的 `Self` 应为 `__Self`（`_inSuperFn` 标志，不再看 `_selfName`）；`this` 作 handle 共享时用它自己的句柄（`_manifold` 的 `self.clone()`）；别的对象的字段读一律 clone（`..get().child` E0507）；prelude `void?` 就是 `void`（`invokeMethod<void>` 的 `Future<T?>`）；被闭包赋值的**参数**也放 cell（`attach(owner, [element])`，之前闭包里的赋值丢了）；限定路径要指向**声明**该项的 trait（`<Self as RenderView>::set_child` → `RenderObjectWithChildMixin`，78 个 E0576）。驱动 diff 10113 行（字段读 clone 占大头），refusal 521。用户改了工作方式：本轮起停掉 /loop，改为自设目标连续推进。 |
| ws461 结果 | 编译尺子 **1900**（ws460 2114，−214），141 crate。 |
| run461 | 还停在 `binaryMessenger`（Expando 按值的修法在 ws461 之后才提交）。顺路又一批（`detail.txt` 启动路径上的）：`Completer<void>.complete()` 打成 `complete(())`（`void?` 归一为 `void`，`Null` 进 `void?` 是 `()`）；`List` 的 downcast 拼成 `Vec`；`dynamic as T?` 不再先 unwrap；`super.m<T>()` 把方法自己的类型实参也写出来；闭包句柄 `__me` 上的限定调用用 `<dyn Trait as Trait>::m(&*__me)`（super 函数里 `__me` 是 trait 对象，`<__Self as ..>` 要的是 `&__Self`；两个 trait 都声明的名字仍要限定，E0034 16 个）；闭包体带自己的 async 性（sync 方法里的 async 闭包内 `try` 要 async 块）；继承进来的基类构造参数绑定写类型；`Future<void>.value()` 写类型实参；**`this` 进 `Object` 槽（Expando/Map 键、`identical`）也算逃逸 → 该类计数**（counted 858→1058，refusal 521→516）。 |
| ws462 结果 | 编译尺子 **1762**（ws461 1900，−138），141 crate：去 101 来 21（16 个 E0034 是 `__me.m()` 的歧义，本轮修；5 个 table 噪声）。 |
| run462 | **无头尺子第一次撞到有返回值的 engine native**：`ServicesBinding.rootIsolateToken` → `PlatformConfigurationNativeApi::GetRootIsolateToken`（`int`）。之前路径上的一切——binding 全链、`RestorationManager`、`MethodChannel.setMethodCallHandler`/`binaryMessenger`——都跑过了。无宿主策略再推一步：有返回值的 native 按**声明类型**读答案（prelude `NativeAnswer`：有宿主就 downcast 宿主给的对象，没宿主给「不在场的 engine」的值——0/false/""/null，都记名、结束时报告）。`RootIsolateToken.instance` 自己把 0 当「没有」。`dynamic` 进 `T?` 经 `dart_nullable` 看是不是 `Null` 对象（53 处）。 |
...
| ws473 | 链：**1537 → 1532**，142。 | |
| run474 | 同一个桩，再下一条：`json.decode(x)` 译成 `(JsonCodec).decode(x, None)`，prelude 的 `decode` 只收一个参数——而且它的体是 `panic!("not written")`。 | 一批（都用 scratch fixture 先复现，`bin/dart2rust_package.dart` 直接吃 fixture 的 dill）：① prelude 写了 `json.decode`（含 reviver），产出译出代码里 `dynamic` 装的东西：`Map<String, Rc<dyn Object>>`/`Vec<Rc<dyn Object>>`/String/i64/f64/bool/Null；② `x as Map<String, Object?>`：Dart 运行时分不出 `Map<String, dynamic>` 和 `Map<String, Object?>`，这里是两个类型，所以"向 dynamic/Object?/标量（及其集合）的集合下转"走 prelude 的 `dart_cast_map`/`dart_cast_list`（`FromDynamic`），精确类型不符时逐元素换表示；③ switch 表达式最后一臂带自己的块（record pattern 绑定的变量，`scaleFontSize`）——`_fallsOutUnassigned` 下到块的最后一句，`unreachable!` 才补得上；④ `a?.toDouble()` 的 `num?` 接收者：标量 `?.` 的体按值绑定（Copy），`it as f64` 不再要解引用；⑤ `Rc<dyn Size> * f64`（`Size.lerp`）：外部 `Rc` 挂不上 `impl std::ops::Mul`（orphan），开放类句柄上的运算符改调 trait 的 `op_*` 方法，像方法一样失败。 |
| ws474 | 链：**1532 → 1488**，142。新桩 3（table 一族，是链挪动露出的旧账）。 | |
| run475 | `dart_cast_map(..).unwrap()` 得到 `None`：用一个 probe crate 试出 prelude 的根病——`impl<T: 'static> Object for T` 是 blanket 的，`Rc<dyn Object>` 自己也是 `'static`，所以 `handle.as_any()` 解析到 **句柄自己的** impl，`downcast_ref::<String>()` 永远失败。译出代码里每一处 `dynamic` 下转都是这样（`is StateError` 之前没露馅只是因为没走到）。 | blanket impl 的 `as_any`/`runtime_type` 在自己是 `Rc<dyn Object>` 时转给里面的对象（一次 TypeId 比较）。同批：`await` 一个 `Future<T?>?` 得 `T?` 不是 `Option<Option<..>>`；async 闭包进 `Future<T>? Function` 槽返回 `Ok(Some(spawn))`；声明 `Future<T>?` 的 async 成员的调用按其 wrapper 定型（`DartFuture<T>`）——三条都是 `BinaryMessenger.send`/`setMessageHandler` 一族。 |
| ws475 | 链：**1483**，142。 | |
| run476 | **宿主的 `install` 全程走通**（locales、settings、`_addView`），`main` → `WidgetsFlutterBinding.ensureInitialized` → `ServicesBinding._initKeyboard` 桩：`syncKeyboardState().then(..)?`——prelude 的 `then` 返回 future 不返回 `Result`，backend 的 `_preludeFailing` 表里却列着 `then`。 | 表里删掉 `then`。同批（链跑着时预读 stubs 表里第一帧 build/layout/paint 路径上的桩，各用 fixture 复现）：① hollow mixin 方法的体来自某个 application 的克隆，克隆的参数类型是 application 的（`ChildType?` 成了 `RenderBox?`），trait 却是 mixin 的——现在按声明的签名 lower 克隆的体，参数读取按声明定型（`RenderObjectWithChildMixin.child`）；② `late Widget built; try { built = build(); }`：try 在这里是闭包，闭包不能给未初始化的绑定赋值——声明在 try 外、在 try 里赋值的局部变量起始 `None`（option local），读时 unwrap（`ComponentElement.performRebuild`）；③ super 函数的体没设 Rust 返回类型，里面 `return` 的 try 带出的是 `Option<()>`（`inflateWidget`）；④ 泛型类的 const 实例缺 PhantomData 字段、且这些字段不是 pub（`PersistentHashMap.empty()`）；⑤ 静态类型 `Object` 的 `c ? a : b` 两支类不同时走 `dart_str`——把 `slotFor` 的 `Object?` 变成了 `String`；删掉这条，两支各自 widen 进 `Object`，插值自己会 `dart_str`；⑥ `BoxConstraints / double` 是类自己的运算符，不是往 `f64` 的 cast（`ViewConfiguration.fromView`）。 |
| ws476 | 链：**1483 → 1525（+42）**，142——mixin 签名那条改动的代价：application 克隆的体撞上 trait 更宽的类型，缺的是别处都会补的 cast。 | 按家族修：① 克隆的参数按声明定型用自己的表（不再借 `_retyped`——那个 fallback 漏进了闭包参数的重定型，`_getOverrideAction` 多了两个桩），返回值 widen 进声明的返回类型；② 经 trait setter 的写（`RenderView` 构造函数里的 `this.child = child`）widen 进**声明成员**的槽：克隆背后的 mixin 自己的成员（`RenderObject?` 不是 `RenderBox?`）；③ `child?.getDryLayout(..)`：trait 说 `RenderObject?`、体说 `RenderBox?`，`?.` 的绑定读取经对象收窄；④ `if/else` 两支的上转型显式化（`dart_object(FontWeight)` 对 `Rc::new(String)`）；`Object` 型条件表达式的 `dart_str` 只在插值片段里恢复（它本来就来自那里）；⑤ 返回 `Null` 的闭包（`then((_) {..})`）把返回类型写出来：`-> Result<_, _>` 下的 `Ok(None)` 推不出 Option 的参数（run477 的 `_initKeyboard`）。 |
| ws477 | 链：**1525 → 1448**（基线 1483 之下 35），142。相对基线还多 20 个：全是 `ContainerRenderObjectMixin` 一族（`insert`/`move`/`_removeFromChildList` 的克隆，`RenderFlex._lastChild` 是 `RenderBox?`、`previousSibling` 是 `RenderObject?`）+ `layoutInfo`（`LayoutInfoType` 是**保留**的类型参数，克隆里的 `BoxConstraints` 替换是那个 application 自己的）。 | 一条规则收口：**CFE 拷进匿名 application 的成员一律代表 mixin 的声明**——拷进应用类的字段和方法按声明的类型 lower，成员的读（`_memberRustType`）和写（`_writeLanding`）按声明定型；例外是声明里提到**保留**类型参数的地方，用克隆自己的替换。参数读取按声明定型后走原来的 promotion 路径（`Some(after)` 那种双包装是没走 promotion 的后果）。 |
...
| ws480 | 链**坏了**：reachable 34。"trait 声明的非 final 字段一律 cell" 太宽：一个字段进了 `Cell<Option<BorderRadiusTween>>`（`_isCopy` 误判它 Copy），错误在函数体外，widgets crate 及其上全没了；同时 `Tween<T>.begin` 一类 trait 视角更宽的字段（`T?` 擦成 `Object?`、struct 里是 `Color?`）的 setter 从 `todo!` 变成编不过的桩。 | 收窄：只有 trait 自己的体在 `this` 上赋值的字段才为它做 cell（setter 本来就为此存在）；setter 把值按 coerce 适配进字段自己的类型。另：`super.insert(child, after: after)` 的实参按 mixin 声明的槽填（`RenderBox?` 上转 `RenderObject?`）。 |
| ws481 | 链：**1425**（−15），142。新桩只有 `get_storage` 的 `flush`/`_madeBackup`（原来是拒绝，现在编到了类型上：`utf8.encode` 在 prelude 里返回 `Vec<i64>` 而 `Uint8List` 是 `Vec<u8>`；`json.encode` 的槽是 `Object?`，prelude 被调时实参不做适配，传进来的是 `Map` 本身）。 | prelude：`utf8.encode` → `Vec<u8>`；`json.encode<V: 'static>(value: V)` 自己装箱（blanket `Object` 的 `as_any` 会穿过句柄）。 |
| run481 | **栈溢出**：gdb 抓到 `foundation_platform::default_target_platform` 无限递归——`platform.dart` 的 `defaultTargetPlatform` 包着 `_platform_io.dart` 的同名 getter，driver 对模块自己定义的名字正确地不写 `use`，于是未限定的调用落回自己。 | 前端：调另一个库的顶层函数/getter 且本库也声明同名时，按那个库的模块限定（`IrStaticCall.module` → `crate::<module>::f()`），模块名由 driver 传入。fixture 两个库互相包裹验证过。同批：async 块里的 `Err(Rc::new(X))` 推不出错误类型，装箱的 throw 显式转成错误类型（`initServiceExtensions`）。 |
| ws482 | 链：**1418**（−7）；members 141（分区少了一个 crate，不是丢了——全部到齐）。 | |
| run482 | 过了 `RendererBinding`（`_manifold` 的 cell 生效）；`WidgetsBinding.initInstances` → `DefaultPlatformMenuDelegate::new` 桩：`const OptionalMethodChannel('flutter/menu')` 进 `MethodChannel` 字段要句柄，const 实例没带类型所以槽没适配。 | const 实例带 `rustType`。同批做了 `main` 路径上下一个已知的拦路石（path_provider 的 `MethodChannelPathProvider.getApplicationDocumentsPath` 是桩）：**trait 的泛型方法经 trait 对象调用**——Rust 把 `where Self: Sized` 的方法排除在 vtable 外，19 个桩。通用机制：每个 trait 泛型方法有一个 object-safe 的**擦除孪生** `m__erased`（类型参数读作 `Object`）：抽象方法旁声明、默认方法旁经 super 函数给默认体、每个覆盖它的实现里转发到自己的泛型版本；经 trait 对象（句柄或 trait 体内的 `this`）的调用走孪生，结果经 prelude 的 `CastErased` 转回调用点声明的类型（`FromDynamic` 的 `Option<T>`/`Future<..>`，`void` 丢弃）。fixture（`Optional extends Channel`）整 crate `cargo check` 通过。 |
| ws483 | 链：**1419**（+1：三个 `*__erased` 孪生在 trait 默认体里缺类参数的 `Clone` 界；−2 是 path_provider 的两个 `invoke_method`）。 | 孪生的声明/默认体带上 `_traitWhere` 的类参数界。 |
...
| ws490 | 链：**1338**（−57）；新桩 8，其中三个构造函数：`Map<SlotType, ChildType> _slotToChild = {}` 拷进类里——CFE 把 mixin 字段的初始化挪进 application 的合成构造函数，那里按克隆的类型（`RenderBox`）widen，而字段按声明擦成 `RenderObject`。 | 见 ws491 行。 |
...
| ws495 | 链**坏了**：reachable 32，桩 88。全类型 `FromDynamic` 漏两处：`Vec<E>: FromDynamic` 要 `E: Clone`，而声明级的参数界没有 `Clone`（`Equality<Iterable<E>>`）；`dyn Equality<<E as DartNullable>::Or>` 要 `Or: FromDynamic`。 | 集合的 impl 不再要元素 `Clone`：同型元素经 `from_same`（有 `Clone` 的型给克隆，没有的给 `None`），结果经 `from_converted` 直接拼（不走要 `Clone` 的 `insert`）；固定界改 `DartNullable<Or: Clone + DartEq + FromDynamic> + DartEq + FromDynamic`。 |
| ws496 | 链：**1303**（+1），141。新桩 14/去 13：新的全是 `Set` 接收者的 `any`/`expand`/`forEach`/`toList(growable)`——静态类规则生效后 owner 从声明类 `Iterable` 变成了 `Set`，Set 分支没有这些。去掉的含 `invokeMapMethod`、`visitChildren`（`Rc<dyn Element>: FromDynamic`）、get 的三个 listener。 | owner 一律取**声明类**（`Iterable`），只在 TFA 把目标虚化到翻译类上（`CanonicalizedMap.cast`）时才用接收者静态集合；`toList()` 在非 `Vec` 接收者上走 prelude 的 `to_list()`。同批：trait 体里经限定路径对 `this` 调泛型 trait 方法也走孪生（`getInheritedWidgetOfExactType<T>`）；泛型调用节点自带结果类型（投影包裹层曾把它盖住），投影的 `T?` 经 `from_option` 回来；`a ?? b` 的左边是局部变量时按克隆读（`inflateWidget` 的 `inactive`）。 |
...
| ws514 | 链：**1208**（**+47**；比 1300 少 92：新 56/去 139），141。`route_super_did_add`/`did_push` 去了（FutureOr 遍历修好了 `then<R>` 的回调）；但 `did_pop_route`/`did_push_route`（×22）、`_handle_navigation_invocation`、`send`、`obtain_key` 等新增：类型遍历看得见 `FutureOr<T>` 之后，prelude 槽位"按类型收缩"的门（`_mentionsTypeParameter`）对 `Future.value(FutureOr<T>?)`/`Completer.complete` 也开了，值被包成 `FutureOr::value(..)`，而 prelude 那里收的是 `Option<T>`。另两族：`as T` 的操作数是 `Option`（`Map<K, Object?>` 查出来的）或被 TFA 收窄成 `f64`（`Tween.lerp`），`from_dynamic(&x)` 类型不对。 | prelude 的 `FutureOr<T>` 槽位按形状（门上 `declared is! FutureOrType`，prelude 收 `T`）；到类型参数的 `as T` 先按记录类型解 `Option`、再把操作数收进 `dynamic`；`!as_opt` 接收者已是 `Option` 时穿过去。 |
| run514 | 停在 `widgets_binding.rs:1781` 的 stub（回归那一族）。 | 同上；另：bare `Function` 建成动态调用对象（`DartFunction { arity, call }`，`dart_function_object`/`dart_call_function`，两个方向都由 `coerce` 生成适配闭包；prelude 的 `then`/`catchError` 的 `onError` 收 `Option<Rc<dyn Object>>`、按元数派发，结果经 `dart_future_or_from_dynamic`）；`Function` 上的动态调用（`f(2)`）不再拒绝。夹具 fnobj。 |
| ws515 | 链：**1160**（−1；比 1300 少 140：新 30/去 160），141。回归收回（`did_pop_route` 族、`send` 等 30 个回来）；`Function` 模型带来 27 个新的：(1) 动态入口里 `dynamic → X` 没有规则的 X（枚举 `AnimationStatus`、`Set<WidgetState>`、`Option<ByteData>`、`()`）原样传过去；(2) `_customActionCallbacks[id]`/`widget.routes![name]` 这类查出来的函数值，IR 上记成 bare `Function`（`Map<K, V>` 的 V 是函数类型时 `V?` 的记录类型），实际 Rust 是有签名的 `Rc<dyn Fn>`，被当成对象去 `dart_call_function`；(3) `ObserverList.contains(listener)`：函数值进 `Object?` 槽时新建对象，身份丢了；(4) `Tween.lerp` 的 `unsafeCast<T>` 操作数是 TFA 收窄成 `f64` 的算式，`IrBinary` 没类型、没进 `dynamic`。 | (1) `coerceInto` 末尾兜底：`dynamic → X` 走 `dart_from_dynamic::<X>`（每个类型都有 `FromDynamic`）；(3) `DartFunction` 带 `identity`（原 `Rc` 地址）和 `original`（原有类型的句柄），`object_eq` 认它；进有类型槽先 `dart_function_same::<F>` 拿回原句柄，没有才做适配闭包；(4) `IrDowncast(.., 'f64')` 六处记 `double`，`from_dynamic(&(x as Rc<dyn Object>))`。(2) 待查。宿主：`_sendPlatformMessage` 的回调是 `DartFunction` 时经 `dart_call_function` 回。夹具 fnobj（含身份）、tween（AOT）。 |
...
| ws520 | **作废**：可达 crate 34/141，"stubbed 745" 没有意义。协变普查把 `RestorableEnum<T extends Enum>` 也擦了，`Enum` 是 dart:core 的、这里没有拼写，签名里的 `Enum` 是函数外的错误，`widgets` 整个 crate 及其上全部不可达。 | 协变参数只在 bound 有句柄拼写时擦（顶类型 → `Rc<dyn Object>`，翻译过的抽象类 → `Rc<dyn Trait>`）。另两处普查修正：构造/静态调用的形参类型直接代换声明类型（`computeFunctionType` 把类参数复制成结构参数，比较永远不等，每个泛型构造调用都被记成协变——`Animation<T>` 因此上榜）；`as` 不算流动（`this as Animation<double>` 是运行时检查，cast 表按实例化回答）；闭包自身的类型参数与其结构副本同名视为同一。带 `DART2RUST_TRACE_COVARIANT=1` 在 gallery 上普查：26 个站点、~45 个参数（Route 家族、Page、Tween、FormFieldState、LocalizationsDelegate、PopEntry、PopupMenuEntry、RestorableProperty、ImageProvider、MapEquality 等）。进 ws521。 |
| ws521 | 链：**1212**（**+99**；比 1300 少 88：新 140/去 236；比 1113 新 135/去 59），141。去的是 Route 家族（routes 6、navigator 4、provider 6、text_form_field 4…）；新的全是 `Tween<T>` 擦除的连锁：`Tween` 擦了、父类 `Animatable<T>` 没擦，`TweenImpl` 不再实现 `Animatable<f64>`（69 个 `?` 类型不合、61 个 mismatched、8 个 `ConstantTween: Animatable<f64>`…）。 | 擦除按继承链**全有或全无**：标记的参数若传给未标记的父类参数、或子类参数（`RestorableEnum<T extends Enum>`）擦不了，就去掉标记，到不动点——Tween、Restorable 家族由此退出（站点保持原状，比半擦少 stub）。走继承链时看穿匿名 mixin 应用（`ModalRoute<T> extends _App<T>`，去重后的应用在别的库里，链在 ModalRoute 断了，PageRoute 被误去）。 |
| run521 | **过了 runApp 的 view**，进了第一次 build，停在 `RootElement.mount`：`widget.as_any().downcast_ref::<RootWidget>()` 得 None——`Rc<dyn Widget>` 自己是 `'static`，blanket `Object` 给的是**句柄**的 `Any`，永远不是 `RootWidget`；所有对 trait 句柄的 `is`/`as` 都一直是假的。 | 降转和 `is` 的 `as_any()` 对句柄（`Rc<dyn Trait>`、`dynamic`、计数类的 `Rc<Struct>`）走 `.as_ref().as_any()`，问对象本身；`this` 和值照旧（43a5e745，进 ws522）。 |
...
| ws530 | 链：**1016**（+1：宿主的 `dump_render_tree` 自己没编过——`to_string_deep` 在 `RenderObject`/`DiagnosticableTree` 两个 trait 上二义，stubs.py 把它 stub 了），142。 | 宿主经 `RenderObject::to_string_deep(&*view, ..)`；`run_main.sh` 不再经 `head -60`（程序被 SIGPIPE 杀、状态是 head 的）。 |
| run530 | **get_storage 的中止没了**（async 返回 await + `readInto` 填缓冲区），native 里第一次出现 `ScheduleFrame`；但 render 树 dump 为**空**：`renderViews` 空——`View` 的 `RenderView` 没注册进 `RendererBinding`，之前"过了 flushLayout"是空转，绘制/合成自然没到。 | 查 `addRenderView`/`_viewIdToRenderView` 的翻译（是没走到 `RawView` 的 mount，还是 map 写在拷贝上）。 |
...
| ws535 | 链：**1047**（+32；新 32/去 0），142。新的全是转发本身：30 个 `will_pop`——经祖先 trait 转发的 async 方法外面又包了一层 `Ok(..)`（trait 方法已经返回 `Result`）；2 个 `did_update_widget`——`RestorationMixin<S>` 的 trait 签名写成了一个 application 的 `DatePickerDialog`（`_unapplied` 对被擦除的参数不做逆代换）。 | 两处都修（见 ws536）。 |
| run535 | **View 的 build 跑起来了**：`_ViewState.build` → `State.widget` 在 `_widget` 为 `None` 上 panic。dill 里 `StatefulElement` 的构造体有 `state._element = this; state._widget = widget;`，IR 里也有（`DART2RUST_TRACE_CTOR`），但 `StatefulElementImpl::new` 的链块里只绑了参数没跑体：后端从 `bases.take(kept)`（最深的那头）取体，而 `chain` 削掉的是靠近的那头没体的基类（`Element`、`ComponentElement`），取到的是 `DiagnosticableTree` 的空体。 | 改成 `chain.reversed`；夹具 openctor（open 类上面再压两层无体基类）。 |
| ws536 | 链：**959**（−56 对 1015；按文件比：新 4/去 60），**138 crate、可达 141**——不是少了一个可达的：`widgets_restoration`/`widgets_scrollable` 不再 `use material_date_picker::DatePickerDialog`，cupertino/material 从 widgets 的环里出来自成 `merged_cupertino_material`（scc_flutter_widgets 从 429 文件降到 378）。去的 60 全是 `did_update_widget` 一族；新 4 个是 `_addDiagnostics` 里 `child.to_diagnostics_node(..)` 在 `DiagnosticableTree`/`DiagnosticableTreeMixin`/`RenderObject` 三个 trait 间二义。带的改动：`will_pop`（`via` 转发不包 `Ok`）、`_unapplied` 对擦除参数也逆代换、`_superSlots` 对声明已被 TFA 删掉的拷贝按 mixin 签名定槽（去重 application 从 `implementedTypes` 找 mixin）、`_staticType` 读拷贝参数按声明类型（夹具 unapply：mixin 的 super 体不再向一个 application 的类下转）、`join` 按元素类型转字符串（`String` 不再带引号）。 | ws537 带构造链修复。 |
...
| ws543 | 链：**903**（+18 对 885；新 18/去 0），141。新 18 全是借出机制的边角：顶层函数的参数没标（`mergeSort`）、super fn 的参数拼写没走 `_param`、闭包拷贝的捕获量借出时没 `mut`、协变借出（`List<ContainerLayer>` 借给 `List<ContainerLayer?>`）、类型参数上调 bound 的方法（`T extends InheritedModel` 的 `isSupportedAspect`）、泛型被调方的 `T?` 结果按边定型（`Or`）却被当 `Option<T>` 包了 `from_option`。 | 逐个修（见 ws544 前）。 |
| run543 | 过了 `inheritFrom` 的拒绝，停在它体内的 stub（上面的 `Or`/`Option<T>`）。 | |
| （ws544 前） | 修：顶层参数也标 `mutRef`；super fn 参数按 `&mut`；闭包捕获拷贝按体内写/借出加 `mut`；协变借出先按槽位类型转换再借出临时量（填进临时量，调用方拿不到——与原来拷贝一样，记为已知缺口）；`IrMethod.typeParameterBounds`：方法类型参数的 Dart bound 是翻译出的抽象类时加 ` + Trait` 约束；泛型被调方返回 `T?` 且实参是本处类型参数时结果按边（投影）定型，交给 coerce 转换；类型参数下转（`FromDynamic`）前先 `.clone()`（`m is T && m.supports(..)` 曾 move 两次）。**toString 协议**：`DartAny::dart_to_string`（默认 `Instance of 'X'`，后端给每个 struct/enum 写自己的：类或最近祖先的 `toString`，枚举 `X.value`），注册表 `DART_STRINGS`，`dart_object_str`（dynamic/Object/类型参数：标量、String、Null、Type、Option 直接答，其余查注册表），前端 `_stringOf`：翻译类走 `!dart_to_string`，可空的 `null` 或之，`List`/`Set` 逐元素 `[a, b]`/`{a, b}`（`join` 的元素规则也走协议），`runtimeType` 在翻译类上走 `dart_runtime_type`，prelude 的 `runtime_type` 把 `i64`/`f64`/`bool`/`String` 叫 `int`/`double`/`bool`/`String`；装箱进 `Object` 的 `int` 字面量带 `i64` 后缀，枚举经 `dart_object` 注册。闭包里对捕获列表的 add/借出让它变 cell（`_CapturedWrites`）。夹具 tostr 只剩两处：`Box<Shape?>` 里 `T = Option<Rc<dyn Shape>>` 的 `$value`（类型参数要 `DartAny` 约束才能精确，记为下一步）、`1.runtimeType`（裸字面量 i32）。 | ws544 量。 |
| ws544 | 链：**1155**（+252 对 903；新 290/去 38），141。方法类型参数上的 ` + Trait` 约束撞上擦除孪生（`__erased` 用 `Rc<dyn Object>` 实例化 `T`，句柄不是 trait：332 个 `Rc<dyn Object>: InheritedWidget`……）。**撤回**约束（IR 字段留着）。 | 改走对象协议：`T` 上调 bound 成员 = 经对象自己的 cast。 |
| （ws545 前） | **DartAny 成为类型参数的通用约束（Object 协议第二阶段）**：prelude 给一切能当类型实参的值实现 `DartAny`——`Rc<T>`/`Box<T>` 转内层（整套：runtime_type、to_string、cast、`dart_any_ref`），`dyn Object`（注册表），`Option<T>`（`null`/内层），`Vec`/`Set`/`Map`/`VecDeque`/`MapEntry`/元组（Dart 的打印形状 `[a, b]`、`{a, b}`、`{k: v}`），标量/String/Null/Type（Dart 的名字与文本），prelude 的其余值类型按名，`dyn Fn(..)` 各元数（`Closure`，同时给它 `Object`）；`DartCastExt` 只剩对 `DartAny` 的一条 blanket（原来 `Rc<S>`/`Option<S>`/`dyn Object` 三条专门 impl 删掉）；后端三处通用约束串加 ` + DartAny`；coerce：类型参数的值进 trait 槽 = `IrCastTo`（`dart_cast_to`，句柄转发、struct 自答）；`dart_object_str<T: DartAny>` = `value.dart_to_string()`；`_receiver` 只向翻译出的抽象 bound 收窄（向 `Object` 收窄曾把 `key.hashCode` 装箱后按地址算）。全部 10 个夹具与 dart 逐字一致（tostr 仅剩裸字面量 `1.runtimeType` 的 `i32`）。 | ws545 量。 |
...
| ws549 | 链：**796**（−67 对 863；新 2/去 69），141。新 2：`rendering_box.rs memoize`（局部函数装成句柄后递给 `impl FnOnce` 槽）、`material_time_picker.rs paint`（`Rc` 非 `Copy` 的 move）。 | |
| run549 | 仍停在 `_FocusState.build`：适配器没造——撕下来的 `requestFocus` 是经 `cond ? f.requestFocus : null` 这样的**非字面量**进槽的，而元数适配只认闭包字面量；且撕取适配器的类型按槽位记成 0 元、闭包本身写着 1 元，`sameRust` 的快路径先把它放过了。 | coerce 的函数规则重写成"元数感知"：值的参数以闭包字面量自己的为准，多出的可空参数一律补 `None`，非字面量也造适配器（绑 `__f`）；`sameRust` 快路径对元数不符的字面量不再放行。装箱闭包按自己的签名拼 `as Rc<dyn Fn(..) -> ..>`（`.map(|__f| ..)` 里推不出 unsize）。局部函数是句柄后：递给只调用它的参数（`impl Fn`）时借出闭包本身 `&*f`（`!fn_ref`，`_boxedFunctionLocals`）。`LinkedList`：prelude 从 `VecDeque` 别名改成侵入式实现（`Rc<RefCell<VecDeque>>` 句柄 + 按地址的旁表，`LinkedListEntry` 在每个 `Rc<E>` 上：`insertAfter/insertBefore/previous/next/unlink/list`），`first`/`last` 在队列类接收者上是方法。另一对象的字段读用同一 cell 谓词（`_inCellOf`）。夹具 linked、closureslot 与 dart 一致。 | ws550 量。 |
| ws550 | 链：**988**（+192 对 796；新 208/去 16），141。装箱闭包的 `as Rc<dyn Fn(..) -> Result<Rc<dyn Widget>, ..>>`：`as` 不把期望类型传进闭包，闭包体里 `Ok(具体 Rc)` 不再向 `dyn Widget` unsize。改成带类型的 `let __h: .. = Rc::new(..)`（期望流进闭包）。run550 因此停在 `RawView.build` 的这类 stub，作废。 | ws551 量。 |
...
| ws554 | 链：**783**（持平），141。 | |
| run554 | 事件循环的 catch 在 `panic = "abort"` 下抓不住（工作区故意 abort：unwind 的落点让巨型 widget 树函数 codegen 半小时）——改成 panic hook 里做 `report()`（反正要 abort，dump 失败也只是同样 abort）。dump 出来 render 树是**空串**：Flutter 的 `toStringDeep` 把渲染放在 `assert(() {..}())` 里，release dill 没有。**Goal 3 的尺子改为结构化遍历**：每个 render object 一行——`runtimeType`、laid-out box 的 `size`、`BoxParentData` 的 `offset`（`~/gallery_upstream/test/dump_render_walk_test.dart` 打参考 `$S/ref_render_walk.txt`，6 行；宿主 `dump_render_tree` 走同样的遍历）。但元素树在 `_FocusInheritedScope (_InheritedNotifierElement)` 就断了，render 树只有 view：追踪见 `_InheritedNotifierElement` 一开始 `dirty=false`——它自己声明了 `bool _dirty = false;`，与另一库的 `Element._dirty` **同名私有字段**（Dart 私有名按库区分，是两个字段），拍平的 struct 只剩一个。通用修：`_memberName`——私有字段与另一库祖先的同名私有字段撞名时，下层的按库标签改名（`_dirty_inherited_notifier`），声明和所有引用都经它。两库夹具 privfield 与 dart 一致。 | ws555 量；run555 看遍历。 |
| ws555 | 链：**787**（+4 对 783），141。新 4：宿主的 `dump_*`（`workspace.py` 的 `runtime_modules` 列表少了个逗号，`rendering_box` 没导出）、`nested.rs set__parent` ×4（provider 的 `SingleChildWidgetElementMixin._parent` 与 `Element._parent` 撞名——改名只认了 `Field`，空心 mixin 的字段是 getter/setter 对，而且 mixin 自身的父类是 Object 查不到冲突）。 | |
| run555 | 元素树建过 `_FocusInheritedScope` 之后停在 gallery 自己的代码：`GalleryOptions.of` → `dependOnInheritedWidgetOfExactType<_ModelBindingScope>()` 得 null——`_inheritedElements[T]` 的键是类型参数的**类型字面量**，被拼成了 Kernel 节点文本 `Type::of("TypeParameterType(..)")`。通用修：类型参数的类型字面量 = `dart_type_of::<T>()`（从 Rust 类型名取类名，`Rc<dyn X>` 是 `X`），擦除的按 bound。改名机制推广：私有字段/getter/setter（含空心 mixin 的访问器对），按声明（`_originalOf`）缓存；mixin 成员经它的每个 application 的父类链查撞名。夹具 privmixin（两库、mixin）、typelit（`Map<Type, W>` 按 `T` 查）与 dart 一致。已知缺口：泛型 mixin 的 `T?` 字段 getter（`Or` vs `Option<T>`）。 | ws556 量；run556 看遍历。 |
| ws556 | 链：**783**（持平；新 2 `nested.rs activate`/去 2），141。新 2：改名两头不一致——去重的 application 里的拷贝按 `dart:mixin_deduplication` 打标签（`_parent_mixin_deduplication`），引用按 mixin 的库（`_parent_nested`）。改成按声明类（拷贝归其 mixin）+ 名字缓存，标签取声明类的库。 | |
| run556 | 仍停在 `GalleryOptions.of` 的 null：类型字面量对了，但 `PersistentHashMap` 里 `key == keyOrNull` 走的是 `dyn Object` 的 `==`——`object_eq` 只认标量/String，其余按地址，`Type` 值比不等。通用修：**`==` 进 Object 协议**——`DartAny::dart_eq_any`（后端给每个 struct/enum 写：downcast 后 `dart_eq`；prelude 的值类型按值；句柄/Option 转发；注册表 `DART_EQS`），`object_eq` 先问它。夹具 objeq（`Map<Object,..>`/`Set<Object>` 以 `Type`、自定义 `==` 的类、String、int 为键）与 dart 一致。 | ws557 量；run557 看遍历。 |
| ws557 | 链：**781**（783→781；去 2 `dart_eq`/`hash_code` 类），141。`dart_identical`（Object 型的 `identical` 按指针或规范核心值）、`dart_shl/shr/ushr`（`>>>` 移位数≥64 时 Rust 溢出，Dart 得 0）。 | |
| run557 | `GalleryOptions.of` 仍 null：追踪显示 `_ModelBindingScope` 存了、又用同名查，map 也在——问题在 `PersistentHashMap` 的翻译本身。夹具 phmuse（Flutter 的 persistent_hash_map.dart 原样 + 以 Type/String/int/自定义类为键）：先 `>>>` 溢出→移位助手；`identical` 在下标上拒绝→`dart_identical`；然后**栈溢出**：`_resolveCollision` 拿到同一个键的两个 hash——`K` 型算 `dart_hash_code`（类自己的 `hashCode`），存成 `Object` 后再算是 `Rc` 的地址 hash——两个 hash 永远不等，一路递归到底。通用修：**`hashCode` 进 Object 协议**——`DartAny::dart_hash_any`（后端给每个 struct/enum 写：`dart_hash_code`；prelude 值类型按值；`dyn Object`/`Rc`/`Option` 转发；注册表 `DART_HASHES`），`RcHashCode` 与 `DartEq for dyn Object` 都问它；后端 `DartEq::dart_hash_code` 走类自己的（或祖先的）`hashCode` getter，计数类无 getter 时按身份。phmuse/hashtrie/objeq/identconst 与 dart 一致。 | ws558 量；run558 看遍历。 |
| ws558 | 链：**781**（持平，stub 无变化），141。 | |
| run558 | `GalleryOptions.of` 仍 null，但原因换了：trie 里 `key == keyOrNull` 已对（`eq=true`），取回的值是 `Rc` ——泛型 `V` 装箱走 `Rc::new(value.clone())`，`Rc<dyn InheritedElement>` 被再套一层成 `Rc<Rc<dyn ..>>`，`from_dynamic::<Rc<dyn InheritedElement>>` 找不回来。通用修：**装箱进 Object 槽取"值本身是的对象"**——prelude `dart_boxed<T: DartAny>(v)`：先 `dart_cast_to::<dyn Object>()`（struct 的 `dart_cast` 新增答 `dyn Object`：自己的那一个 handle），无则 `Rc::new(v)`；后端类型参数值上转 Object/dynamic 槽改走它；`FromDynamic for Rc<T>` 兜底也认"装箱了的 handle"。夹具 phmuse 加 `PersistentHashMap<Type, Base>` 存子类取回调方法：一致。（不可表示的旁支：无 `==` 的值类做键——Dart 按身份，值 struct 按结构，AOT 还会删掉未读字段；记下，不在路径上。） | ws559 量。 |
| ws559 | 链：**788**（+7）。全是同一件事：`dart_boxed(unreachable!(..))`——`!` 不是 `DartAny`，never type fallback 报错。修：`IrLiteral.unreachable` 一个实例（frontend 用它，backend 按身份识别，`IrThrowValue`/`Never` 型/尾随的块值同论）→ `dart_boxed::<Null>(..)`。 | ws560 量。 |
| ws560 | 链：**781**（回到 ws558；stub 无变化），141。 | |
| run560 | **`GalleryOptions.of` 过了**（`_ModelBindingScope` 取回来了）。下一站 `InheritedModelElement.updateDependencies` 的 stub（早就在：`type annotations needed`——`dart_boxed(Set::new())`，`HashSet<T>()` 丢了 `T`）。两处通用修：(1) frontend 的 `HashSet/HashMap` 工厂路径带上类型实参（`Set::<T>::new()`）；(2) `_erasedParameter`：非翻译类（prelude 映射的 `dart:` 类）的类型参数从不擦除——擦除是本编译器所写声明的性质。夹具 hsetbox（泛型类里 `HashSet<T>()` 进 Object 槽、`as Set<T>?` 取回再 `..add`）与 dart 一致。 | ws561 量；run561 看下一站。 |
| ws561 | 链：**780**（-1：`update_dependencies` 出了），141。 | |
| run561 | `updateDependencies` 过了；下一站 google_fonts 的 `loadFontIfNecessary`（catch 里 `print(..)`）：**顶层 `print` 拒绝**。通用修：`dart:` 顶层函数→prelude 的**一张表** `_coreTopLevel`（`dart:core.print`→`dart_print(Rc<dyn Object>)`，`dart:async.scheduleMicrotask`→`_schedule_microtask`，原来的 if 并进去），表里带每个参数的 Rust 槽（prelude 被调方的槽 `_widened` 不去扩，这里显式 `coerce`）。顺带：字面量集合装箱带类型（`dart_boxed::<Vec<i64>>(vec![1, 2])`，否则 `Vec<i32>` 打成 `Instance of 'int'`）。夹具 printcore（print 字符串/类/null/double/`Object?` 里的 list）与 dart 一致。 | ws562 量；run562 看下一站。 |
| ws562 | 链：**780**（持平：`get_asset_variants` 出、`load_font_if_necessary__body` 进——原先整个拒绝，现在译出来了，卡在 `await byteData`（`Future<ByteData?>?`）——`match byte_data {..}` 把局部里的 future 移走，下一行又读）。通用修：可空 future 的 await 在**地方**（局部/字段）上 match 它的句柄副本 `(x).clone()`。夹具 awaitnull 编译并与 dart 一致。 | ws563 量；run563 看下一站。 |
| ws563 | 链：**779**（-1：google_fonts 的 `load_font_if_necessary__body` 编过了），141。 | |
| run563 | 过了字体加载；下一站 `GoogleFontsVariant.toString` 的 stub：`replaceFirst(from, to, [startIndex])` 的可选第三参前端照填 `0`，prelude 的 `dart_replace_first` 只收两个。修：prelude 按 Dart 的元数收 `start_index`（`substring`/`indexOf` 早就这样）。夹具 replfirst 与 dart 一致。 | ws564 量；run564 看下一站。 |
| ws564 | 链：**762**（-17：全是 `replaceFirst` 的调用方——本地化的 super 转发、`StackFrame` 解析、字体 variant），141。 | |
| run564 | 字体 variant 过了；下一站 `ThemeData()` 构造函数 stub：`splashFactory ??= useMaterial3 ? (useInkSparkle ? A : B) : C`——`??=` 的临时 `let` 无标注，三个 const 类进一个 trait 槽，`if` 的臂各自 `dart_object(..)` 不带 `as`。`expr` 的条件式早就给每臂 `_explicitUpcast`，`let` 走的 `_returned` 的条件式没有。修：`_returned` 同样。夹具 condconst 与 dart 一致。 | ws565 量；run565 看下一站。 |
| ws565 | 链：**761**（-1：`ThemeData::new` 编过了），141。 | |
| run565 | `ThemeData()` 过了；下一站 `Typography._withPlatform` 的 stub：`final (a, b) = switch ..` 记录模式——CFE 的缓存临时量读 `#t9.$2` 两次，Rust 里 `__t9.1` 按值读是移动。修：地方（局部/字段）里的记录字段读 `.clone()`。夹具 recdestr 与 dart 一致。 | ws566 量；run566 看下一站。 |
| ws566 | 链：**750**（-11：记录字段读的 clone——slider/switch 的 `paint`、`RenderWrap` 布局、tooltip、container padding 等），141。 | |
| run566 | `_withPlatform` 还是 stub，换了错误：`__t10` possibly-uninitialized——switch 表达式的带标签块；`_fallsOutUnassigned` 要求最后一句是无 else 的 `if`，而 TFA 把 `null` 臂删成 `{ ; }` 留在末尾。修：末尾的空语句/空块不算数（`_lastMeaningful`/`_saysNothing`）。夹具 recdestr 加只以非空调用的 `resolve2`（复现 TFA 删臂）：与 dart 一致。 | ws567 量；run567 看下一站。 |
| ws567 | 链：**749**（-1：`_with_platform` 编过了），141。 | |
| run567 | `Typography` 过了；下一站 `ThemeData._themeExtensionIterableToMap` 的 stub：`Map<Object, ThemeExtension>.unmodifiable(<..>{..})`——prelude 被调方的槽 `Map<dynamic, dynamic>`（Kernel 对裸 `Map` 参数的拼法）被当作要 coerce 的，值 map 的 V 被扩成 `Rc<dyn Object>`，而 prelude 的 `unmodifiable(other: Map<K, V>)` 是泛型。修：`_calleeTranslated` 对**全 dynamic 实参**的槽（裸泛型）返回 false；`Map<String, dynamic>` 这类部分 dynamic 照旧。夹具 unmodmap 与 dart 一致。 | ws568 量；run568 看下一站。 |
| ws568 | 链：**750**（+5/-4）：全 dynamic 槽一律不 coerce 过头了——`postEvent(String, Map)`、`Timeline.startSync(.., Map?)`、`ImageCache` 的 `Map<String, dynamic>?` 事件参数在 prelude 里就是 `Map<Rc<dyn Object>, ..>`。收窄为**集合构造/工厂**（`Map.unmodifiable`、`Set.from`、`List.from` …）自己的裸集合槽。 | |
| ws569 | 链：**745**（-4：`_themeExtensionIterableToMap` + 三个 l10n `<static>`（`HashSet<String>.from([...])`）），141。 | |
| run569 | **主题建完，元素树长到 `WidgetsApp → RootRestorationScope → SizedBox`，渲染树有了 `RenderConstrainedBox`**。第一处 panic 挪到字体加载的 future 里：`AssetManifest.loadFromAssetBundle` 把工厂 `_AssetManifestBin.fromStandardMessageCodecMessage` 当函数值传——`ConstructorTearOffConstant` 拒绝。通用修：构造/工厂撕下 = 类的关联函数（`IrFunctionRef(类, 名)`，无名→`new`，`_`→`new_`），类型由其 FunctionNode（生成构造函数的返回补成类本身）；可达性 census 同样计入。夹具 ctortear 照 `CachingAssetBundle.loadStructuredBinaryData` 的形状写，再揪出三件：(1) `_typeKept` 不进 `FutureOr<T>`——parser 槽 `FutureOr<T> Function(..)` 的 `T` 没被调用处的实参替换，适配器里 `_Bin` 没上转成 `dyn Manifest`；(2) 裸保留类型参数的 `T?` 槽是投影 `<T as DartNullable>::Or`，值走 `from_option`（声明早就这么拼，`_widenedInto` 尾巴的 `Some` 不是）；prelude 被调方的 `FutureOr<T>?`（`Completer.complete`）同槽；(3) prelude `future_value` 的槽也改成 `Or`（原 `Option<T>`，和 `complete` 不一致），无参时 `from_option(None)`。ctortear 与 dart 一致。 | ws570 量；run570 看下一站。 |
| ws570 | 链：**755**（+12/-2）。三组回归：(a) `Future<void>.value()` 无参——省略的可选参填成 `None`，槽改成 `Or` 后 `()` 不是 `None`：前端无参就不传参，prelude `future_none<T>()`；(b) 投影包装用到了函数体自己的 `T? x = ..` 局部（`DiagnosticsProperty.getChildren` 等）——函数体里的局部是 `Option<T>`，只有**实参边**才投影（`_argumentEdge`）；(c) 构造函数撕下进带命名参数的函数槽（`RoundedRectangleBorder.new` 作 `ShapeBorder Function({side, borderRadius})`）——命名参数顺序适配器只认 `StaticTearOffConstant`，改认 `TearOffConstant`，且适配器的结果 coerce 进槽的返回（`Rounded` → `dyn Shape`）。夹具 tearnamed 与 dart 一致。 | ws571 量。 |
| ws571 | 链：**743**（-2 vs ws569：cupertino_dialog 的两个；ws570 的 +12 全回去了），141。 | |
| run571 | 撕下过了；下一站 `CachingAssetBundle.loadStructuredBinaryData` 的 super 函数 stub：闭包按值拷进 `final` 字段（`_structuredBinaryDataCache`）时拼成 `this_.field.clone()`——trait 体里字段是访问器。修：`_copyOf` 在 `_fieldsAreAccessors` 下走访问器——共享字段取 `_cell()`（闭包和对象共一个 map），普通字段取值，late 再 `unwrap`。夹具 supercap 与 dart 一致。 | ws572 量；run572 看下一站。 |
| ws572 | 链：**740**（-3：`loadStructuredBinaryData` 等三个 super 函数），141。 | |
