...
现在是队头,也是**最大的一块**,而且和可变性纠缠——
Dart 的闭包捕获可变状态,Rust 的 `Fn`/`FnMut`/`FnOnce` 有借用规则。
第 12、13 轮建的 `_mutating` 传播会用得上,但**捕获是跨函数的**,
比类内传播难一个量级。

**动手前先量**(这是第 3、10、14 轮反复证明有用的一步):

1. 有多少闭包**不捕获任何东西**(纯函数,直接 `fn` 或 `|x| ...`)
2. 有多少只捕获**不可变**的
3. 有多少捕获**可变**状态(要 `FnMut`,而且捕获物的生命周期要能说清)
4. 有多少捕获 `this`(Flutter 的回调几乎全是这种)

第 4 类大概率占压倒多数,而它正是最难的——`this` 是 `&self`,
闭包要活得比调用久,Rust 这边要 `Rc`/`Weak` 或者别的所有权安排。
**这可能不是一轮能做完的,先量清楚再决定切多大一块。**

## 当前队头(685 文件 / 3331 个类 / 9392 次拒绝)
...
| 419 | super 构造函数调用 |
| 395 | 方法引用(撕下来的方法) |
| 373 | 赋值给 `a.b`(另一个对象的字段) |
| 314 | 字符串插值 |
| 300 | getter 引用(泛型替换后) |

**591 + 300 = 891 的"getter 引用未解析"值得先查一眼**:
这可能是记账问题而不是真缺口(第 3 轮 `abstract method` 74→57 就是这样),
如果是,它比闭包便宜得多。

## 两条要记住的警告

1. **"零拒绝"(870 / 3331)不等于"翻译好了"**:引用其他库的东西仍靠手写桩。
