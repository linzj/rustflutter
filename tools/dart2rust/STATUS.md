...
- **当前读数（ws1126）**：拒绝 **9**（ws1101 的 12 起算;ws1123 去掉 `Platform.toJson`,ws1124 去掉 `_addLicenses` ×2），桩 **16**（ws1098 的 36 起算，每轮 gone N / new 0，`stubdiff.sh` 逐字节；ws1124 → ws1126 gone 2：`scheduleTask` 的 super 函数与 `WidgetsBinding` 的副本），unstubbable **0**，可达 **69**，0 error；渲染树连采五次 **707 节点 / 0 panic / 0 类型差异**；夹具 **126 个，125 AGREE + `ffistruct` 故意红**；panic 尺子 **「程序还能走、这边走不了」的中止点 = 0**（ws1121 的 `fold` 两处 `expect` 让它变 2,ws1122 改成不中止的写法后归 0）。
...
- **16 条桩,逐条写下理由(ws1126;ws1119 的 21 条去掉 `_initLicenses`、`LicenseRegistry.licenses`、`_PackagesViewState.licenses`、`scheduleTask` ×2)**。按「那个程序」的理由分:
  ...
  - **本编译器的欠账,6 条,不是程序的理由(ws1126)**:dart:io 的 `HttpClient`/`Stream.pipe` 不在 prelude(`io_client send`);`TypedData` 在 prelude 是空 struct,而 `Uint8Buffer._buffer` 是 `Vec<i64>`,一个 `List<int>` 字段装着 Uint8List 时元素宽度已经丢了,`_buffer as TypedData` 无处落(`typed_buffer` ×2);擦除的 `T?` 字段的 `?` 只活在字段拼法里(`_RouteEntry.handleDidPopNext`、`push_named`,见下面两条痕迹);槽位 mixin 三处改动要一起做(`slotted…debugDescribeChildren`)。
