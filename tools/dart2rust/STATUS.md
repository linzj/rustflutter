...
- 夹具尺子：`bin/fx.sh` / `bin/allfx.sh`，判据是 **panicgate**（panic 一律不算过，先于退出码查）。2026-09-11（`65d1aa91`）起缓存按**输入分两半**：`lib/prelude.dart` 只声明一个 `const rustPrelude`，且只被逐字写成 `dart_prelude.rs`，所以改 prelude 不可能改动别的产物——**没动**的一遍 6m51 → **11.6s**（判据逐条相同），**只动 prelude** 的一遍每个夹具 14s → 5.2s，动后端仍是全量。缓存戳从前读在翻译**之后**，只省了 cargo 那 3.5s。
...
- **panic 尺子 `bin/panic_ruler.py`（ws1094 新增）**：把工作区里**每一种会中止的写法**数全——`panic!`/`unreachable!`/`todo!`/`assert*`/`.expect`/`.unwrap()`——**先出总数再分类**，对不上就报 MISCOUNTED。它取代的是 `grep -c 'panic!("uncaught'`：那是一行的模式，而 `erased_cast_failed` 的字符串写在 `panic!(` 的下一行，所以尺子报 0、东西还在、六处在调它。**量一个字符串只会静默地错，量总数不会。**
  - **`a8055b16` 又发现它自己在发牌照**：它把任何认不出的 `dart2rust:` 消息折进一个桶 `dart2rust: .. (other)`，并把那个桶算作合法，于是宣布规则收尾的那天它的最后一行写着 0，真数是 **516**。前缀是这个编译器写的，前缀后面说的话却未必是这个编译器的事实——`panic!("dart2rust: a {} where a `{}` was wanted")` 就是一个挂着本编译器名字的 Dart `TypeError`。桶已删，每条消息各按原文单列；**新出现的一条在有人替它辩护之前一律算不合法**。
- **当前读数（ws1096）**：拒绝 **12**，桩 **36**（桩集合与 ws1094 逐字节相同），unstubbable **0**，可达 **69**，0 error；渲染树连采五次 **707 节点 / 0 panic / 0 类型差异**；夹具 **104 个，103 AGREE + `ffistruct` 故意红**；panic 尺子 **“程序还能走、这边走不了”的中止点 = 510**（生成侧 9,947 处、prelude 19 处）。510 里 **495 是 `_hardUnwrap` 两家**——“懒 `late` 格子，读在写它那行的下一行”和“刚用 `is_none` 试过的 `Option`”——按规则自己的问句它们说的是**翻译器**的事实，但目前没有任何东西让它们上合法名单。prelude 剩的 19 里另有三条说的是程序的事：`Future.delayed`/`Future.value` 的非空 T 取 null、`Sink::close` 没有错误通道；一条 `dart:ffi has no Abi.values index` 其实是拒绝，只是没按拒绝措辞写。
...
- **ws1094 那句“没有一处再说‘那个程序’的事”是错的**，错在尺子而不在手段：见上面 panic 尺子那条。真数是 516；ws1096 把 `dart_from_dynamic`（128 处调用）、`dart_function_of` 的三个提问者、`json_write` 的兜底和三处 Completer 改成值之后是 **510**。顺手补上一个自己挖的坑：`DartFuture::map` 的回调槽是 `Fn(T) -> U`，于是 `dart_from_dynamic` 在它体内没有 `Result` 可走，冒出 4 个**匿名** `.unwrap()`——比有名字的 panic 更坏。改在槽上：回调可失败，它的错误就是这个 future 的错误，正是 Dart 对会抛的 `then` 的做法。
...
**一句话**：翻译与渲染结构已接近收尾，数字停在 **12 拒绝 / 36 桩 / 69 可达 / 707 行 0 差异 / 510 个“程序还能走”的中止点（其中 495 是懒初始化的尾读）**；再往下主要是运行时子系统与值语义/别名工程。
