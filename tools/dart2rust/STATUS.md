...
## 下一步:闭包(974 + 740 = 1714,新队头)

躲了很多轮,现在是队头而且遥遥领先。**和可变性纠缠**:
Dart 的闭包捕获可变状态,Rust 的 `Fn`/`FnMut`/`FnOnce` 有借用规则。

**先量四类**(这一步在第 3、10、14、15、18、19、20、21 轮都值回票价):

1. 不捕获任何东西 → 直接 `fn` 或 `|x| ...`
2. 只捕获不可变的 → `Fn`
3. 捕获可变状态 → `FnMut`,捕获物的生命周期要说清
4. **捕获 `this`** → Flutter 的回调几乎全是这种,最难:
   `this` 是 `&self`,闭包要活得比调用久

第 4 类大概率压倒多数。**量清楚后先切第 1、2 类**,别一次吃下。

## 也该记一笔:具体基类的方法还没扁平化

这一轮扁平化了**字段**和**构造函数**,但**具体基类的方法**没有。
`Padded extends Square`,如果 `Square` 有方法,`Padded` 现在够不着——
抽象基类走 trait,具体基类什么都没有。fixture 里 `Square` 恰好没方法,
**所以这个缺口没有被测到**。记在这里,别等它咬人。

## 当前队头

**Kernel(发布路径,664 库 / 3665 类 / 10838 次拒绝)**
...
1. **"零拒绝"(Kernel 846 / 3665)不等于"翻译好了"**:引用其他库的东西仍靠手写桩。
2. **`testdata` 的桩已经吃力**。真正的答案是翻译 `dart:core` / `dart:ui` 的最小子集。
3. **哪把仪器守哪条守卫。** 断言、rustc、前端对照各有盲区。
4. **短路语义最容易静默出错**(`??` 错了十七轮)。
5. **fixture 里的值要能分辨错误配对。** 第 21 轮参数反序的变异能编译,
   只有"两个参数不同"的那条测试杀得掉它。
