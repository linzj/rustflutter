...
(525 库 / 2743 类 / 1231 个零拒绝的类 / 11614 个成员发出 / 3354 次拒绝)

| 次数 | 要建的东西 |
|---|---|
| 587 + 425 | **闭包捕获 `this` / 撕方法——同一件事,见第 30 轮的测量** |
...
**节奏**(用户 2026-09-02 两次指出):一轮攒几个 blocker 一起实现,最后统一验一次;
`fixtures.py` / `regen.py` 并发跑;**变异扫描只碰它够得到的 fixture**
(`bin/mutate.py`,一轮 55 秒而不是 5 分钟);变异只针对**承重的**判断,
不为每个改动都配一个。
