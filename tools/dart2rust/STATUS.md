...
## 下一步:`try/catch` —— `Result` 的另一半

`throw` 做完了,现在缺**接住它**的一侧(171 处 `try/catch`,174 处
`on <Type> catch`)。Rust 这边是 `match`:

    match self.checked(v) { Ok(x) => ..., Err(e) => ... }

`?` 是"往上传",`match` 是"在这里停"——而第 24 轮量到 `try` 只挡住了 20 个成员,
所以**接住的地方很少,但它是传播唯一的边界**,做完之后传染范围才算准。

三件要当心的:

1. **`on <Type> catch` 要类型匹配。** 现在每个函数的错误类型是**一个具体类型**,
   所以 `on RangeError catch` 就是 `Err(e)` 分支;但 `on Exception catch`
   抓的是基类,而基类没被翻译——**先量那 174 处抓的都是什么类型**。
2. **带 stack trace 的 catch 有 133 处(占 78%)。** `Result` 不带栈。
   这一条**大概率带不过来**,要如实记成损失而不是假装它在。
3. **`try/finally`(73)和 `Result` 是分开的问题**,Rust 的答案是 `Drop` 守卫。

## 当前队头

**Kernel(发布路径,664 库 / 3665 类 / 10040 次拒绝)**

| 次数 | 要建的东西 |
|---|---|
| 904 | 闭包捕获 `this`(要所有权模型) |
| 465 | 方法撕下来当值 |
| 421 | 字符串拼接 |
| 402 | `for` 语句 |
| 312 | switch 当表达式 |
...
1. **"零拒绝"(Kernel 900 / 3665)不等于"翻译好了"**:引用其他库的东西仍靠手写桩。
