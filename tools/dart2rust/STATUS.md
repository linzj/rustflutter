...
| `E0425` 找不到名字 | 2793 |
| `E0433` 解析不到 | 879 |
| `E0038` trait 不是对象安全的 | 326 |
| `E0424` `self` 用在没有 self 的地方 | 152 |
| `E0277` trait 没实现 | 149 |
| `E0428` 重名 | 144 |

`E0433`(879)第 48 轮量过,点名最多的是 `Theme` 86、`Paint` 74、`_Set` 70、
`Tristate` 38——**这些多数是 flutter 自己的类**,查它们为什么没被发出来。

`E0424`(152)`self` 用在没有 self 的地方:**这是个真 bug**,
不是缺东西——自由函数里出现了 `self`,该查发射点。
