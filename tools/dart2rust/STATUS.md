...
5153 个错误。`E0425`(2995)里点名最多的,第 46 轮量过:

| | 次数 | 是什么 |
|---|---|---|
| `T` / `ChildType` / `K` / `V` | 332 / 29 / 27 / 26 | **泛型还有漏的地方** |
| `Set` / `Future` / `Duration` / `DateTime` / `ByteData` / `StackTrace` | 635 | `dart:core`,**要先处理 `external`** |
| `Matrix4` | 190 | `package:vector_math`,不在这个 dill 里 |
| `Paint` / `TextPainter` / `FocusNode` … | 各几十 | 被拒的类,要一个个看 |

**下一轮**:泛型漏的那 414 处——类和方法的参数都做了,还有地方在漏,
查是哪一种(typedef?mixin?扁平化时基类的参数?)。
