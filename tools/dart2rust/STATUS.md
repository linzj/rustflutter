...
## 下一步:字段赋值 + setter(1144 + 812 = 1956)

可变性那件大事,现在是队头。**动手前先规划**,这比之前任何一轮都大。

已量到的分布可以直接用(第 10 轮的 Kernel 计数):
字段写里 **6220 次穿过 `this`**(方法内改自己),**3869 次穿过别的对象**。
**先做第一类**,它只需要 `&mut self`;第二类要可变的接收者,难得多。

三个要先回答的问题:

1. 方法写字段就要 `&mut self`,而 `&mut` 会传染到调用者。**谁判断传染边界?**
2. `impl Add for Alignment` 这类 trait 方法签名固定(`self`),里面有赋值怎么办?
3. Dart 的 `a.x = 1`,若 `x` 有 setter 就要变成 `a.set_x(1)`——
   **这需要知道 `x` 有没有 setter**,是跨类信息。

## 当前队头(685 文件 / 3331 个类 / 11154 次拒绝)
...
闭包(768 + 555 = 1323)是继可变性之后的下一块,而且**它和可变性纠缠**:
Dart 的闭包捕获可变状态,Rust 的 `FnMut` 有借用规则。别分开规划。

## 两条要记住的警告

1. **"零拒绝"(819 / 3331)不等于"翻译好了"**:引用其他库的东西仍靠手写桩。
2. **`testdata` 的桩已经吃力**。下一堵墙是依赖图——翻译一个类要连带它依赖的一切,
   手写桩跟不上。真正的答案是翻译 `dart:core` / `dart:ui` 的最小子集。
