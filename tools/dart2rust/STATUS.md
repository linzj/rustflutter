...
(525 库 / 2743 类 / 1195 个零拒绝的类 / 11559 个成员发出 / 3393 次拒绝)

| 次数 | 要建的东西 |
|---|---|
| 581 + 423 | **闭包捕获 `this` / 撕方法——同一件事,见第 30 轮的测量,等对象表示定下来** |
| 144 | 写另一个对象的字段(穿过形参那半) |
| 140 | 非 const 静态字段(`LazyLock`) |
| 120 + 92 + 49 | `MapLiteral` / `ListConstant` / `MapConstant`——**要先定 `List`/`Map` 的表示** |
| 66 | 没有函数体 |
| 63 | `RecordLiteral` |
| 44 | `await` |

**下一轮**:集合。这不是一个 blocker,是"翻译 `dart:core` 最小子集"的头一步
——`List<T>` 是不是 `Vec<T>`、`Map` 的**插入顺序**要不要保(Dart 的 Map 是
有序的,`HashMap` 不是)。先量这些 map 是被查还是被遍历,再决定。
