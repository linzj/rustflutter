...
//!
//! Built as a staticlib and linked into a small C++ shim, because the engine
//! it draws through is C++. `rustflutter_app_main` is the real entry point.
...
/// headlessly instead, which is what the tests and CI use.
#[unsafe(no_mangle)]
pub extern "C" fn rustflutter_app_main(argc: c_int, argv: *const *const c_char) -> c_int {
    ...
    match png_output(&args) {
        ...
        None => match app.show(build(), "Hello, World! - rustflutter") {
            Ok(()) => 0,
            Err(err) => {
                eprintln!("rustflutter: could not show a window: {err}");
                1
            }
        },
    }
