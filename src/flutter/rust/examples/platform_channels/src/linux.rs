...
//! The Linux half of the probe, which does not exist yet.
//!
...
//! a `WM_CLOSE` the application refuses. The Linux host has no window to drive
//! yet -- see `rustflutter_host_stub.cc` -- so the input-driven half of this
//! example reports SKIP rather than pretending.
//!
...
///
/// `None` because there is no second way here: there is no Linux host window
