...
pub extern "C" fn rustflutter_app_main(argc: c_int, argv: *const *const c_char) -> c_int {
    ...
    match run(&options) {
        Ok(()) => 0,
        Err(code) => {
            eprintln!("hello_world_app: the host exited with {code}");
            1
        }
    }
}
