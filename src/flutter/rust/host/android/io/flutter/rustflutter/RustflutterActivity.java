...
public class RustflutterActivity extends Activity implements SurfaceHolder.Callback {
  ...
  private static final class HostView extends SurfaceView {
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
      return handleKey(keyCode, event, /* down= */ true);
    }

    @Override
    public boolean onKeyUp(int keyCode, KeyEvent event) {
      return handleKey(keyCode, event, /* down= */ false);
    }

    /**
     * Hardware keys, which a phone rarely has and a tablet with a keyboard does.
     *
     * <p>Two things happen to every key, in this order. It goes to the framework
     * as a key -- which is what a shortcut and the focus traversal are made of,
     * and what {@code onKeyUp} exists for at all, since a press with no release
     * leaves the framework believing a key is still held. Then, on the way down
     * and with a field attached, it edits: the editing keys move the caret,
    ...
    private boolean handleKey(int keyCode, KeyEvent event, boolean down) {
      ...
      // A control character is what a key *is*, not what it typed: Tab is a
      // traversal and Escape is a dismissal, and neither belongs in a text
      // field even though `getUnicodeChar` willingly returns 0x09 and 0x1b for
      // them. Ctrl or Alt held means the same thing -- the key is a shortcut,
      // not text.
      ...
      nativeKey(keyCode, event.getScanCode(), down, event.getRepeatCount() > 0, character);
  ...
  /**
   * One hardware key on its way to the framework.
   *
   * <p>Both of Android's numbers go over, because they answer different
   * questions: the key code is what the key means and the scan code is where it
   * is. The mapping to Flutter's values is in C++, beside the Windows one, so
   * that both are regenerated by one run of {@code gen_key_map.py}.
   */
  private static native void nativeKey(
      int keyCode, int scanCode, boolean down, boolean repeat, String character);
