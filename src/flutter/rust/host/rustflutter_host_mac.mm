...
//------------------------------------------------------------------------------
// Text input: the shared `TextInputHandler` over the engine's own
// `TextInputModel` -- see rustflutter_text_input.h, which grew out of this
// file and is now the iOS host's too. The macOS half that stays here is the
// `NSTextInputClient` on the content view below, which is what lets an input
// method engage at all.
